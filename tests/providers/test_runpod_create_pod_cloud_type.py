...
def test_default_cloud_type_stays_all() -> None:
    ...
    p.create_instance(InstanceSpec(image="runpod/pytorch:latest"))
    assert _input(_create_body(captured))["cloudType"] == "ALL"
...
def test_secure_cloud_type_emitted_on_wire() -> None:
    ...
    assert _input(_create_body(captured))["cloudType"] == "SECURE"
...
def test_community_cloud_type_emitted_on_wire() -> None:
    ...
    assert _input(_create_body(captured))["cloudType"] == "COMMUNITY"
