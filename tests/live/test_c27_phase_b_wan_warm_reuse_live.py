"""C27 Phase B live smoke — Wan + ComfyUI cold-skip OR PROVEN-PROTECTION.

Re-fires the C25 Task 4 / C26 Task 14 deferred acceptance gate with C27
restart-loop protections active. Two ``kinoforge generate`` subprocesses
60 s apart on real Wan 2.1 14B T2V; gen2 should auto-attach via B3
warm-reuse and finish in < 0.7 × gen1 wall (CLEAN-PASS).

If Wan regresses into the C26 Phase B symptom (container restart loop —
GPU/CPU/disk near zero with uptime_seconds < 90 every tick), the C27
predicate trips RESTART_LOOP_REAP, the pod is destroyed, gen1 aborts
via the cancel token → PROVEN-PROTECTION.

Either outcome closes the deferred gate.

Gated by ``KINOFORGE_LIVE_RUNPOD=1`` AND ``KINOFORGE_LIVE_TESTS=1``.
Live spend ceiling: $0.60.
Spec: docs/superpowers/specs/2026-06-13-c27-restart-loop-stall-detection-design.md §12.4.
"""
...
_CFG_PATH = Path("tests/live/cfg_c27_phase_b.yaml")
_PROMPT_PATH = Path("/workspace/prompt-field-realistic.txt")
_SIDECAR_PATH = Path("tests/live/_c27_phase_b_evidence.json")
_BUDGET_USD_CAP = 0.60
_GEN_TIMEOUT_S = 60.0 * 60.0  # 60 min per gen
_BETWEEN_GENS_SLEEP_S = 60.0
# Indicators that either util-aware predicate self-classified and tore down.
_PROTECTION_INDICATORS: tuple[str, ...] = (
    "RESTART_LOOP_REAP",
    "consecutive_low_uptime_count",
...
def _detect_protection(stderr: str, stdout: str) -> bool:
    """Heuristic: did either C26 STALL_REAP or C27 RESTART_LOOP_REAP fire?"""
    combined = (stderr or "") + "\n" + (stdout or "")
    return any(token in combined for token in _PROTECTION_INDICATORS)


def test_c27_phase_b_wan_warm_reuse_live() -> None:
    assert _CFG_PATH.exists(), f"cfg missing: {_CFG_PATH}"
    assert _PROMPT_PATH.exists(), f"prompt missing: {_PROMPT_PATH}"

    prompt = _PROMPT_PATH.read_text(encoding="utf-8")
    state_dir = (
        Path(os.environ.get("TMPDIR", "/tmp")) / f"c27-phase-b-{int(time.time())}"
    )
    state_dir.mkdir(parents=True, exist_ok=True)

    pod_id: str | None = None
    outcome: str = "INCONCLUSIVE"
    acceptance_path: str | None = None  # "CLEAN-PASS" or "PROVEN-PROTECTION"
    ...
    try:
        ...
        if r1.returncode != 0:
            if _detect_protection(r1.stderr, r1.stdout):
                outcome = "PROVEN-PROTECTION"
                acceptance_path = "PROVEN-PROTECTION"
            else:
                outcome = "FAILED"
                failure_reason = f"gen1 rc={r1.returncode} (no protection indicator)"
        else:
    ...
    sidecar: dict[str, Any] = {
        "task": "C27 Task 14 — Wan + ComfyUI 2-CLI cold-skip OR PROVEN-PROTECTION",
        "outcome": outcome,
    ...
    assert outcome in {"CLEAN-PASS", "PROVEN-PROTECTION"}, (
        f"Phase B did not close C25/C26 deferred gate: outcome={outcome!r} "
