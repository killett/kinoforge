...
def test_luma_ray_live_e2e_smoke(tmp_path: Path) -> None:
    ...
    cfg.engine.bedrock_video.output_s3_uri = f"s3://{bucket}/kinoforge-output/"
    ...
    # Build engine + backend (via raw cfg dict — BedrockVideoEngine adapter
    # consumes dict). Override the same field here too — this dict, not
    # `cfg`, is what actually drives the submit() call below.
    import yaml
