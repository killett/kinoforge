...
- **C30. Investigate why RunPod containers restart every ~30s during clone phase (Wan 1.3B cfg).** C28 Phase A v5 observed exactly this: with both `stall_reap_enabled` + `restart_loop_reap_enabled` set to `false` in the diagnostic cfg, the container entered a chronic restart loop with 20 trap fires uploaded over a 10 min window (uploads spaced ~31s, all identical state — `boot.log` = `Cloning into 'ComfyUI'...`, `models/diffusion_models` empty, no GPU processes). Diag bundle: `s3://kinoforge-pod-diagnostics/boot-logs/c28-phase-a-20260613T235704-a1/diag-*.txt` (20 files). Root cause is downstream of C28 — C28's diagnostic mechanism captured the loop in high fidelity, but the *why* (RunPod platform health-check killing the container at ~30 s grace, OR a `runpod/pytorch:2.4.0` base-image quirk that exits PID 1 fast, OR a Wan-1.3B custom-node import that crashes during the first `python -c "import comfy"` smoke) is its own investigation. Promote with a Phase A-style spec when the next attempt at running Wan on a stock RunPod pod is needed. C28 Phase B (pre-baked image) sidesteps this entirely by skipping clone — so this is mostly a curiosity for stock-image users.
- **C29. Heartbeat starts BEFORE wait_for_ready (boot-phase protection).** Surfaced by C28 Phase A v2 diagnostic. The kinoforge `hb_loop.start()` callsite lives at `orchestrator.py:1033`, AFTER `_provision_instance_and_build_backend` returns — which means the C26 STALL_REAP + C27 RESTART_LOOP_REAP predicates only protect AFTER `wait_for_ready` lifts. During the boot phase (provision_script + engine wait_for_ready), only `boot_timeout` protects. Reproducible evidence: C28 Phase A v2 pod `6f1kau8g9kjm4q` sat at GPU=0, CPU=1 for 25 min during a slow HF download; `cfg.compute.lifecycle.stall_window_s=1200` (20 min) should have fired but ledger showed no heartbeat fields at all — `hb_loop.start()` had not been called yet because `wait_for_ready` was still polling a 502 proxy. S3 diag snapshot at `s3://kinoforge-pod-diagnostics/boot-logs/c28-phase-a-20260613T223749-a1/diag-20260614T060748Z.txt`. Scope when promoted: move hb_loop construction into `_provision_instance_and_build_backend` right after `create_instance` returns (instance.id available), thread `cancel_token` + `heartbeat_loop_factory` through the function signature, return the started loop to `deploy_session` for steady-state continuation. Risk: cross-cuts the cache-hit / warm-reuse path; needs careful test sweep. Promote when next slow-boot leaks unprotected wall-clock budget.
