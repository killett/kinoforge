...
**STATUS INDEX (current as of 2026-09-11. Provenance, newest first: U36 filed 2026-09-11 from
nine live attempts; U34 live-run PARTIAL 2026-09-11 (no regression, discriminator NOT yet run);
...
Thirty-six items, U1-U36. **Thirty are fixed** (U1, U2, U3, U4, U6, U7, U8, U9, U11,
U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30, U31,
U32, U33), **two are
partly fixed** (U5, U10), **one is fixed but OWES a live proof** (U24), **three are newly OPEN**
(U34 — U33's fix is live-proven to cause no regression but it is NOT settled whether the cu124
index or the base image supplies `torch 2.6.0+cu124`; the committed probe that separates them is
one ~$0.015 command and is the next session's first action. U35 — the serverless cfg renders now
but the snapshot harness cannot capture its create call, so it is still unfrozen. U36 — RunPod's
advertised `lowestPrice` under-predicts the realized rate by 5-10x, so the default cap turns a
routine run into a create-then-destroy loop; the guard is correct, the loop is the cost), and
**U13 is NOT
...
1. **U34 — ONE command, ~$0.015, and it is the first thing to run.** The probe cfg is already
   committed and ready: `tests/live/probes/u34-spandrel-no-pytorch-index.yaml` (`223532c9`).
