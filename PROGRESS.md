...
shipped 1.3B grid cfg's offer list because of two NEW defects filed 2026-09-14: **U43** (an
accelerator name the provider's catalog does not carry is silently inert under `filter_offers`'
exact-match rank — seven shipped cfgs affected, and it is the whole of U36's "L4-first ordering"
cause) and **U44** (`cuda="12.8"` hardcoded on every RunPod offer, so an AMD card passes `min_cuda`).
...
Forty-four items, U1-U44. **Thirty-nine are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
U11, U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30,
U31, U32, U33, U34, U36, U37, U38, U39, U40, U41, U42), **two are
partly fixed** (U5, U10), **NO live proof is owed** (U24 discharged 2026-09-11), **three are OPEN**
(U35 — the serverless cfg renders now but the snapshot harness cannot capture its create call, so
it is still unfrozen. U43 — an accelerator name the provider catalog does not carry ranks no higher than an unlisted one,
silently; seven shipped cfgs affected. U44 — `cuda="12.8"` hardcoded on every RunPod offer, so an
AMD MI300X passes a CUDA filter it cannot satisfy. U37 — CLOSED the same day it was filed: the FlashVSR cu128 half
...
| U43 | OPEN — filed 2026-09-14, found while root-causing U36 | **An accelerator name the provider's catalog does not carry is SILENTLY INERT.** `filter_offers` ranks by exact string equality (`o.gpu_type in placement.accelerators`, `core/offers.py:57-60`); a name that matches nothing returns `len(accelerators)` — the same rank as a GPU nobody asked for. No warning, no error, and the cfg reads as though its preference is being honoured. **Seven shipped cfgs name GPUs RunPod does not have**: `NVIDIA RTX 4090` (the catalog id is `NVIDIA GeForce RTX 4090`) in `grids/runpod-diffusers-wan-2_1-1_3b-base.yaml`, `grids/runpod-diffusers-wan-2_1-1_3b-base-no-loras.yaml`, `runpod-diffusers-wan-2_1-1_3b-t2v-lora-flexible-warm-reuse-smoke.yaml`, `runpod-diffusers-wan-2_1-1_3b-t2v-strength-grid.yaml` and `tests/live/probes/u24-swap-base.yaml`; `RTX 4090` + `A100 40GB` in `runpod-diffusers-serverless.yaml`; `A100 80GB` + `H100 80GB` in `runpod-comfyui-wan-2_2-14b-t2v.yaml`. **This is the whole of U36's "L4-first ordering" cause.** Proven through the real code path against the live catalog: the shipped 1.3B grid cfg's `find_offers` returns **L4 at rank 0** (it is the one name that does match) and the 4090 it meant to lead with at **rank 4**, behind an AMD MI300X. `7ba8c3c1`, which dodged the ordering on the U34 probe cfg, worked only because that cfg spells the id correctly — so the dodge was real but did not generalise, and nobody could see why. **The product fix is the WARNING, not the seven renames**: a cfg naming a GPU its provider does not have must say so out loud, the way `validation/unsupported_fields` already reports ignored `disk_gb`. Rename the cfgs as well, but a rename alone leaves the next typo just as invisible |
| U44 | OPEN — filed 2026-09-14, found while root-causing U36 | **`find_offers` hardcodes `cuda="12.8"` on every RunPod offer** (`providers/runpod/__init__.py:628`, commented "RunPod standard image baseline") regardless of what the GPU actually is. RunPod's catalog carries **AMD Instinct MI300X OAM**, which has no CUDA at all; it passes `min_cuda: "12.4"` unchallenged and is offered for a CUDA workload. In the shipped 1.3B grid cfg it currently ranks **second** — ahead of every NVIDIA GPU the cfg named — because U43 makes the intended 4090 preference inert. Advertised $0.500 unfiltered, **$2.390 secure**, so it is also the most expensive way to fail. The offer's `cuda` field is the only thing standing between a CUDA-only engine and an AMD card, and today it is a constant. Fix shape: derive it from the GPU (or drop non-CUDA vendors from the RunPod catalog outright), and note that `Offer.cuda` being a provider-asserted constant is worth auditing on the other enumerating providers too |
...
### NEXT ACTION (single, current as of 2026-09-14)

**U43 — make an accelerator name the provider's catalog does not carry say so out loud.** It is the
last thing standing between this ledger and the reason U36 took nine live attempts to understand:
`filter_offers` ranks by exact string equality, so `NVIDIA RTX 4090` against a catalog id of
`NVIDIA GeForce RTX 4090` ranks no higher than a GPU nobody asked for — silently. **Seven shipped
cfgs** are affected (the list is in U43's row). The product fix is the WARNING, not the seven
renames; rename them too, but a rename alone leaves the next typo just as invisible. Note the
validation `Check` protocol is STATIC and has no network, so this cannot be a `checks/` module that
enumerates a live catalog — the natural site is where the catalog is already in hand and
provider-agnostic: `filter_offers` in `core/offers.py`, which sees both the offers and the requested
names, and would cover SkyPilot's cfgs at the same time.

Then **U44** (`cuda="12.8"` hardcoded on every RunPod offer, so an AMD MI300X passes `min_cuda:
"12.4"`). U36's fix incidentally removed the MI300X from the shipped 1.3B grid cfg — its secure rate
$2.390 is over that cfg's $0.60 cap — but the defect stands, because an `any` cfg or a higher cap
offers it again. While fixing it, audit whether `Offer.cuda` is a provider-asserted constant on the
other enumerating providers too.

After that **U35** is harness-only and is the last item from the original matrix.
