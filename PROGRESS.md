...
**STATUS INDEX (current as of 2026-09-12. Provenance, newest first: the SHIPPED 1.3B swap grid
...
Forty-two items, U1-U42. **Thirty-eight are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
...
partly fixed** (U5, U10), **NO live proof is owed** (U24 discharged 2026-09-11), **two are OPEN**
(U35 — the serverless cfg renders now but the snapshot harness cannot capture its create call, so
it is still unfrozen. U36 — RunPod's advertised `lowestPrice` under-predicts the realized rate by
5-10x, so the default cap turns a routine run into a create-then-destroy loop; the guard is
correct, the loop is the cost, and the immediate ORDERING cause is now dodged in the probe cfg
though the underlying item stands. U37 — CLOSED the same day it was filed: the FlashVSR cu128 half
...
| U36 | OPEN — filed 2026-09-11 from nine live attempts, NOT a correctness defect | **RunPod's advertised `lowestPrice` under-predicts the realized pod rate by 5-10x, so the default `max_usd_per_hr = 2.20` turns a routine run into a create-then-destroy loop.** Measured 2026-09-11 while trying to land the U34 probe: RunPod's `gpuTypes.lowestPrice.uninterruptablePrice` for this cfg's whole allowlist reads **RTX 3090 $0.22, RTX 4090 $0.34, L4 $0.44** (A4000/A5000 null = no stock), and `find_offers` builds its `Offer.cost_rate_usd_per_hr` from exactly that field (`providers/runpod/__init__.py:576-592`). RunPod then places the pod on whatever machine is free, and the pod's own `costPerHr` came back **$2.39/hr on seven of nine attempts** — so S4's realized-rate check (`_enforce_rate_cap`) destroyed each one with `RateCapExceeded: realized $2.3900/hr exceeds cap $2.2000/hr`. **F4 worked exactly as designed — that is the point, not the complaint.** The eighth attempt landed an RTX 4090 at **$0.74/hr** (still 2.2x the advertised $0.34) and ran green, so the pool is not uniformly expensive; it moved within ~40 minutes. **What this costs:** every capped attempt still creates and destroys a real pod (~$0.008 each; nine attempts ~= $0.07 for zero output), and there is no pre-launch rate refusal on the RunPod path — S5 added one for SkyPilot ('an over-cap plan is refused BEFORE `sky.launch`'), and the RunPod equivalent cannot exist in the same form because the realized rate is not knowable until the pod is placed. **Fix directions, none obviously right:** (a) treat the `lowestPrice`-derived estimate as a lower bound and widen the default cap, which weakens the guard; (b) retry inside `create_instance` on `RateCapExceeded` with the over-priced machine excluded, so one CLI invocation converges instead of the operator looping; (c) surface the advertised-vs-realized gap in the error so the operator sees `advertised $0.34 -> realized $2.39` rather than a bare cap breach. **(b) is the one that would have saved this session's $0.07 and ~10 minutes.** Do NOT 'fix' this by raising the cap in the shipped cfgs |
...
### NEXT ACTION (single, current as of 2026-09-12)

**Measure U36 on SECURE hosts before building anything.** This session's last run billed $0.27/hr
secure against $0.49-$0.59 community, which suggests the advertised-vs-realized gap U36 describes
is largely a community-pool phenomenon. If that holds, pinning `cloud_type: secure` is a smaller
and more honest fix than the in-provider retry — and the retry may not be needed at all. That is
one `find_offers` + one create away from being known, and it changes what U36 should do. Original
framing, still valid if the measurement disagrees:
its own entry names option (b) — retry inside `create_instance` on `RateCapExceeded` with the
over-priced machine excluded, so one CLI invocation converges instead of the operator looping.
This session paid U36's tax three separate times (nine attempts on U34's probe, then the shipped
grid cfg being unplaceable twice), so the case for (b) is now empirical rather than predicted.
**Do NOT raise the caps in shipped cfgs** — but DO consider whether the shipped 1.3B grid cfg's
$0.40 cap should be revisited on its own merits, since no current RunPod offer can satisfy it.
After that, **U35** is harness-only and is the last open item.
