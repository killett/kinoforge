...
- **C26. RunPod util-aware stall classify.** Spun off from C25 Task 4 closeout (2026-06-13). The current `classify()` (`src/kinoforge/core/reaper.py`) gates on liveness + heartbeat freshness + age only — a pod that booted cleanly but whose in-pod provision script has stalled (model download hung, ComfyUI never bound port, container restart loop) is still classified `LIVE` because heartbeats tick normally from the orchestrator side. RunPod's GraphQL `pod.runtime` exposes `gpus { id gpuUtilPercent memoryUtilPercent }` and `container { cpuPercent memoryPercent }` (probed 2026-06-13). Scope: (a) extend `RunPodGraphQLHeartbeatEndpoint.write` (or add a sibling `_capture_util_snapshot`) to fetch `runtime.gpus` + `runtime.container` per tick and write into the ledger under `last_util` + `last_util_tick`; (b) add a new `Verdict.STALL_REAP` value with thresholds (e.g. "gpu_util < 5 % AND container.cpu < 20 % for N consecutive ticks → STALL"); (c) thresholds become `compute.lifecycle.stall_*` cfg knobs; (d) classify uses the new fields when present, falls back to current behaviour when absent; (e) live smoke replicates the C25 Task 4 Wan + ComfyUI workload — STALL must fire within the cfg-defined window, and the smoke must auto-tear-down. No persistent state changes for non-RunPod providers (substrate Protocol unchanged). Promote to its own spec + plan when the next operator wants production stall detection on a Wan-class workload.
