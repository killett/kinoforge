...
**STATUS INDEX (current as of 2026-09-14. Provenance, newest first: **U36 is FIXED (`cd133238`) and it was
...
Forty-five items, U1-U45. **Forty-two are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
...
partly fixed** (U5, U10), **NO live proof is owed** (U24 discharged 2026-09-11), **one is OPEN**
(U45 — the serverless create path discards `placement` and sends no `env`; filed 2026-09-14 by the
golden U35 brought into existence, and NOT fixed because what a serverless endpoint really costs has
not been measured. U43 and U44 were both filed AND fixed on 2026-09-14, hours apart, out of U36's root cause. U37 — CLOSED the same day it was filed: the FlashVSR cu128 half
...
### NEXT ACTION (single, current as of 2026-09-14, third update)

**U45 — measure what a RunPod serverless endpoint actually costs, BEFORE changing anything.** This
is the same discipline that turned U36 from a mystery into a one-field fix in an afternoon: the
serverless path hardcodes `"gpuIds": "ADA_24"`, discards the `offer` it is handed, and reports
`cost_rate_usd_per_hr=0.0`, which satisfies `_enforce_rate_cap` vacuously. Whether that 0.0 is a
defect or an honest "per-execution billing does not have an hourly rate" is **not known**, and the
fix differs completely depending on the answer. One endpoint, one job, one billing read settles it.
Do NOT start by wiring `placement` into the mutation — find out what the units are first.

The sharper half needs no measurement and can go first if preferred: the serverless mutation carries
**no `env` at all**, so there is no `KINOFORGE_PROVISION_SCRIPT` and no `RUNPOD_TERMINATE_KEY` on
that path, where the pod path injects both. Whatever that path is for today, it is not booting a
kinoforge server. Both facts are frozen in the golden added 2026-09-14, so a fix shows as a diff.
...
- **`Offer.cuda` is a provider-asserted constant on the other enumerating providers too.** U44 fixed
  RunPod's; Modal, SkyPilot and Local were not audited. Same question: does anything in that catalog
  fail to satisfy what the field claims?
- **RunPod's catalog contains a GPU type whose id is the literal string `unknown`**, priced and
  bookable. U44 leaves it on the CUDA baseline rather than widening its vendor blocklist, but
  nothing anywhere decides what booking `unknown` should mean.
