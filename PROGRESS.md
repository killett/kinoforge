...
Forty-four items, U1-U44. **Forty-one are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
U11, U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30,
U31, U32, U33, U34, U36, U37, U38, U39, U40, U41, U42, U43, U44), **two are
partly fixed** (U5, U10), **NO live proof is owed** (U24 discharged 2026-09-11), **one is OPEN**
(U35 — the serverless cfg renders now but the snapshot harness cannot capture its create call, so
it is still unfrozen. U43 and U44 were both filed AND fixed on 2026-09-14, hours apart, out of U36's root cause. U37 — CLOSED the same day it was filed: the FlashVSR cu128 half
...
### NEXT ACTION (single, current as of 2026-09-14, second update)

**U35 — teach the snapshot harness RunPod's create-serverless mutation, then delete the
`EXCLUDED_CONFIGS` entry.** It is the last open item from the Modal command matrix and the only one
left that is harness-only: `runpod-diffusers-serverless.yaml` is deployable and simply unfrozen, so
the golden ratchet cannot see a wire change to the ONE serverless cfg shipped. The capture dies on
`ValueError: RunPod create-serverless returned no endpoint id` because `mode: serverless` routes to
`saveTemplate` while the stub transport answers every mutation with the on-demand
`podFindAndDeployOnDemand` shape. Fix the stub, drop the exclusion, review the new golden.

Worth doing alongside it, both surfaced 2026-09-14 and neither urgent:

- **`Offer.cuda` is a provider-asserted constant on the other enumerating providers too.** U44 fixed
  RunPod's; Modal, SkyPilot and Local were not audited. The question is the same one: does anything
  in that catalog fail to satisfy what the field claims?
- **RunPod's catalog contains a GPU type whose id is the literal string `unknown`**, priced and
  bookable. U44 deliberately leaves it on the CUDA baseline rather than widening its blocklist to
  cover it, but nothing anywhere decides what booking `unknown` should mean.
