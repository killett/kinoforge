...
bookable" was an assertion stitched across two probes that never overlapped, and is corrected on its
row. No code change, deliberately. Before that: **U48 FIXED** — the `Offer.cuda`
audit U44 asked for, run across the other three enumerating providers, and **SkyPilot's was worse
than RunPod's**: it reported a CUDA version from a lookup that CANNOT succeed (`InstanceTypeInfo` has
no `cuda` field), so every offer carried the same constant `"12.0"` and any cfg leaving `min_cuda` at
its `"12.8"` default had its **entire catalog filtered away**. All five shipped SkyPilot cfgs had
been bent around it. Modal and Local audited and deliberately NOT changed (Modal's is a real,
sourced, NVIDIA-only fleet claim; Local's is a fixture on an unbilled in-process fake). **Score: 2 of
4 providers asserted a CUDA version they could not know, and both were load-bearing.** Before that:
**U47 FIXED** — `CheckRegistry.
...
never worked (declares `EndpointInput!`, calls `saveTemplate` which takes `SaveTemplateInput`;
RunPod answers `GRAPHQL_VALIDATION_FAILED` as a raw HTTP 400, the shape CLAUDE.md warns reads as an
outage), diagnosed for **$0.00** because GraphQL validates before it executes — and **U45** — that
path hardcodes `gpuIds: "ADA_24"`, discards the `offer` it is handed, and sends no `env` at all.
Before that: **U36 FIXED (`cd133238`) and it was
never a pricing mystery** — four real pods for ~$0.01 measured realized == advertised(pool) EXACTLY,
4 arms out of 4, so RunPod's rate is perfectly deterministic and the whole gap is `find_offers`
sending an UNFILTERED `lowestPrice` (which answers for the COMMUNITY pool) while the create books
`cloudType: SECURE`. All three fix directions U36 proposed are retired; the fix is to pass the cfg's
own `cloud_type` into the query. That measurement also showed U36's "realized $2.39 on the L4" was a
MISATTRIBUTION — the L4 bills $0.490 and $2.390 is the **AMD MI300X**, which ranks SECOND in the
shipped 1.3B grid cfg's offer list because of two NEW defects filed AND FIXED 2026-09-14:
**U43** (`9c45675c` — an accelerator name the provider's catalog does not carry is silently inert
under `filter_offers`' exact-match rank; seven shipped cfgs affected, it is the whole of U36's
"L4-first ordering" cause, and its own first cut then shipped two false positives that only running
it against the live catalog exposed) and **U44** (`cuda="12.8"` hardcoded on every RunPod offer, so
an AMD card passed `min_cuda`; fixed by naming non-CUDA VENDORS rather than allowlisting a brand
prefix, which would have dropped RunPod's CUDA-capable Tesla V100s).
The 2026-09-12 "secure is cheaper" reading turns out to be CORRECT at $0.27 (the RTX A5000's secure
rate, confirmed once its stock returned mid-session — an earlier note in this section calling it
unexplained was itself wrong and is corrected in place); its INTERPRETATION was the error, because it
compared different GPUs. Held to the same GPU, secure is strictly 1.5-2.3x community. Before that: the SHIPPED 1.3B swap grid
now runs end-to-end (`status=full`, pod `v08y8axqfzu4fo`, frame QA PASS) after an operator cap
decision plus THREE distinct defects found by five live runs — U42 (community-pool reclamation,
the cfg was the only RunPod one not pinned to secure), U41 (stall grace ignored the cfg's declared
boot_timeout, killing pods at 1/10th of their budget) and U40 (the flatline predicate cannot see a
network-bound download on RunPod; fixed, then proven INSUFFICIENT because its log channel is not
reachable in the window it targets). **`max_usd_per_hr` on that cfg was raised 0.40 -> 0.60 by
operator decision 2026-09-12** (`2c446eef`) — at 0.40 no current RunPod offer could satisfy it.
U24 CLOSED
2026-09-11 — live-proven at last, but only after its own proof found the fix was ONE FIELD SHORT
(U39); U39 filed+fixed 2026-09-11 (index rows never emitted the `ports` tag RunPod derives proxy
URLs from, so an ephemeral pod was findable and not attachable); U38 filed+fixed 2026-09-11 (a
grid that failed printed `status=partial` and an empty directory, discarding the cell stderr it
had already captured — the fix is what made U39 findable); U34 CLOSED
2026-09-11 — ANSWERED by a paired live A/B (~$0.05): the cu124 INDEX supplies the wheel (bare
`2.6.0` under PyPI vs `2.6.0+cu124` under cu124, pods `67k6ihp9w1usli` / `u7g1h7fu0j7scm`) and the
two arms' outputs are BYTE-IDENTICAL; U37 filed 2026-09-11, split out of U34's resolution (the
FlashVSR x4 cfg's cu128 intent is expressed by no cfg and provably installs +cu124); U36 filed
2026-09-11 from nine live attempts, and its L4-first ORDERING cause was diagnosed and dodged
2026-09-11 (`7ba8c3c1`) without raising any cap;
U33 fixed 2026-09-11
...
aborted the Modal image build on an unbound `${HF_TOKEN}`). **U19 and U6 are both LIVE-PROVEN as of 2026-09-11, for $0.036 and $0.014** — the first
`kinoforge interpolate` warm-attach there has ever been, and the first `kinoforge deploy` that
reaches a serving pod. **U13 is the only item with nothing left to do offline:** four mechanisms
are eliminated by measurement and the holder appears only on a real failing Modal upscale. **All
three items filed as "past the one-guard bar" — U24, U31 and (earlier) U28 — turned out not to
be**, each for the same reason: the assessment was reasoned from the shape of the mechanism
without grepping the layer it claimed was missing. **U31 was filed 2026-09-10** while measuring
...
| U13 | **NOT REPRODUCIBLE at HEAD** — two live runs 2026-09-11 (~$0.4), nothing fixed | The hang was hunted live and **did not happen**. Both runs reached a GENUINE server-side failure on Modal and the CLI exited cleanly (`returncode 1`), with the watcher's 2-second poll never even reaching its 45 s grace clock. **Tier 1** — cheap T4 spandrel pod (`tests/live/_u13_spandrel_modal_cfg.yaml`): `UpscaleFailed: upscale job u-0ec19ee6… failed on server: Could not load meta information`. **Tier 2** — the SHIPPED FlashVSR A100 cfg, the exact T2-01/T2-05b family: `UpscaleFailed: upscale job u-0a87e2f3… failed on server: \u0060pyav\u0060 can not handle the given uri.` Tier 2 was run precisely because a clean exit on one engine is not a clean exit on the one where the hang was seen. Evidence committed: `tests/live/_u13_hang_probe_evidence_*.json`; harness `tests/live/_u13_hang_probe.py` (the exception is allowed to ESCAPE `main()`, and the hang would have been dumped via SIGABRT — a mechanism verified offline BEFORE spending, since a `kill -9` with no stack is what lost the 2026-09-06 evidence). **⚠️ The one dimension NOT reproduced, stated plainly:** both induced failures happen at DECODE, before any inference, whereas T2-01 failed at the mp4 ENCODE after a full multi-minute run. That path can no longer be reached on purpose — the `av<18` pin (U12) fixed the writer — so a holder created only during a long inference/poll cycle would not have been caught here. **U31 is NOT the explanation**, and that is settled without spending: `upscale` submits nothing to the pool (`UpscaleStage` calls `engine.upscale` directly; `GenerateClipStage`, the only `pool.submit` caller, is skipped), and the 2026-09-10 offline run exited in 0.8 s while the U31 defect was still present. **Nothing was fixed** — this is a negative result, not a closure. Reopen on the next sighting and run the committed probes first; they now cost minutes, not a hunt |
...
| U19 | FIXED + LIVE-PROVEN 2026-09-11 (`96d6920e`, $0.036) | `_capability_for_model` had no term for interpolation, so a loaded `rife-*` pipeline mapped to `None`, never reached `/health`'s `capabilities[]`, and `kinoforge interpolate` had to be carved out of the T14 pre-flight entirely — a half-failed RIFE pod looked identical to a healthy one. **Both halves shipped together**, because either alone is a defect: the server maps `rife-` → `"interpolate"`, and `_cfg_want_stages` returns `capability_key().stages` unfiltered (`_HEALTH_UNGATEABLE_STAGES` deleted). `test_no_shipped_cfg_demands_a_stage_no_pod_can_advertise` already failed on the half-done state, with `ADVERTISABLE_STAGES` hand-transcribed so the two sides cannot silently agree on a new term. **21 goldens moved** — the server module is embedded in the provision script — and the regeneration was REVIEWED, not accepted: decoding every changed payload shows the same 9 added lines and nothing else, with the surrounding script byte-identical in all 20 launch payloads and in `_golden_provision.json`. **LIVE-PROVEN on Modal, $0.036, pod `interpolate-20260911-000240`** (cold `✓ App deployed in 161.523s`, a full re-bake since the embedded module changed; run 2 from a fresh process attached in 11 s with `✓ App deployed` and `✓ Created objects` both at count 0). **The attach is NOT the evidence** — under the old carve-out the gate was skipped, so an attach would have happened either way; that trap is the U3 shape and it is why the proof is the pod's own answer: `GET /health` → `capabilities: ["interpolate", "upload"]` with `models: [{name: rife-rife426, on_device: cuda, ready: true}]`, the exact term this entry recorded as unadvertisable. `GET /util` at the same moment read `gpu 0.0 / cpu 1.1 / mem 1.2, uptime 195s`. Frame-QA **PASS** on both outputs (480²/60fps/304f, matching §25 and the source's duration to 4 ms; no ghosting, no colour cast, source-faithful motion) with the honest limit stated: 5 sampled frames cannot prove inter-frame smoothness. Teardown verified from fresh processes. See `successful-generations.md` §25's See-also |
...
| U22 | FIXED, LIVE-PROVEN 2026-09-10 ($0.00 extra — shared U16's pod) | **The deferral happened, and the proof is TICK 2 specifically.** Against RunPod `kinoforge-5af92f87`, aged past the gate and idle, three real ticks 20 s apart read `LIVE / LIVE / REAPED`. **Tick 2 is the clean evidence and tick 1 is NOT:** at tick 1 the pod was still early-boot (`runtime = null`), so both readings were `None` and the old conservative-on-ignorance rule would have returned LIVE anyway — that tick proves nothing. By tick 2 the readings were numeric (a direct probe at 19:07:40 read `gpu=0.0 cpu=0.0`), the age gate was long past, and the pre-fix predicate replayed on exactly that state returns **True** while the post-fix one returns **False** — i.e. the old code reaps there and the new code defers, which is the whole defect in one tick. Tick 3 then reaped on three consecutive samples and destroyed the pod. **This run also EXPOSED a new defect that undermines this very fix — see U30**, found precisely because the recorded evidence showed `gpu_util_pct: null` and the tick attribution had to be checked rather than assumed. The orphan verdict rests on `ephemeral_orphan_samples` CONSECUTIVE low-util readings (default **3**, `CLAUDE.md`'s live rule) counting the current probe, matching `_ephemeral_stall_predicate` twenty lines away. **The window applies only where samples can exist** — an operator decision, not an inferred one: `stall_history` is `SweeperLoop`-owned, so `kinoforge reap` one-shot (a single tick in a fresh process, nowhere to bank a second sample) keeps the single-sample rule behind its two opt-ins + the age floor; requiring a window there would have made `reap --apply --include-orphans` a permanent no-op and regressed exactly what U28's $0.0406 live proof established the day before. `ephemeral_orphan_samples: 1` restores the old rule in the daemon too. **The fix's own trap, caught and pinned:** the deque it reads was sized from `stall_window_s` alone and collapsed to maxlen 1 under `stall_reap_enabled: false`, which would have "closed" U22 by making ORPHAN_REAP silently unreachable; `maxlen` is now `max(stall_maxlen, samples - 1)` — a FLOOR that does not shorten the wider stall window (pinned both ways, since STALL_REAP is inside `DEFAULT_APPLY_POLICY` and acts with no opt-in). Default 3 lives in `reaper.py`, not only cfg, so a threshold dict that drops the key fails SAFE — the U20 shape mirrored, where defaulting to `1` would have reinstated U22 with every test still green. Offline only: `pixi run test` → 5562 passed; eleven new tests, RED confirmed on five before implementation. ~~**Owed: a live re-run of the U9 cell**~~ — **DISCHARGED 2026-09-10** by the run described at the head of this row, which is the U9 cell's shape (idle ephemeral pod, sweeper ticks, `--include-orphans`) on RunPod rather than Modal |
...
| U24 | **CLOSED — FIXED + LIVE-PROVEN 2026-09-11** (`ee2dc937` + **`8be27067`**, ~$0.11 over two runs). The proof needed a SECOND fix (U39) | **LIVE-PROVEN 2026-09-11, and the proof found that this fix was ONE FIELD SHORT.** First attempt (pod `85ms24bc6kuc1m`, `kinoforge-8b2f2abb`): cell 0 cold-booted and generated at 100% GPU, then cell 1 **refused to attach** — `runpod.ensure_endpoints returned an empty map, given a seed carrying ports=['8000','8001']`. The index lookup this entry added worked; what failed is one step later, and is filed and fixed as **U39** — `to_entry_dict` never emitted `tags["ports"]`, which is what RunPod derives its proxy URLs from, so the repairing door was asked to rebuild endpoints without being told which ports. *This entry made the pod findable and not attachable.* **Second attempt, with U39 fixed: FULL.** Pod `kp4d0mcqvfa4zj` (`kinoforge-7097113c`, $0.49/hr), `status=full`, exit 0, composed 1440x480 mp4. All four criteria met: **(1)** cell 1 cold-booted under an opaque `kinoforge-<8hex>` name — note the observed RunPod form is `kinoforge-7097113c`, NOT the `kinoforge-eph-<8hex>` this entry predicted (that prefix is Modal's); **(2)** cells 2 and 3 ATTACHED — the utilisation monitor polled ten times across the whole ~7 min run and saw **exactly one pod id**, where three cold boots would have produced three ids and ~5 min of boot each; **(3)** the controller's destroy fired — pod gone, confirmed from a FRESH process (`kinoforge list` both lines, provider-side `list_instances()` empty); **(4)** the index was empty afterwards. **Frame QA PASS**, and the composition carries its own extra evidence: the three captioned cells are VISIBLY DIFFERENT (dark dress / red dress / blue dress with butterflies), so the LoRA stack genuinely swapped between cells on the shared warm pod — had the swap silently no-op'd, all three columns would look identical. **Run against probe copies, not the shipped cfg**, and that is U36: `examples/configs/grids/runpod-diffusers-wan-2_1-1_3b-base.yaml` caps at $0.40/hr while the pod realized $0.49, so the SHIPPED CFG IS CURRENTLY UNPLACEABLE ON RUNPOD — the first run died on `RateCapExceeded` before any cell. Probes: `tests/live/probes/u24-swap-group.grid.yaml` + `u24-swap-base.yaml`, differing from the shipped pair by one line. **The cause was only visible because of U38**: the first grid failure printed `status=partial` and an empty directory, and the captured stderr was discarded by the summary | The refusal is lifted and its premise was wrong. The entry read "a lora_swap group shares ONE pod, so no single cell's `--ephemeral` can own the shared pod's `delete_on_completion`". **`delete_on_completion` does not govern pod teardown at all** — its three consumers are hosted-job deletion (`remote_backend`), the store-run scrub (`EphemeralSession.__exit__`) and the profile alias (`provision_state`) — and **the shared pod has always been destroyed by `_run_swap_group`'s own `finally`** (`pixi run kinoforge destroy --id`), on every exit path. The controller owns teardown, which is exactly the ownership the refusal said did not exist. **The real blocker was a third thing the entry never mentions:** cells 2..N resolve `--attach-pod` through `Ledger.read`, and under STRICT the ledger is a PER-PROCESS in-memory mirror (`ledger_record=False` diverts writes to `session.in_memory_ledger`), so cell 1's pod is invisible to its siblings. `_resolve_attach_pod` now falls back to the ephemeral index — the cross-process handoff the entry says "would need" to be built, which **already existed** and which `_scan_warm_candidates` and `kinoforge reap` (U18) both union in; this was the third read site and the only one still missing it. Index-row teardown is already covered: the controller's `kinoforge destroy --id` goes through `destroy_confirmed(..., ephemeral_index=...)`, which removes the row on a confirmed destroy. **Six tests** — the index fallback (RED first), the WAK gate still applying to an index-sourced row (falsified by deleting the check), an unknown id still refusing, `--ephemeral` reaching every swap cell's argv in ROOT position (appending it after the subcommand is what made Task 6's live cell A2 exit 2), and a plain grid getting no flag. The stub needed a provision record and a destroy branch before a swap chain could run in tests AT ALL — which is why the refusal's own test never exercised a second cell. **⚠️ LIVE PROOF OWED, and no $0 stand-in was accepted:** `grid --dry-run` returns BEFORE `run_grid`, so it never reached the refusal and demonstrates nothing either way — that near-miss is recorded because it would have read as evidence. The proof needs a real swap group: cell 1 cold-boots under an opaque `kinoforge-eph-<8hex>` name, cell 2 attaches from a separate process with no `✓ App deployed`, the controller destroys the pod, and the index is empty afterwards. Same standing as U17 and U21 |
| U25 | FIXED 2026-09-10 (`602c3f7a`, $0.00) | Nothing under `core/grid/` consulted the ephemeral policy, so `grid --ephemeral` wrote the whole `output/_grid_<id>/` tree — per-cell cfgs (prompt + effective cfg), the captured child stderr, per-cell mp4s, provision records — and cleaned up none of it, every path stamped with the local timestamp the flag exists to keep out of durable state. **The tree cannot be skipped, so it MOVES:** ffmpeg composes FROM those mp4s, so under a strict policy the per-cell working root is a private `tempfile.mkdtemp` and the whole run body is wrapped in try/finally — `run_grid` has FOUR returns (teardown / partial / ffmpeg / full) and the runs that go wrong are the ones with most left behind. **What stays is the deliverable, in both shapes, and that is a decision worth stating rather than inferring:** "skip local writes" has never meant skipping the artifact — `generate --ephemeral` still publishes its clip — so the composed mp4 keeps its place under the operator's output dir, and a run that could not compose still hands over `_grid_<id>_partial/` with the clips it did make. Deleting those would destroy the run's OUTPUT rather than its traces. `_retain_local_intermediates` reads the active session's policy and falls back to the `ephemeral=` kwarg only when no session is bound (from the CLI one always is); `policy.ledger_record` stands in for "is this session strict?", the same proxy `matcher.py`'s `always_reprobe` already uses — grid intermediates are not one of `EphemeralPolicy`'s five named gates, and mapping them onto an unrelated gate would read worse than reading strictness directly, so **no policy field was added** and the spec's 10-field count test stands. **Five tests; three RED first** (intermediates gone with the composed mp4 kept; no `.stderr.txt` anywhere under `output/`; the POLICY governing when only the session and not the kwarg says ephemeral — which is U25's literal complaint). **The other two were passing vacuously** before the scratch dir existed, so rather than bank them as green each was **falsified by deliberately breaking the implementation**: cleanup attached to the success return alone went RED on the failure-path test, and cleanup ordered before `_move_to_partial_dir` harvests from the tree went RED on the partial-deliverable test. A guard that has never failed is not yet a guard. This entry's own reproducer probe now answers `True` |
...
| U33 | FIXED 2026-09-11, OFFLINE-PROVEN — and it was THREE defects, not one | **Every diffusers build passed `pip install -q --extra-index-url None`, so the PyTorch CUDA index was never applied.** `model_dump()` emits an unset `str | None = None` field as a PRESENT key whose value is `None`, so `.get(key, DEFAULT)` returns `None` and the default is dead code. Measured before the fix on all 20 shipped diffusers cfgs: key present `True`, value `None`, `.get(key, DEFAULT)` → `None`. **The `or` rewrite is the fix** (`diffusers_cfg.get(k) or DEFAULT`), and it had to be applied at every site on that model, because the filing named one and there were three. **(a)** `pytorch_extra_index_url` — U33 as filed; `_PYTORCH_EXTRA_INDEX_URL` (cu124) was dead code and the cu128 pin the code's comment calls *"required by the FlashVSR x4 cfg"* could not have worked. Masked because PyPI's torch 2.6.0 wheels already bundle cu124. **(b)** `image`, on the same model and the same call shape, LATENT and NOT masked: a diffusers cfg that omits `engine.diffusers.image` got the literal string `'None'` as its container image. Proven in the RED run (`assert rp.image == 'None'`). It has never bitten only because every shipped cfg happens to set `image` explicitly — so the documented default has never once been reachable. **(c)** one level up, `engine_block.get("diffusers", {})`: `runpod-diffusers-serverless.yaml` comments its whole `diffusers:` block out, so the block itself dumps as present-and-`None` and `render_provision` died on a bare `AttributeError: 'NoneType' object has no attribute 'get'` — on the `kinoforge deploy` path, AFTER cfg validation passed. This one was found by the new class guard, not by reading, and `tools/snapshot_launch_payloads.py` had been carrying an `EXCLUDED_CONFIGS` entry describing it as an un-deployable config whose fix would be *"the config grows an `engine.diffusers` block"* — the wrong diagnosis of a real crash, now corrected in place. **Blast radius, as the filing predicted:** 20 launch-payload goldens + `tests/engines/diffusers/_golden_provision.json` moved. Every changed line in every golden — base64+gzip env payloads, Modal `image_build_script`, SkyPilot `task_config/setup` — was decoded and diffed: **26 changed script lines, all of them the `--extra-index-url` line, zero others.** Full suite 5613 → green, 21 golden failures before regeneration and none after. Tests: `tests/engines/test_diffusers_optional_cfg_defaults.py` (25), which goes through `load_config(...).model_dump()` rather than a hand-built dict — **that is the whole defect, and why the existing `test_render_provision_pip_install_includes_pytorch_extra_index_url` passed against broken code for months: its `_minimal_cfg()` omits the key, so `.get`'s default fires in the test and never in production.** *Generalise: a fixture that is not the production shape is not a test of the production path.* Fix + tests `af36000b` (RED scaffold `743b8f76`), follow-ups U34/U35 below |
| U34 | **CLOSED — ANSWERED LIVE 2026-09-11 by a paired A/B, ~$0.05.** The cu124 INDEX supplies the wheel, and it is byte-for-byte safe | **U33's fix turns on an install path that has never once run on a pod — on EVERY diffusers cfg, not just FlashVSR.** Filed narrowly ("the FlashVSR x4 cfg claims cu128 and renders cu124") and that framing was too small. Measured for $0.00 with `pip install --dry-run --report`, python 3.11, platforms `manylinux_2_28_x86_64` + `manylinux1_x86_64` + `linux_x86_64`: **PyPI-only resolves `torch 2.6.0`, `manylinux1_x86_64`, sha256 `7979...`; adding `--extra-index-url https://download.pytorch.org/whl/cu124` resolves `torch 2.6.0+cu124`, `linux_x86_64`, sha256 `d4c3...`.** Different version string, different wheel, different hash — because the pytorch indexes publish under LOCAL version tags (`2.6.0+cu124`, `2.8.0+cu128`) and `2.6.0+cu124` sorts ABOVE bare `2.6.0` under PEP 440, so pip prefers it. Since the engine rendered the literal `--extra-index-url None` on every build there has ever been, **no kinoforge pod has ever installed a `+cuXXX` torch wheel**; every green live run to date was on PyPI's default build. U33 restores the documented intent (`_PYTORCH_EXTRA_INDEX_URL` = cu124, matching the `runpod/pytorch:2.4...cuda12.4.1` image) and that is the right target — but it is an UNEXERCISED path, and `+cuXXX` wheels vendor CUDA differently from PyPI's, so the dependency closure is not the same one either (the probe used `--no-deps` and cannot speak to that). **Therefore: the next live diffusers run is the proof, and it is a blocker rather than polish.** Cheapest sufficient shape is upscale-only (~4 min / ~$0.08, `upscale_only: true`, fixture `output/20260630-221857_..._Photorealistic-cinem.mp4`): capture `pip list | grep -E 'torch|nvidia'` and `python -c 'import torch;print(torch.__version__, torch.version.cuda, torch.cuda.is_available())'` from the pod, then frame-QA the output per the visual-QA rule. **Fallback if it regresses:** make an unset key mean *no* extra index at all (behaviour-preserving) instead of defaulting to cu124 — that still fixes the literal-`None` render, the `image` default and the serverless crash, and leaves `_PYTORCH_EXTRA_INDEX_URL` opt-in. **Secondary, unchanged:** the x4 cfg pins a cu128 image and a `bsa-cu128-torch2.8-v1` wheel while installing `torch==2.6.0` and setting no `pytorch_extra_index_url`, so it now gets `2.6.0+cu124` — the comment's cu128 intent is expressed by no cfg. **LIVE RESULT 2026-09-11, and it is PARTIAL — do not read it as a closure.** One green run on RunPod pod `h0921u4lt7bmln` (RTX 4090 $0.74/hr, ~$0.015, 70 s wall, spandrel x2 upscale-only): the pod ran **`torch 2.6.0+cu124` and `torchvision 0.21.0+cu124`**, provision succeeded end to end, artifact published, frame-QA PASS (480x480 -> 960x960, scene and colour faithful to source, real detail gain in flowers/waterfall/hair, mild RealESRGAN plasticity in the face which is normal for the model; none of the entry-#13/#14 false-colour corruption). Teardown verified from a fresh process — `kinoforge list` both lines clean, zero pods provider-side. **So U33 causes no regression on this path.** **What the run does NOT establish:** that the cu124 INDEX is what supplied that wheel. pip logged torch as `Requirement already satisfied ... (2.6.0+cu124)` and the whole provision took 64 s — far too short to have pulled a 766 MB wheel — so the base image (`runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04`, whose tag says 2.4.0) may already ship `+cu124`, in which case U33 is a no-op on RunPod and the real exposure is Modal's different base plus any image bump. **The discriminator is committed and READY but did NOT run:** `tests/live/probes/u34-spandrel-no-pytorch-index.yaml` (`223532c9`) pins `pytorch_extra_index_url` to PyPI itself, making the flag a no-op and reproducing pre-U33 behaviour without touching engine code — still `+cu124` means the image supplies it, bare `2.6.0` means the index does. Nine live attempts failed to place it: seven `RateCapExceeded` at $2.39/hr and two transient RunPod create errors. That is U36, not a fault in the probe. **Next session: run the probe first, it is ~$0.015 and one command.** **A retracted intermediate claim, kept because the mistake is the lesson:** the first two resolves passed `--platform manylinux1_x86_64` alone, which silently excludes the pytorch index's `manylinux_2_28`/`linux_x86_64` wheels, and reported a byte-identical wheel — i.e. "the fix is inert". That was an artifact of the probe, not a property of pip. *A negative result from a filtered query is a statement about the filter.* | **RESOLUTION 2026-09-11 (second session), and it required measuring BOTH arms rather than one.** Two pods, one harness, one session, differing by exactly one rendered flag and holding accelerator ORDER identical so the arms could not diverge by landing on different hardware: **ARM A** (`tests/live/probes/u34-spandrel-no-pytorch-index.yaml`, pod `67k6ihp9w1usli`, `--extra-index-url https://pypi.org/simple`, i.e. pre-U33) -> **`torch 2.6.0`, `torchvision 0.21.0`, both BARE**; **ARM B** (`tests/live/probes/u34-spandrel-cu124-index.yaml`, pod `u7g1h7fu0j7scm`, `--extra-index-url https://download.pytorch.org/whl/cu124`, i.e. post-U33) -> **`torch 2.6.0+cu124`, `torchvision 0.21.0+cu124`**. **So the INDEX supplies `+cu124`; the base image ships BARE 2.6.0 despite its `2.4.0` tag.** U33 really does change the installed wheel on every RunPod diffusers cfg, exactly as this entry predicted — and the unexercised path is now exercised. **It is also safe, proven harder than 'no regression':** the two arms' output mp4s are **BYTE-IDENTICAL** (sha256 `d2943653f3a69fb7...`, 1294708 B both), and both pass frame QA at 960x960 (coherent, colour faithful, real detail gain, mild RealESRGAN plasticity; none of the entry-#13/#14 false-colour corruption). The **fallback named above (make an unset key mean no extra index) is therefore NOT needed** and should not be built. Evidence committed verbatim: `tests/live/_u34_torch_probe_evidence_67k6ihp9w1usli.txt` and `..._u7g1h7fu0j7scm.txt`; harness `tests/live/_u34_torch_probe.py`. **A correction to the PARTIAL reading above, and it is the interpretive lesson:** the 2026-09-11 line *'Requirement already satisfied ... (2.6.0+cu124)'* was read as evidence the IMAGE shipped `+cu124`. It is not. It comes from the **SECOND** pip invocation, by which point the FIRST had already installed the cu124 wheel. **Both arms show that identical 'already satisfied' shape at that point and disagree only in the version string** — so the shape proved nothing and the string proved everything. *When two hypotheses predict the same log SHAPE, the shape is not the evidence; find the field they disagree on.* **A second artifact of a filter, the same family as the `manylinux1` mistake already recorded on this row:** the pip line runs with `-q`, so `Collecting` / `Downloading` appear in NEITHER capture. Arm A's zero-download silence was briefly taken as proof nothing was installed; it was a property of `-q`. Provision wall-time is not evidence about wheels either — arm B pulled the 766 MB wheel inside a ~80 s provision on RunPod's network. **What is NOT closed by this** and is now filed as **U37**: the secondary cu128 mismatch below — the FlashVSR x4 cfg's cu128 intent is still expressed by no cfg, and it now demonstrably installs `+cu124` rather than the `cu128`/torch-2.8 pairing its comment claims.
| U35 | **FIXED 2026-09-14** — the exclusion list is now EMPTY | The snapshot harness's stub answered every mutation with the on-demand shape, so `mode: serverless` — which routes to `saveTemplate` — died on `RunPod create-serverless returned no endpoint id`, and the ONE serverless cfg shipped sat outside the ratchet. The stub now branches on the mutation and returns `{"data": {"saveTemplate": {"id": "endpoint-golden"}}}`; `EXCLUDED_CONFIGS` is empty and documented as the correct state. **The repo's own expiry mechanism drove the sequence**: `test_excluded_configs_exist_and_still_fail_to_capture` re-runs every excluded capture and demands it STILL fails, so fixing the stub turned that test RED first — the exclusion could not be left behind. Goldens 31 -> 32. **The capture's `seam` field now names the mutation it actually used**, because labelling a `saveTemplate` payload `podFindAndDeployOnDemand` would make the golden lie about which wire it froze — and that accurate label is what lets the content guards split by shape. **Two guards needed scoping, and scoping them was the risk**: `test_runpod_goldens_carry_a_real_create_pod_input` and `..._the_provision_script_on_the_wire` assert `cloudType` / `dockerArgs` / `ports` / `env`, none of which exist on `saveTemplate`. Loosening the POD contract until a serverless payload fit would have gutted the guard for the 31 configs that really are pods, so both are now scoped to pod goldens **with a `>= 5` floor so the scoping cannot silently empty them**, and the serverless shape got its OWN guard rather than a pass. That guard asserts derived values hand-computed from the cfg (`job_timeout: 20m` -> `executionTimeoutMs == 1_200_000`, `idle_timeout: 1h` -> `idleTimeout == 3600`), so a seconds-for-milliseconds slip fails where a field-is-present check would not; both falsifications (unit slip, payload collapsed to `{}`) were confirmed to fail it |
| U36 | **FIXED 2026-09-14** (`cd133238`), root-caused the same day for ~$0.01 on four pods | **Not a pricing mystery — a POOL-FILTER BUG.** `find_offers` sends `lowestPrice(input: { gpuCount: 1 })` with NO pool filter, and that query returns the **community** floor on every GPU present in both pools; `_create_pod` then books `cloudType: SECURE`. kinoforge filters, ranks and rate-caps on the price of a pool it never books into. **Measured on four real pods (`tests/live/probes/u36_pool_price_probe.py`, committed RED first at `2a8d37e4`): realized == advertised(pool) EXACTLY, 4 arms out of 4** — 4090/SECURE adv(secure) 0.740 -> realized **0.740**; 4090/COMMUNITY 0.340 -> **0.340**; L4/SECURE 0.490 -> **0.490**; MI300X/SECURE 2.390 -> **2.390**. Against the UNFILTERED number those same arms read 2.18x / 1.00x / 1.11x / 4.78x, which is the "5-10x under-prediction" this item was filed for. **RunPod's rate is perfectly deterministic**; there is no variance to model and nothing to converge on. Catalog-wide: on every dual-pool GPU the unfiltered price is byte-identical to the community price, and secure runs 1.5-2.3x higher. **All three proposed fixes are retired** — (a) widening the cap weakens a correct guard to paper over a wrong lookup, (b) the in-provider `RateCapExceeded` retry is a convergence loop around a question with an exact answer, (c) surfacing `advertised -> realized` dresses up the wrong number instead of asking for the right one. **FIXED in `cd133238`**: `find_offers` takes a `cloud_type` and `_create_with_offer_retry` passes the cfg's own, so the catalog is priced for the pool the create books. `cloud_type: any` deliberately keeps sending the unfiltered query — it books `cloudType: ALL` and may land in either pool, and pinning `secureCloud: false` there would drop every secure-only GPU (L4, A40, MI300X) from a catalog that can legitimately book them. Five tests; four drive `find_offers` through a transport that prices per pool as RunPod really does, and the FIFTH drives `create_instance` and asserts the wire — because the other four pass `cloud_type` by hand and would all stay green if the production call site never did. **Falsifying the call site made exactly that one test fail**, which is the U3 lesson applied rather than re-learned. Full suite green (5700 passed), no golden moved (this is a READ query). **Two corrections to this row's own filing.** (1) "realized $2.39/hr on the L4" was a MISATTRIBUTION: the L4 bills **$0.490** secure, measured, and $2.390 is the **AMD Instinct MI300X**'s secure rate, measured to the cent — the over-cap creates were almost certainly landing on an AMD card. (2) The eighth attempt's RTX 4090 at $0.74/hr, recorded here as "still 2.2x the advertised $0.34", is exactly RunPod's published **secure** 4090 price; nothing drifted and the pool never moved. **Why this survived:** the provider's own capability docstring states the false claim in terms — `RATE_DETERMINISTIC` is justified as "the pod is booked on the `gpuTypeId` kinoforge selected from RunPod's own catalog, so the price that survived the pre-book filter is the price billed." The capability is true and the justification is false, because filter and create read different pools. **Also retired: the 2026-09-12 "secure is cheaper" reading** ($0.27/hr secure vs $0.49-$0.59 community) does not reproduce — secure is uniformly the more expensive pool and no catalog GPU advertises $0.27 in either. One uncontrolled reading against four controlled arms; the arms win |
| U43 | **FIXED 2026-09-14** (`9c45675c`), filed the same day while root-causing U36 | **An accelerator name the provider's catalog does not carry is SILENTLY INERT.** `filter_offers` ranks by exact string equality (`o.gpu_type in placement.accelerators`, `core/offers.py:57-60`); a name that matches nothing returns `len(accelerators)` — the same rank as a GPU nobody asked for. No warning, no error, and the cfg reads as though its preference is being honoured. **Seven shipped cfgs name GPUs RunPod does not have**: `NVIDIA RTX 4090` (the catalog id is `NVIDIA GeForce RTX 4090`) in `grids/runpod-diffusers-wan-2_1-1_3b-base.yaml`, `grids/runpod-diffusers-wan-2_1-1_3b-base-no-loras.yaml`, `runpod-diffusers-wan-2_1-1_3b-t2v-lora-flexible-warm-reuse-smoke.yaml`, `runpod-diffusers-wan-2_1-1_3b-t2v-strength-grid.yaml` and `tests/live/probes/u24-swap-base.yaml`; `RTX 4090` + `A100 40GB` in `runpod-diffusers-serverless.yaml`; `A100 80GB` + `H100 80GB` in `runpod-comfyui-wan-2_2-14b-t2v.yaml`. **This is the whole of U36's "L4-first ordering" cause.** Proven through the real code path against the live catalog: the shipped 1.3B grid cfg's `find_offers` returns **L4 at rank 0** (it is the one name that does match) and the 4090 it meant to lead with at **rank 4**, behind an AMD MI300X. `7ba8c3c1`, which dodged the ordering on the U34 probe cfg, worked only because that cfg spells the id correctly — so the dodge was real but did not generalise, and nobody could see why. **FIXED in `9c45675c`.** The warning lives in `filter_offers`, not a `validation/` Check — the Check protocol is STATIC and has no network, while `filter_offers` already holds the provider's catalog, so SkyPilot and Modal are covered by the same code. All seven cfgs renamed to real catalog ids. **One golden moved, and it is the evidence**: the comfyui payload was asking RunPod to book `gpuTypeId: "A100 80GB"`, an id RunPod does not carry. **Two defects in the first cut of the warning, both found by RUNNING it against the live catalog rather than by reading it, and both of the same family as the bug it was fixing.** (1) It called `NVIDIA RTX A5000` a typo — a real id that simply had no secure stock that minute, because `find_offers` drops null-priced GPUs before `filter_offers` sees them. The warning was advising the operator to break a working cfg, which is the exact cry-wolf failure its own second test was written to prevent; the provider now passes its FULL id set, out-of-stock entries included. (2) It suggested `NVIDIA RTX A4000` for `NVIDIA RTX 4090` — `difflib` ranks the shorter candidate higher on raw character overlap, so the hint pointed at a different card of a different generation. Suggestions now prefer a token SUPERSET, which is what separates "the same card written loosely" from "a different card that looks similar". **A test was also deleted from the first cut**: `test_declaring_no_accelerators_does_not_warn` claimed to pin the early-return ordering, but falsifying that ordering left it green — an empty preference list makes the loop a no-op either way, so the bug it described could not happen. *A test whose stated bug is imaginary implies coverage that is not there* |
...
| U48 | **FIXED 2026-09-16** — the `Offer.cuda` audit U44 asked for, and SkyPilot's was worse than RunPod's | **`SkyPilotProvider` reported a CUDA version sky never told it, from a lookup that could not succeed.** `_offers_from_records` built every offer with `cuda=_record_field(info, "cuda", default="12.0")`, which READS like a catalog lookup. It is not one: `sky.catalog.common.InstanceTypeInfo` has no `cuda` field at all — verified against the pinned sky in `live-skypilot`, whose `_fields` are `cloud, instance_type, accelerator_name, accelerator_count, cpu_count, device_memory, memory, price, spot_price, region`. The default was therefore the ONLY value producible, on every offer, forever. **The harm was not cosmetic, and it is worse than U44's**: `Placement.min_cuda` defaults to `"12.8"`, so a cfg that simply never mentions CUDA had its **entire catalog filtered away** — an H100 rejected against a floor describing nothing about it. Demonstrated: default floor -> `[]`, floor 12.0 -> survivors. **All five shipped SkyPilot cfgs set `min_cuda` to `"0"`, `"11.0"` or `"12.0"`** — the config surface bent around a constant, and load-bearing for the two that name no `accelerators` (`skypilot-gpu.yaml`, `skypilot-lambda-comfyui.yaml`), since `_select_accelerator` returns `placement.accelerators[0]` before the catalog is consulted at all. **Fixed three ways**: `consumes()` declares `min_cuda` UNSUPPORTED (it claimed CONSUMED, which kept `UnsupportedFieldCheck` silent while the operator's floor did nothing but empty the catalog); the offer carries a named `_SKY_CUDA_UNKNOWN` sentinel instead of a fake read, so no future reader restores a lookup that cannot succeed; and a `_SUBSTITUTE` entry names what really pins the hardware (`accelerators`, which bypasses the floor entirely), making the finding a **WARN** — an ERROR row would have REFUSED the two shipped cfgs that set `min_cuda` perfectly reasonably. **The sentinel is `"9999"`, deliberately not `"99.0"`**: that is the exact value `test_field_consumption_parity::_above_every_cuda` uses to mean "no catalog entry can meet this", and a sentinel colliding with a guard's impossible-value fixture is a trap. **The parity guard forced the declaration and its evidence to stay in step** — dropping `min_cuda` to UNSUPPORTED made `test_no_proof_outlives_its_declaration[skypilot]` and `test_wire_proof_holds[skypilot-min_cuda]` fail until the obsolete wire proof was removed, which is exactly what that guard is for; the old proof only ever demonstrated that a fabricated constant could be filtered against itself. **Three tests**, one of them pinning the UPSTREAM fact (`"cuda" not in InstanceTypeInfo._fields`) so that if a future sky ever does publish CUDA, it fails and the sentinel gets replaced — it `importorskip`s sky and was verified to actually RUN under `pixi run -e live-skypilot`, not skip forever |
| — | AUDIT RESULT 2026-09-16 (no defect) | **Modal and Local were audited in the same pass and are NOT changed.** **Modal**: `_MODAL_CUDA = "12.8"` is an assertion, and its own comment admits it was chosen so the catalog survives the default floor — but the fleet is NVIDIA-only (T4, L4, A10, L40S, A100-40GB, A100-80GB, H100), so U44's non-CUDA-vendor bug cannot arise, and the claim (Modal runs recent drivers) is real, sourced and works at the default. Deliberately left alone: giving it SkyPilot's `UNSUPPORTED` treatment would REMOVE a filter that currently bounds a real risk. **Caveat recorded, not fixed**: the value never varies, so `min_cuda` is all-or-nothing on Modal too (`"12.9"` empties the catalog with no warning), and the 2026-07-08 snapshot would not notice a fleet that gained older hardware. **Local**: `cuda="12.8"` on two synthetic offers is a FIXTURE, not a claim — the provider is unbilled, simulates every instance in-process and launches nothing (`_local_inert` already makes every unsupported field a WARN). No action. **So the audit's score is 2 of 4 providers asserted a CUDA version they could not know, and both were load-bearing**: RunPod's offered an AMD card to a CUDA workload (U44), SkyPilot's emptied the catalog for any cfg that used the default (U48) |
| U49 | **ANSWERED 2026-09-17 — NO DEFECT, and it corrects a claim this ledger made without checking** | **RunPod's catalog really does carry a GPU type whose id is the literal string `unknown`, but it is neither priced nor reachable — and the note that said otherwise was an assertion, not a measurement.** The previous NEXT ACTION called it "a bookable GPU type ... priced and bookable". Both halves are false. Read verbatim 2026-09-17: `{"id": "unknown", "displayName": "unknown", "memoryInGb": 0, "secureCloud": false, "communityCloud": false, "manufacturer": "unknown", "cudaCores": 0, "lowestPrice": {"minimumBidPrice": null, "uninterruptablePrice": null}}`. **Where the wrong claim came from is the lesson**: the probe that first surfaced the id listed only ids (it was looking for non-NVIDIA vendor prefixes for U44), and the separate probe that listed PRICES filtered to `memoryInGb >= 24`, which excludes this row at 0. So the id was seen in one place and the price in another, and "priced" was filled in by assumption across the gap. *Two probes that never overlapped are not corroboration.* **Three independent things already exclude it, verified empirically rather than reasoned:** (1) `find_offers`' null-price guard drops it before an `Offer` is ever constructed — the guard's own comment says why ("otherwise we would happily return them as $0 offers"); (2) `memoryInGb: 0` means any `min_vram_gb > 0` would drop it anyway, and **no shipped RunPod cfg sets 0**; (3) `secureCloud: false` AND `communityCloud: false` — it is in NEITHER pool, and a row in no pool cannot be booked by definition, which every real GPU contradicts by being in at least one. Proven by running the real `find_offers` against the live catalog with a deliberately wide-open placement (`min_vram_gb=0, min_cuda="0", max_usd_per_hr=100`): **31 offers returned, `unknown` absent**. **No code change, deliberately.** A pool-flag filter (`skip rows in neither pool`) would be defensible and cannot false-positive the way U43's name match or U44's vendor prefix could — but it guards a case two existing filters already cover and that has never been observed, and it would be the third new guard in three days on a codebase that has just recorded four over-blocking near-misses. The pool flags are recorded HERE as the reason it is safe, so the next reader does not have to re-derive it. **What would change this:** `unknown` appearing with a non-null price, which the A5000 stock-out taught is not unimaginable — a catalog read is a snapshot. Even then bound (2) holds for every shipped cfg |
...
| U40 | FIXED 2026-09-12 (`07bb70cd`, $0.00 offline; found live) — **and proven INSUFFICIENT the same day** | **The boot-liveness probe killed a HEALTHY pod mid model-download.** Pod `rohjrsmre9obsp`: aborted with `boot stalled (provision crashed or util flatline)` while its bootstrap log showed live `huggingface.co` requests, `Fetching 19 files`, and 9233 B of growth between two fetches 30 s apart. Structural on RunPod: `_is_flat` is CPU-0 AND mem-flat AND disk-flat-or-unknown, an HF fetch is CPU-idle (network-bound) and mem-flat (streams to disk), and `providers/runpod/util.py` documents that `disk_percent` is ALWAYS None there — so the predicate degenerates to 'CPU 0 and memory flat', which a healthy download satisfies exactly. `classify_boot_liveness` already RECEIVED the log tail and read it only for a failed trap rc; it now also treats a CHANGED tail as progress, placed BELOW the trap-rc check because a crashing provision grows its log at the moment it dies (the trap appends `rc=<N>`). Five tests, both failure modes falsified. **⚠️ It did not work in production, and that is the lesson:** the next live run died identically, because the port-8001 sidecar is NOT proxy-reachable during early boot — two fetches during the flatline returned 0 bytes — i.e. the channel the fix depends on does not exist in the window the fix is for. *Correct logic, unreachable.* Superseded in effect by **U41** |
...
| U53 | FILED 2026-09-22, OPEN — pre-existing, found and ratcheted (not fixed) by Task 9 of the MiniMax-H3 LoRA shared seam build | **Every RunPod diffusers config that embeds the FlashVSR/RIFE/spandrel bootstraps is already over the ~101 KB point where `podFindAndDeployOnDemand` returns a raw HTTP 500 with no GraphQL error body** (CLAUDE.md "Known infra gotchas"). The currently-committed, currently-enforced measurement (`tests/providers/test_env_payload_ceiling.py`, full RunPod wire shape — key+value summed over every `input.env` entry, not just the provision-script blob) puts the two largest tied configs (`runpod-diffusers-wan-2_2-14b-t2v-flashvsr-1080p-upscale`, `runpod-diffusers-wan-2_2-14b-t2v-flashvsr-upscale`) at **127,917 B**; 8 of the 13 shipped RunPod diffusers configs are over the ceiling. **Pre-existing, not caused by this branch.** An earlier, narrower measurement (script bytes alone, not the full env total) reported the largest config at 123,684 B post-branch vs. 120,036 B pre-branch HEAD — a ~3.6 KB delta attributable to this branch — but that pre-branch figure was never reproduced under the fuller method the committed ratchet now uses (a `git worktree` re-measurement attempt failed on environment setup, not on the number; see `.superpowers/sdd/2026-09-22-h3-lora-shared-seam/task-9-report.md`), so treat "this branch added ~3.7 KB" as approximate. **Root cause of the vanished headroom:** `_create_pod` gzips the WHOLE provision script (`providers/runpod/__init__.py:1323`), but each embedded server module is ALREADY gzip+base64'd individually first (`engines/diffusers/__init__.py:127,198`) — gzip-of-base64-of-gzip buys almost nothing, because base64 text is already close to incompressible. **CLAUDE.md's "~74 KB script → ~72 KB base64, ~4× headroom" note (Known infra gotchas) is therefore rotted and still needs correcting** — real headroom is negative for 8 of 13 shipped configs, not 4×. The identical claim is duplicated as a source comment at `providers/runpod/__init__.py:1317-1322` and is equally stale. `tests/providers/test_env_payload_ceiling.py` is a RATCHET (stops the breach from silently growing worse), not a fix; shrinking what gets embedded is its own project needing its own live proof, explicitly out of scope for the task that added the ratchet. Fix direction: drop the outer gzip (near-zero payoff today) and spend the reclaimed margin, or actually shrink what each config embeds |
| U54 | FILED 2026-09-22, OPEN — pre-existing, found by review during the MiniMax-H3 LoRA shared seam build | **A persisted ledger `lora_inventory` row can be silently corrupted into redaction placeholders before any cross-process reader — including the warm-reuse matcher — ever sees it.** `Ledger._write_entries` (`core/lifecycle.py:556`; the filing that first flagged this named the method `_persist`, which does not exist — the real name is `_write_entries`, corrected here) runs `RedactionRegistry.instance().redact_json()` over the WHOLE ledger payload before every disk write. Both `core/lora_apply.py`'s `_record_inventory` and the pre-existing `warm_reuse/integration.py:126-132` swap path call `_register_observed_lora_refs(...)` on the pod's response IMMEDIATELY BEFORE `ledger.touch(..., lora_inventory=inventory_dicts, ...)` — deliberately, per `_record_inventory`'s own docstring, so the refs are redactable in any LOG line that might later name them. Side effect: those exact ref strings are now registered tokens, so the SAME `_write_entries` call that persists `lora_inventory` also substitutes each ref with a `<lora:ref:…>` placeholder in the PERSISTED DATA ITSELF, not just in logs. **Not vault-gated** — `_register_observed_lora_refs` (`core/warm_reuse/redaction.py`) registers refs unconditionally, so this fires on any run that has LoRAs and a ledger, vault or not; a vault naming LoRA refs (`core/vault.py:236`) widens it further. `warm_reuse/matcher.py:307` (`inventory_refs = [_entry_field(e, "ref") for e in inventory_entries]`) compares those persisted refs against raw cfg refs from `resolve_active_lora_stack`; `Ledger._read_entries_from_disk` performs no un-redaction (redaction is one-way), so a FRESH PROCESS reading a row a PRIOR process wrote sees only placeholders and every ref mismatches — the matcher would plan to re-download everything already present and evict everything it should keep. Equally true of the existing swap path (`warm_reuse/integration.py`), not introduced by this branch. Not exercised by any test: `tests/core/test_warm_reuse_redaction.py` covers only the registration helper in isolation, never a `_write_entries` round trip through an active registry. Fix direction: exempt the ledger's `lora_inventory.*.ref` path from `redact_json`'s deep-walk, or have the matcher compare against a registry-aware canonical form instead of raw strings |
...
**Status update 2026-09-09 (`fix/ephemeral-and-recovery-gaps` — U11 regression fix; OFFLINE, $0.00,
no provider or network call).** Commit `b9b4fcd6`. Task 6's A2 above stands as the record of the
live failure; this closes it. `_build_generate_cmd` now emits `--ephemeral` in ROOT position
(`pixi run kinoforge --ephemeral generate …`) instead of appending it after the `generate`
subcommand, because `--ephemeral` is a session-global flag consumed by `main()` before dispatch
and the root parser is its only correct home. The audit that fix required turned up a SECOND,
unrecorded instance of U11's leak: `p_grid` re-declared `--ephemeral` with an implicit
`default=False`, and argparse copies every key of a subparser's fresh namespace onto the parent,
so `kinoforge --ephemeral grid …` parsed to `ephemeral=False` and ran non-ephemeral while exiting
0 — fixed with `default=argparse.SUPPRESS` in the same commit. The test gap that shipped the
regression is closed: the new tests feed the argv the executor actually builds through the real
...
- **U11 — FIXED, LIVE-PROVEN 2026-09-09 (`b9b4fcd6`, after a same-day live-caught regression fixed
  ...
  *Fix shape chosen:* emit `--ephemeral` in ROOT position (`pixi run kinoforge --ephemeral
  ...
  is a session-global flag read by `main()` before dispatch (`with EphemeralSession(enabled=
  args.ephemeral, …)` wraps every `_DISPATCH` call), so the root parser is its only correct home;
  per-subparser copies of a root flag are precisely what causes the second defect below.
  *Second defect, found by the audit and fixed in the same commit:* `p_grid` was the ONLY
  subparser re-declaring `--ephemeral`, with an implicit `default=False`. argparse parses a
  subcommand into a FRESH namespace and then copies every key of it onto the parent, so a
  root-set `True` was overwritten:
  ```
  $ pixi run python -c "from kinoforge.cli._main import _build_parser; p=_build_parser(); \
      print(p.parse_args(['--ephemeral','grid','--spec','x']).ephemeral)"
  False
  ```
  That is U11's original leak (run id + local timestamp published to the provider) reached
  through the other door, and it was silent — the command exits 0. Fixed with
  `default=argparse.SUPPRESS`, so the key is absent unless the flag is actually given after
  `grid` and either position now yields `True`.
  *Test gap closed (the point of the exercise).* Three new tests take the argv
  `_build_generate_cmd` / `_build_swap_generate_cmd` actually build, strip the
  `pixi run kinoforge` launcher prefix, and feed the remainder through the real
  `_build_parser().parse_args()` — which errors on any leftover token — asserting `args.cmd`,
  `args.ephemeral` and (swap) `args.attach_pod`. A fourth pins `--ephemeral` on both sides of
  `grid`. RED was confirmed against the pre-fix code for the exact live reason:
  ```
  $ pixi run pytest "tests/core/test_grid_executor.py::\
      test_build_generate_cmd_argv_parses_under_the_real_cli_parser[True]" -q
  status = 2, message = 'kinoforge: error: unrecognized arguments: --ephemeral\n'
  E       SystemExit: 2
  ```
  GREEN after the fix; `pixi run pytest tests/core tests/cli -q` → **2256 passed**. The round-2
  `lora_swap`-under-`--ephemeral` refusal (clean stderr + exit 2) is unchanged and still covered.
  *Audit of the same defect class.* The only other argvs kinoforge composes against itself are
...
- **U13 — NOT REPRODUCIBLE at HEAD (hunted live 2026-09-11, ~$0.4; nothing fixed) — the CLI hung
  forever after `UpscaleFailed` instead of exiting.**
  ...
  **That first step was RUN on 2026-09-10 and did NOT reproduce the hang** — see the four
  mechanisms eliminated in this item's STATUS INDEX row. The hunt then moved live.
  **LIVE RESULT 2026-09-11: the hang did not happen, on either engine.** Two runs, both reaching
  a GENUINE server-side failure on Modal, both exiting cleanly (`returncode 1`) so fast that the
  watcher's 2-second poll never reached its 45 s grace clock:
  - **Tier 1**, a cheap T4 spandrel pod written for this (`tests/live/_u13_spandrel_modal_cfg.yaml`):
    `UpscaleFailed: … Could not load meta information`.
  - **Tier 2**, the SHIPPED FlashVSR A100 cfg — the exact T2-01 / T2-05b family:
    `UpscaleFailed: … \u0060pyav\u0060 can not handle the given uri.` Tier 2 was run *because* a clean
    exit on a cheap engine is not a clean exit on the engine where the hang was actually seen.
  **How the evidence was protected, since that is what failed last time.** The child lets the
  exception ESCAPE `main()` (catching it would destroy the one asymmetry that might make U13
  failure-path-only), and a hang would have been dumped by SIGABRT into `faulthandler` — a
  mechanism verified offline BEFORE any spend, because a `kill -9` with no stack is exactly what
  lost the 2026-09-06 observation. The probe also had to be fixed mid-hunt: its first version
  `rmtree`'d the child's logs, so the first build failure destroyed its own diagnosis.
  **⚠️ The one dimension NOT reproduced.** Both induced failures land at DECODE, before any
  inference. T2-01 failed at the mp4 ENCODE after a full multi-minute run, and that path cannot be
  reached deliberately any more — the `av<18` pin (U12) fixed the writer. A holder created only
  during a long inference/poll cycle would therefore not have been caught here.
  **U31 is not the explanation**, settled without spending: `upscale` submits nothing to the pool
  (`UpscaleStage` calls `engine.upscale` directly, and `GenerateClipStage` — the only
  `pool.submit` caller — is skipped), and the 2026-09-10 offline run exited in 0.8 s while the U31
  defect was still live.
  **Nothing was fixed. This is a negative result, not a closure.** What it buys the next session
  is a committed harness: if the hang is seen again, run
  `pixi run -e live-modal python tests/live/_u13_hang_probe.py [cfg]` and the dump names the
  holder in one run. Found along the way and fixed: **U32**.
...
- **U17 — FIXED, OFFLINE-PROVEN in `8069f376`+`45f4254c` (2026-09-08; live re-proof still owed,
  ...
  **ROUND-2 FIX — `45f4254c` (Task 4, 2026-09-08).** Review of `8069f376` found the lookup had
  ...
  Three fixes, each RED-first: (a) `_find_app_id` now catches around `self._lister()` and returns
  `None`, so an absent `modal` binary / unauthenticated CLI / bad JSON degrades to the pre-U17
  by-name stop instead of aborting before any stop attempt with a raw traceback; (b) the record
  scan skips a malformed entry and continues, so a bad record ordered BEFORE the match no longer
  strands a resolvable, billing app — it still raises a `TeardownError` naming the expected/actual
  shape when nothing matched AND the shape was bad, which is what the acceptance criterion asks
  for; (c) among records sharing a `description`, one whose `state` is not `stopped`/`stopping...`
  wins, because `modal app list` deliberately keeps recently-stopped apps
  (`modal/cli/app.py:104`) and `modal app stop` on an already-stopped one exits non-zero
  (`modal/cli/app.py:513-520`) — first-match-wins could therefore stop nothing and leave the live
  app billing, where the pre-fix by-name path would have hit it. The post-stop poll loop was
  hardened the same way (a listing failure or a non-dict record there must not become a teardown
  traceback after a stop that already succeeded), and a `TeardownError` raised by the stopper is
  now propagated rather than re-wrapped into a nested "failed to stop ...: failed to ..."
  message. Test-side: the provider-error test's injected failure is now id-free (`"boom"`), so
  only the wrapper can put the app id in the message — the round-1 version passed a wrapper that
  dropped the id, because the injected message carried it; and `default_stop`'s own
  `CalledProcessError` wrapper, which shipped untested, now has two tests (the wrapped message
  names the identifier and the exit code; the argv is `modal app stop <identifier> --yes` with the
  raw `ap-...` id passed through verbatim). Fixture corrections: `tasks` is a string in the real
  JSON (`modal/cli/app.py:128` emits `str(...)`), and every synthetic app_id is now the 22 body
  characters `^ap-[a-zA-Z0-9]{22}$` requires. **Proof, still offline only**:
  `pixi run pytest tests/providers/test_modal_destroy_by_app_id.py -v` → 10/10 passed;
  `pixi run pytest tests/providers -q` → 599 passed, 2 skipped, 6 xfailed, including the two
  pre-existing `tests/providers/modal/test_provider.py` destroy tests unmodified. The live
  re-proof caveat above is unchanged.

  **ROUND-3 FIX — `887db3e1` (2026-09-09, whole-branch review Finding 2 follow-up).** `9e3cc1cc`
  correctly fixed the confirmation poll's false-positive on a stuck `"initializing..."` app, but
  built `states_by_name` as a plain dict comprehension keyed by app name — with two listing
  records sharing a name (a stop racing a redeploy, the same duplicate class `_find_app_id`
  already defends against), whichever record iterated last silently won the key, so a stopped
  duplicate landing after a live one could make the poll report "gone" while the live app kept
  billing. Fixed by building `states_by_name` from the same `live_first` ordering `_find_app_id`
  uses, then `setdefault` so the first (non-stopped, if any) record per name wins regardless of
  listing order. RED: a new test parametrized over both orderings failed for `live_first` only (0
  sleeps instead of 40) — confirming a single ordering would have hidden the bug. Proof, still
  offline only: `pixi run pytest tests/providers/modal/test_provider.py
  tests/providers/test_modal_destroy_by_app_id.py -q` → 24 passed.
...
- **U19 — FIXED + LIVE-PROVEN 2026-09-11 (`96d6920e`, $0.036) — the in-pod capability vocabulary
  had no term for interpolation, so `kinoforge interpolate` could not participate in warm reuse
  at all.**
  ...
  **Fix (2026-09-11), both halves in one commit because either alone is a defect.** The server now
  maps the `rife-` prefix to `"interpolate"`, and `_cfg_want_stages` returns
  `capability_key().stages` unfiltered (`_HEALTH_UNGATEABLE_STAGES` deleted). Dropping the filter
  alone is the refuse-always bug this entry was filed to prevent; teaching the prefix alone leaves
  the gate off. The repo already had the guard for the first of those —
  `test_no_shipped_cfg_demands_a_stage_no_pod_can_advertise`, whose `ADVERTISABLE_STAGES` is
  hand-transcribed from the server's return values precisely so the two sides cannot silently
  agree on a new term.
  **21 goldens moved, and the regeneration was reviewed rather than accepted.** The server module
  is embedded in the provision script, so every launch payload carrying it changed. Decoding each
  changed payload (base64 → gzip) shows the SAME 9 added lines and nothing else, and the
  surrounding script is byte-identical in all 20 launch payloads and in `_golden_provision.json`.
  **The live proof, and the trap it had to avoid.** The obvious proof — "run interpolate twice and
  show the second one attaches" — proves **nothing**, because under the old carve-out the
  `/health` gate was skipped entirely and the attach would have happened either way. That is the
  U3 shape (a test that passes whether or not the wire is connected). The evidence is the pod's
  own answer. Modal pod `interpolate-20260911-000240`, cfg-default 60 fps deliberately NOT
  overridden with `--fps` (fps is part of the capability key — T2-07 measured `--fps 32` and
  `--fps 60` hashing differently — so a differing one would have made the two runs unmatchable by
  construction):
  - `GET /health` on the live pod → `{"models":[{"name":"rife-rife426","on_device":"cuda",
    "ready":true}],"capabilities":["interpolate","upload"]}`. **That is the term this entry
    recorded as one no pod could ever advertise.** Pre-fix, the same pod answered
    `capabilities: ["upload"]`.
  - Run 1 cold-created the pod (`✓ App deployed in 161.523s` — a full image re-bake, expected,
    since the embedded server module changed). Run 2, from a FRESH process, logged
    `warm-reuse: attached to interpolate-20260911-000240` and produced its output in **11 s**,
    with `✓ App deployed` and `✓ Created objects` both at count **0**.
  - `GET /util` at the same moment: `gpu_util_percent=0.0, cpu_percent=1.1, memory_percent=1.2,
    uptime_seconds=195` — idle post-job, consistent with both jobs having completed.
  **Frame-QA PASS on both outputs**, 5-frame contact sheets read against a source sheet:
  480×480 / 60 fps / 304 frames / 5.067 s each, matching §25 exactly and the source's 5.0625 s to
  4 ms; colour faithful with no cast; subject pose and butterfly positions tracking the source
  frame-for-frame at every sampled timestamp; no ghosting or halo around the moving subject
  (RIFE's classic failure) and no warping in the waterfall spray. The two runs' sheets are
  indistinguishable from each other — the point being that the warm-attached run is not a degraded
  one. ⚠️ Stated limit: five sampled frames prove fidelity and the absence of artifacts at those
  points, **not** inter-frame smoothness.
  **Teardown verified from fresh processes** after the orchestrator exited: `kinoforge list` both
  lines, zero non-stopped Modal apps, preflight PASS. Spend **$0.036**. Logged as a See-also under
  `successful-generations.md` §25 — same `(modal, DiffusersEngine, rife-rife426, interpolate)`
  tuple, so no new section; what is new is that the interpolate path participates in warm reuse at
  all.
...
- **U24 — FIXED 2026-09-11 (`ee2dc937`, $0.00 offline; LIVE PROOF OWED) — `grid --ephemeral`
  refused `lora_swap:` cells instead of covering them.**
  ...
  **Fix (2026-09-11) — and BOTH reasons this entry gives for the refusal are wrong.**
  1. *"no single cell could own the shared pod's `delete_on_completion`"* — `delete_on_completion`
     **does not govern pod teardown at all.** Its only three consumers are hosted-job deletion
     (`core/remote_backend.py`), the store-run scrub (`EphemeralSession.__exit__`) and the profile
     alias (`core/provision_state.py`). No pod is destroyed by it.
  2. *"the pod is created by cell-1's process but torn down by whichever process runs last"* —
     **no cell tears it down.** `_run_swap_group` destroys the shared pod in its OWN `finally`
     (`pixi run kinoforge destroy --id`), on every exit path, and always has. The controller owns
     teardown, which is exactly the ownership the refusal said was missing.
  **What actually blocked the combination is a third thing this entry never mentions:** cells
  2..N resolve `--attach-pod` through `Ledger.read`, and under the strict policy the ledger is a
  **per-process in-memory mirror** — `ledger_record=False` diverts `Ledger.record` to
  `session.in_memory_ledger` (`core/lifecycle.py:513,553`) — so cell 1's pod is invisible to its
  siblings and cell 2 would have exited 1 with "not in ledger" about a pod that is alive.
  `_resolve_attach_pod` now falls back to the **ephemeral index**: the cross-process handoff this
  entry says "would need" to be built already existed, and `_scan_warm_candidates` and
  `kinoforge reap` (U18) both union it in. This was the third read site and the only one still
  missing it. Index-row cleanup needed nothing new either — the controller's `kinoforge destroy
  --id` runs `destroy_confirmed(..., ephemeral_index=...)`, which removes the row on a confirmed
  destroy, so no dead row survives for the next run's matcher to pick (the U28 hazard).
  **Six tests.** Three on the attach gate: the index fallback (RED first); the WAK check still
  applying to an index-sourced row, **falsified by deleting the check** so it is a guard rather
  than a hope; and an unknown id still refusing. Two on the executor, replacing the refusal tests:
  every swap cell's argv carries `--ephemeral` in **ROOT** position (appending it after the
  subcommand is what made Task 6's live cell A2 exit 2 with `unrecognized arguments`), and a plain
  grid's cells get no flag at all. The sixth is the stub work that made any of it possible — the
  harness had to learn to write a provision record and to answer the controller's `destroy`,
  **without which no swap chain could run in tests at all**, which is why the refusal's own test
  never exercised a second cell and why the premise went unchecked for three days.
  **⚠️ LIVE PROOF OWED — and a near-miss worth recording.** `kinoforge --ephemeral grid --spec
  <swap spec> --dry-run` now exits 0, and that is **not evidence**: `_cmd_grid` returns on
  `args.dry_run` BEFORE `run_grid` is ever called, so it never reached the refusal and would have
  printed the same line before the fix. It is written down because it reads like a demonstration
  and is not one. The real proof needs a live swap group: cell 1 cold-booting under an opaque
  `kinoforge-eph-<8hex>` name, cell 2 attaching from a separate process with zero `✓ App deployed`
  lines, the controller's destroy firing, and the ephemeral index empty afterwards. Same standing
  as U17 and U21 — offline-proven, debt recorded rather than glossed.

- **U25 — FIXED 2026-09-10 (`602c3f7a`, $0.00 offline) — an ephemeral `grid` still wrote
  local artifacts under the strict policy.**
  ...
  **Fix (2026-09-10).** The working tree could not simply be SKIPPED — ffmpeg composes from those
  per-cell mp4s — so it MOVES: under a strict policy the per-cell working root is a private
  `tempfile.mkdtemp` instead of `output/_grid_<id>`, and the whole `run_grid` body is wrapped in
  try/finally so it is removed on every exit. `run_grid` has FOUR returns (teardown / partial /
  ffmpeg / full), and cleanup on the composed-successfully one alone would have left everything
  behind on exactly the runs that went wrong. The stderr dump needed no separate gate once the
  tree moved: it is written next to the per-cell cfg, so it travels with it.
  **What deliberately STAYS, stated rather than left to inference:** the composed mp4, and — for a
  run that could not compose — `output/_grid_<id>_partial/` with the clips it did make.
  `--ephemeral` has never meant skipping the artifact (`generate --ephemeral` still publishes its
  clip); deleting those would destroy the run's OUTPUT, not its traces. The `_grid_<id>_partial`
  path does carry the local timestamp, and that is the accepted residue of keeping a failed run's
  deliverable.
  **How the policy is consulted, and why no field was added.** `_retain_local_intermediates` reads
  the active `EphemeralSession`'s policy, falling back to the `ephemeral=` kwarg only when no
  session is bound — from the CLI one always is, strict or default, so the policy governs in
  practice and the kwarg covers direct library callers. `policy.ledger_record` stands in for "is
  this session strict?", which is the SAME proxy `matcher.py`'s `always_reprobe` already uses.
  Grid intermediates are not one of `EphemeralPolicy`'s five named write gates; adding a sixth
  would have moved the spec-§8.1 field-count test in `tests/core/test_ephemeral.py`, and mapping
  them onto an unrelated existing gate would read worse than reading strictness directly.
  **Five tests, and the honest part is how the last two were handled.** Three were RED first: the
  intermediates gone while the composed mp4 stays; no `.stderr.txt` anywhere under `output/`; and
  the policy governing when only the SESSION — not the kwarg — says ephemeral, which is this
  entry's literal complaint and the thing a minimal `if ephemeral:` fix would have left standing.
  The other two guard bugs the fix itself could introduce and were **passing vacuously** before
  the scratch dir existed, so each was **falsified by deliberately breaking the implementation**
  and re-running: cleanup attached to the success return alone → RED on the failure-path test;
  cleanup ordered before `_move_to_partial_dir` harvests from the tree → RED on the
  partial-deliverable test. *A guard that has never failed is not yet a guard.*
  **This entry's own reproducer probe now answers `True`.**
...
- **U26 — `sweep()`'s ephemeral union does not honour a single-id-scoped ledger view.**
  ...
  **FIXED 2026-09-09 (whole-branch review Finding 1, `a8cbb54c`).** `sweep()` gained an optional
  `single_id: str | None = None` kwarg; when set it filters `EphemeralIndex(store=store).rows()`
  to the one row matching `row.id == single_id` before the union loop runs. `_cmd_reap` now calls
  `sweep(..., single_id=single_id)`. Every other call site (`SweeperLoop._tick_once`, every live
  and offline sweeper/reap test) passes no `single_id`, so the daemon's "process every row every
  tick" contract is untouched — confirmed by re-running the full sweep/sweeper battery (108 tests
  across `tests/cli/test_cmd_reap.py`, `tests/core/test_sweep_ephemeral_union.py`,
  `tests/core/test_reaper_sweep.py`, `tests/core/test_sweeper.py`,
  `tests/core/test_sweeper_loop_stall_history.py`, four `tests/integration/test_sweeper_*.py`
  files, `tests/core/test_reaper_orphans.py`, `tests/core/test_reaper_actor.py`) green with zero
  edits to any of them.
  **Reproducer re-run against the fixed code (offline, $0)** — same script as above, with
  `single_id='some-other-id-not-in-ledger-or-index'` now threaded into the `sweep()` call:
  ```
  scoped ledger.entries(): []
  sweep() snapshot keys (post-fix, single_id scoped): []
  ```
  Empty, where the pre-fix run above showed `['unrelated-pod']`.
  **New CLI-level test** (`tests/cli/test_cmd_reap.py::test_reap_id_scopes_sweep_apply_to_the_named_pod_only`)
  exercises the sharper, live-pod-safety-relevant case the reproducer above only gestures at:
  `--id target-pod --apply --include-orphans` with BOTH `target-pod` and an unrelated
  orphan-eligible `unrelated-pod` row in the index (both 600 s old under a 60 s
  `ephemeral_orphan_age_s` gate). RED against pre-fix code: `provider.destroyed == ['target-pod',
...
- **U27 — FIXED (offline-proven, CLI-demonstrated at $0.00) 2026-09-09 —
  ...
  **Reproducer (offline, $0.00):**
  ```
  $ pixi run kinoforge batch --ephemeral \
      -c examples/configs/modal-diffusers-wan-2_1-1_3b-t2v.yaml \
      --manifest /dev/null
  ```
  **Actual output (run 2026-09-09):**
  ```
  usage: kinoforge [-h] [--state-dir DIR] [--env-file PATH] [--vault PATH]
                   [--ephemeral] [--debug-show-secrets] [--version]
                   SUBCOMMAND ...
  kinoforge: error: unrecognized arguments: --ephemeral
  ```
  exit 2. The working form is `kinoforge --ephemeral batch …` (that is the form Task 6's A1 used
  and live-proved).
  **Pre-existing, NOT introduced by `fix/ephemeral-and-recovery-gaps`.** `--ephemeral` has only
  ever been declared on the root parser (`src/kinoforge/cli/_main.py:381`) plus `p_grid`; nothing
  on this branch touched `p_batch` (`:830-880`), and `git show` of the branch's commits confirms
  no parser change to it.
  **Suspected site / why it is filed rather than fixed.** The contained fix is one argument on
  `p_batch` (`--ephemeral`, `action="store_true"`, `default=argparse.SUPPRESS` — SUPPRESS is
  mandatory, or it would clobber the root value exactly as `p_grid` did, see U11). But the real
  question is a design one that should be decided once and applied uniformly: **should
  `--ephemeral` be accepted after EVERY subcommand** (`generate`, `upscale`, `interpolate`,
  `deploy`, `provision`, `batch`, `grid`) or should `p_grid`'s copy be removed so the flag is
  strictly root-only? Either answer is defensible; picking one for `batch` alone in a regression
  fix would deepen the inconsistency that caused U11. Left out of Task 2's regression fix
  deliberately.
...
- **U28 — FIXED and LIVE-PROVEN 2026-09-09 — the `--ephemeral` launch row never received its
  ...
  **Symptom.** During a live `--ephemeral batch`, the `EphemeralIndex` row that names the pod
  ...
  `row.endpoints` is non-empty (`src/kinoforge/core/reaper_actor.py:409-411`), so
  `_ephemeral_orphan_predicate` has no utilisation reading and, being conservative-on-ignorance,
  returns `LIVE`. Live consequence (Task 6, B1): against a genuinely idle pod at age 218 s with a
  direct `/util` read of `gpu_util_percent=0.0, cpu_percent=2.0`, and a cfg with
  `ephemeral_orphan_reap_enabled: true` / `ephemeral_orphan_age_s: 60`,
  `reap --apply --include-orphans` still printed `LIVE` and
  `acted on 0: 0 destroyed · 0 forgotten · 0 drift-skipped · 0 deferred · 0 failed`.
  Survivable on Modal, where the reserved name IS the app id so `destroy --id` works off the row
  alone; **larger on RunPod, where it is not — that is U16's blast radius.**
  **Reproducer (offline, $0.00) — the ordering is visible in the source:**
  ```
  $ grep -n "result = batch_generate(\|_settle_batch_launch_row(ctx" src/kinoforge/cli/_commands.py
  ```
  **Actual output (run 2026-09-09):**
  ```
  1453:        result = batch_generate(
  1486:    _settle_batch_launch_row(ctx, cfg, launch, single, ledger_ids_before)
  ```
  The settle is line 1486, unconditionally after the call at 1453 returns; the `except` clauses in
  between deliberately do NOT settle (ruling C1, so the classifier can age the row out). A SIGKILL
  ...
  **Assessment (2026-09-09, SUPERSEDED) — this is a RESTRUCTURE, not a contained change.** The endpoints do not exist in
  ...
  be upgraded mid-flight — plus its interaction with the reaper's classification of a
  half-upgraded row. And it is not a batch-only change: the identical reserve-then-settle shape
  exists at all four call sites —
  ```
  $ grep -n "_ephemeral_launch_row_reserve(ctx" src/kinoforge/cli/_commands.py
  966:  (generate)   1127:  (upscale)   1242:  (interpolate)   1441:  (batch)
  ```
  — so fixing only `_cmd_batch` would leave three commands with the same hole and a fifth code
  path shape to maintain. Filed rather than forced, per the project's "a correct diagnosis with a
  filed follow-up is a good outcome" rule.
  **Suspected site:** `_cmd_batch` (`src/kinoforge/cli/_commands.py:1440-1486`) and
  `_settle_batch_launch_row` (`:2300`), but the actual fix belongs at the orchestrator seam
  (`core/orchestrator`'s `deploy_session`) that first knows the endpoints. Relates to **U16**
  (RunPod: the reserved name is only a partial handle) and **U3** (a Modal pod's endpoint URL is
  unreachable from a fresh process — the same emptiness forced Task 6's util monitor to scrape the
...
- **U29 — CLOSED 2026-09-09 (`424e52d1`, offline-proven) — `p_batch` re-declared the ROOT
  ...
  **Symptom.** Same argparse namespace-clobber mechanism as the `p_grid` half of U11: argparse
  ...
  the root value the operator passed. `main()` then does `env_file = Path(args.env_file) if
  args.env_file is not None else None` (`:1057`) and loads the DEFAULT `.env`. No warning, no
  error — a batch run against the wrong credentials or the wrong provider account looks normal.
  **Reproducer (offline, $0.00):**
  ```
  $ pixi run python -c "from kinoforge.cli._main import _build_parser; p=_build_parser(); \
...
edit it; it belongs to the coordinator).

**What shipped.** H3 (`minimax_h3_server.py`) now mounts the same `/lora/set_stack` HTTP
contract Wan serves, extracted into a shared router (`servers/_lora.py`, D1) so a second
from-scratch LoRA loader was not written. The schema generalises `branch` (Wan's MoE
high_noise/low_noise/auto vocabulary) into `target: str | None` (a server-profile-scoped
routing token — H3's is `transformer`, future `transformer_ref`); `branch` still loads, maps
onto `target`, warns once per entry, and a disagreeing pair is a hard config-load error (see
`docs/breaking-changes.md`, new top entry). A new `core/lora_apply.py` seam applies the
resolved stack over HTTP once a pod reports ready and **fails the run** (not a warning) if the
apply does not succeed — this is what makes a bad LoRA stack visible instead of silently
producing LoRA-less video. `LoraServerSupportCheck` refuses an unserveable `loras:` block at
config load for diffusers engines. The turbo-LoRA config
(`examples/configs/modal-diffusers-minimax-h3-t2va-lora-turbo.yaml`) shipped alongside a new
env-payload ratchet (`tests/providers/test_env_payload_ceiling.py`) — see **U53** below for what
that ratchet found and did NOT fix.

**Known debt this increment creates, spec §10 (do not lose this by only reading the code):**
1. **Two LoRA implementations coexist.** Wan keeps its own inline `LoraTarget` /
   `_inventory_snapshot` machinery in `wan_t2v_server.py`; H3 uses the new shared
   `servers/_lora.py` router. Unifying them means disentangling Wan's registry from the
   `/upscale` + `/interpolate` co-residency LRU — deliberately out of scope this increment.
2. **No eviction on H3.** H3 serves `/lora/set_stack` without the LRU eviction policy Wan's
   warm-reuse matcher uses, and nothing routes a warm-attach decision onto an H3 pod yet — so
   `<volume>/loras` on an H3 pod only ever grows (see `docs/warm-reuse.md`'s new subsection).
3. **Wan cold-boot `cfg.loras` is still a no-op.** `KINOFORGE_INITIAL_LORA_STACK_JSON` (the
   env-var path that would apply a LoRA stack before `/generate` is ever called) remains
   dormant on the Wan side — unchanged by this branch, not fixed by it. The new `lora_apply.py`
   seam applies the stack over HTTP AFTER the pod is ready, which is a different mechanism and
   does not touch this gap.

**Five defects filed by this build, deliberately NOT fixed (widening scope at the end of a
branch): U53-U57** — see the URGENT ACTION ITEMS table below for the full record of each. In
one line apiece: **U53** RunPod diffusers configs are over the ~101 KB create-mutation ceiling
(pre-existing, this branch adds ~3.7 KB) and CLAUDE.md's "~4x headroom" note is rotted; **U54**
a persisted ledger `lora_inventory` row can be silently redaction-corrupted before a
cross-process matcher ever reads it; **U55** an H3 inventory row's `branch` field is stamped
with H3's `target` vocabulary (e.g. `"transformer"`), which will fail warm-attach's branch
comparison the moment matcher integration is wired; **U56** a hosted-engine cfg carrying
`loras:` still generates LoRA-less, now WARNed instead of silent, but the real fix needs
per-engine LoRA capability in the registry; **U57** three LoRA-resolution failure classes
(`LoraStackConflict`, `UnknownAdapter`, the zero-artifact `ValidationError`) first fire inside
`core/lora_apply.py` AFTER the pod already booted, so they cost a boot instead of failing at
preflight.

**Two documentation corrections made this session:** the spec + plan's turbo-LoRA byte count
was transcribed from the WRONG sibling file's probe output (see the Task 9 controller-correction
note in `.superpowers/sdd/2026-09-22-h3-lora-shared-seam/progress.md`) — fixed in both docs. The spec's §2 D8 ("Wan is untouched") is annotated to point at §3, since `LoraTarget` in
`wan_t2v_server.py` DID gain the additive `target` field (forced by `extra="forbid"`); the
...
**U34 is CLOSED, and it closed the way the ledger keeps teaching: by measuring the thing rather
than reading it.** The question was whether the cu124 INDEX or the base IMAGE supplies
`torch 2.6.0+cu124`. Two pods, one harness, one session, one rendered flag different, accelerator
ORDER held identical so the arms could not diverge on hardware:

- **ARM A** (PyPI-pinned index = pre-U33), pod `67k6ihp9w1usli` -> **`torch 2.6.0` BARE**.
- **ARM B** (cu124 index = post-U33), pod `u7g1h7fu0j7scm` -> **`torch 2.6.0+cu124`**.

**The INDEX supplies it. The image ships bare 2.6.0 despite its `2.4.0` tag.** U33 does change the
installed wheel on every RunPod diffusers cfg — and it is safe in the strongest available sense:
**both arms' output mp4s are BYTE-IDENTICAL** (same sha256, same 1294708 B), both frame-QA PASS.
The fallback U34 proposed (make an unset key mean *no* index) is **not needed and should not be
built**. The residual cu128 concern is split out as **U37**.

**Why one arm was not enough, and this is the reusable part.** Arm A alone showed bare `2.6.0` and
"every pin already satisfied". It was tempting to stop there and pair it against the PREVIOUS
session's `+cu124` reading. That comparison would have been wrong: the prior reading came from the
**second** pip invocation, after the first had already installed the cu124 wheel, so it never
described the image at all. *When two hypotheses predict the same log SHAPE, the shape is not the
evidence — find the field they disagree on.* Both arms print the identical "Requirement already
satisfied" line; only the version string separates them.

**The harness cost three failed captures before it captured anything, and each failure was mine:**
1. Run 1 read the log AFTER the run — `--no-reuse` had already destroyed the pod. The evidence
   lives for ~100 s and only on the pod.
2. Run 2 polled on the main thread, which sat blocked in `readline` for the 98 silent seconds it
   was supposed to be sampling. Moved to a background thread.
3. Run 3 got **HTTP 403 on every poll** — and the `:8000` CONTROL is what made that readable: the
   orchestrator pulled the artifact off `:8000` on that same host seconds later, so same host,
   same moment, one client served and one refused => the fault was the CLIENT. Cause was already
   documented in the engine: RunPod fronts pods with Cloudflare, which **403s the default
   `Python-urllib` User-Agent**. *A negative result that cannot name its own cause is not a
   measurement* — the first version of `_fetch` collapsed 404, DNS, TLS and empty-body into the
   string "not servable yet", which was worth nothing.

**Two filter artifacts caught, same family as the `manylinux1` mistake already on U34's row:** the
pip line runs with `-q`, so `Collecting`/`Downloading` appear in NEITHER capture — arm A's silence
was briefly taken as "nothing downloaded"; it was a property of `-q`. And provision wall-time is
not evidence about wheels: arm B pulled 766 MB inside a ~80 s provision.

**U36 was diagnosed and dodged rather than paid again.** `placement.accelerators` is a preference
RANKING, not an allowlist (`filter_offers` sorts by it, does not drop unlisted types), and
`_create_pod` walks it in order. The probe led with the **L4** — advertised $0.44/hr, realized
$2.39/hr, over the $2.20 cap — which is the entire nine-attempt loop. Reordering so the 4090 leads
(`7ba8c3c1`) placed first-try. **The cap was deliberately NOT raised**: the guard behaved correctly
on every one of those nine attempts; the ordering was the cost. U36 itself stands as filed.

**Spend ~$0.05 across six pods** (one create failed transiently at $0.00 — U36's second failure
mode). Teardown verified from a fresh process after every run: both `kinoforge list` lines clean.
Frame QA done on every output video.

**U37 was filed and then CORRECTED within the hour by the $0.00 render it asked for — against
itself.** It inherited U34's prose claim that the FlashVSR x4 cfg "installs `torch==2.6.0`" under a
cu128 image. It installs **no torch at all**, deliberately, with a long comment explaining that
*reinstalling* torch is what breaks BSA's c10 ABI. Measured over every shipped cfg: **20 pin
`torch==` and do get a wheel changed by U33; 5 pin none — and the five are exactly the cu128/BSA
ones the filing accused.** The genuine residual is **Modal**, whose `python:3.13-slim` base ships
no torch, so it really downloads the wheel U33 switched (RunPod's image already satisfied the pin,
which is why U34's byte-identical proof does NOT transfer). *Fourth time in this ledger a filing's
scope claim was a hypothesis and one grep settled it.*

**U37 was then ANSWERED the same session, and it took TWO Modal runs because the first had no
channel to read.** Modal streams no container stdout, the engine's pip step runs `-q`, and
`modal app logs` on a stopped ephemeral app returns the image build — so the startup log line added
for this (`01e8fdaa`) was the right fact on the wrong channel, and the first Modal run was green
while learning nothing. `/health` now carries the torch build too (`1f7e568c`), and pod
`eph-aab25c06` answered **`{"version": "2.6.0+cu124", "cuda": "12.4"}`** on `python:3.13-slim`.
Since that base ships no torch at all, Modal genuinely DOWNLOADS the wheel U33 switched — where
RunPod's image had already satisfied the pin. **The index decides the build on both providers.**
*A diagnostic is only as good as the channel it lands on; check the channel exists on the provider
you need it for.*

**U24 is CLOSED — and its proof found the fix was one field short.** The swap group ran twice.
First attempt: cell 0 cold-booted and generated, cell 1 refused to attach
(`ensure_endpoints returned an empty map`) — the index lookup U24 added worked, and the step AFTER
it did not, because the row never emitted the `ports` tag RunPod derives proxy URLs from (**U39**,
fixed). Second attempt: **`status=full`**, one pod `kp4d0mcqvfa4zj` serving all three cells,
controller destroy fired, index empty, frame QA PASS — and the three captioned cells are visibly
different, so the LoRA stack genuinely swapped on the shared warm pod rather than silently
no-op'ing. **Two things worth carrying:** the observed RunPod ephemeral name is
`kinoforge-<8hex>`, not the `kinoforge-eph-<8hex>` U24 predicted (that prefix is Modal's); and the
whole chain was only visible because of **U38** — the first failure printed `status=partial` and
an empty directory while discarding the stderr it had already captured. *A live run that produces
no diagnostic is a live run spent twice.*

**U36 got sharper evidence, at no extra cost:** the SHIPPED grid cfg
(`examples/configs/grids/runpod-diffusers-wan-2_1-1_3b-base.yaml`) caps at $0.40/hr and the pod
realized $0.49, so that example is currently **unplaceable on RunPod** — the first run died on
`RateCapExceeded` before any cell. The proof ran against probe copies
(`tests/live/probes/u24-swap-group.grid.yaml` + `u24-swap-base.yaml`, one line different), because
U36's entry says in terms not to fix this by raising caps in shipped cfgs.

### SESSION 2026-09-12 — the shipped 1.3B swap grid runs end-to-end, after three more defects

**Operator decision:** `max_usd_per_hr` on `examples/configs/grids/runpod-diffusers-wan-2_1-1_3b-base.yaml`
raised **0.40 -> 0.60** (`2c446eef`), on that cap's own merits: at $0.40 no current RunPod offer
could satisfy it, so the shipped example was unrunnable. $0.60 clears the observed rates and still
refuses the L4 tier that realized $2.39. **U36 itself is untouched** and this is not licence to
raise other shipped caps.

**The cap change worked on the FIRST run** — the pod placed and cleared the rate check, where
$0.40 had destroyed it. Everything after that was three unrelated defects wearing one symptom
("cell 0 unrecoverable"), found by five live runs at ~$0.02 each:

- **U40** — the flatline predicate cannot distinguish an HF download from a corpse on RunPod
  (CPU-idle, mem-flat, `disk_percent` always None). Fixed with a log-progress signal, then
  **proven insufficient the same day**: the port-8001 sidecar is not proxy-reachable in the early-boot
  window the fix targets, so it never fired. *Correct logic, unreachable.*
- **U41** — the grace ignored the cfg's own `boot_timeout: 30m` and aborted at ~3 min. Now scaled
  to 25% of the declared budget, clamped [90 s, 900 s]. **Falsifying the wiring left all 23
  boot-liveness tests green**, the same shape as U40's inertness, so the seam is pinned by its own test.
- **U42** — the cfg was the only RunPod one not pinned to `cloud_type: secure`, and the community
  pool reclaimed two pods mid-boot. The symptom is indirect: terminate then fails `POD_NOT_FOUND`,
  which reads as a GraphQL error rather than "the host took your pod".

**Final run GREEN:** pod `v08y8axqfzu4fo`, `status=full`, exit 0, composed 1440x480 mp4, ONE pod id
across 14 polls (all three cells shared the warm pod), teardown verified from a fresh process,
frame QA PASS with three visibly distinct LoRA stacks.

**A pricing result that inverts the obvious assumption:** the secure host billed **$0.27/hr**,
CHEAPER than the community pods realizing $0.49-$0.59. So secure is not a cost trade here, and
**U36's advertised-vs-realized gap may be substantially a community-pool artifact** — worth
measuring before building U36's retry.

### SESSION 2026-09-14 — U36 is SOLVED, and it was never a pricing mystery

**U36's root cause is a POOL-FILTER BUG in `find_offers`, and RunPod's advertised rate turns out to
be perfectly deterministic.** Measured on four real pods for ~$0.01
(`tests/live/probes/u36_pool_price_probe.py`, committed RED before the spend at `2a8d37e4`):

| gpu / pool | adv(all) | adv(pool) | realized | |
|---|---|---|---|---|
| RTX 4090 / SECURE | 0.340 | 0.740 | **0.740** | 2.18x vs all, **1.00x vs pool** |
| RTX 4090 / COMMUNITY | 0.340 | 0.340 | **0.340** | 1.00x vs all, **1.00x vs pool** |
| L4 / SECURE | 0.440 | 0.490 | **0.490** | 1.11x vs all, **1.00x vs pool** |
| MI300X / SECURE | 0.500 | 2.390 | **2.390** | 4.78x vs all, **1.00x vs pool** |

**Realized == advertised(pool) EXACTLY on four arms out of four.** There is no variance to model, no
"RunPod places you on whatever is free", no 5-10x under-prediction. `find_offers` sends
`lowestPrice(input: { gpuCount: 1 })` with **no pool filter**, which returns the COMMUNITY floor on
every GPU that exists in both pools; the create then pins `cloudType: SECURE`. kinoforge filters,
ranks and rate-caps on the price of a pool it never books into. Measured across the whole catalog:
on every dual-pool GPU the unfiltered price is byte-identical to the community price, and secure
runs 1.5-2.3x higher.

**This retires all three fix directions U36 proposed.** (a) widening the cap weakens the guard to
paper over a wrong lookup; (b) an in-provider `RateCapExceeded` retry is a convergence loop around a
question that has an exact answer; (c) surfacing "advertised $0.34 -> realized $2.39" dresses up the
wrong number instead of asking for the right one. The fix is to pass the cfg's own `cloud_type` into
the `lowestPrice` query — one field, and the cap then means what it says.

**The provider's capability docstring states the false claim in terms**, which is why this survived:
`RATE_DETERMINISTIC` is justified as "the pod is booked on the `gpuTypeId` kinoforge selected from
RunPod's own catalog, so the price that survived the pre-book filter is the price billed." The
capability is *true* — RunPod's rate really is deterministic — and the justification is *false*,
because the filter and the create read different pools. A capability declaration that reasons from
the mechanism instead of measuring it is the same failure this ledger has now recorded five times.

**U36's "realized $2.39/hr on the L4" was a MISATTRIBUTION, and the real machine is worse news.**
The L4 bills $0.490 secure, measured. $2.390 is the **AMD Instinct MI300X**'s secure rate, measured
to the cent. The MI300X ranks **second** in the shipped 1.3B grid cfg's offer list — an AMD card, for
a CUDA workload, ahead of every NVIDIA GPU the cfg actually named. Two defects put it there, both
filed below:

- **U43** — `filter_offers` ranks by exact string equality (`o.gpu_type in placement.accelerators`),
  so an accelerator name the provider's catalog does not carry is **silently inert**: no warning, no
  error, and the GPU it meant to prefer ranks no higher than one nobody asked for. **Seven shipped
  cfgs** name GPUs RunPod does not have — `NVIDIA RTX 4090` (the catalog id is `NVIDIA GeForce RTX
  4090`), `RTX 4090`, `A100 80GB`, `H100 80GB`, `A100 40GB`. Proven through the real code path: the
  shipped 1.3B grid cfg's `find_offers` returns the **L4 at rank 0** and the 4090 it intended to lead
  with at rank 4, behind the MI300X. *This is the whole of U36's "L4-first ordering" cause* — and
  `7ba8c3c1`, which dodged it on the probe cfg, worked only because that cfg spells the id correctly.
- **U44** — `find_offers` hardcodes `cuda="12.8"` on **every** RunPod offer regardless of GPU
  (`providers/runpod/__init__.py:628`), so an AMD MI300X, which has no CUDA at all, passes a
  `min_cuda: "12.4"` filter and is offered for a CUDA workload.

**Both were fixed the same day, and both first cuts were wrong in the same direction — over-blocking
a correct config.** U43's warning called the RTX A5000 a typo (a real id with no stock that minute)
and suggested `NVIDIA RTX A4000` for `NVIDIA RTX 4090` (difflib prefers the shorter string). U44's
obvious rule — "exclude anything whose id does not start with NVIDIA" — would have silently dropped
RunPod's `Tesla V100-PCIE-16GB` and `Tesla V100-SXM2-16GB`, which are CUDA cards. The first was
caught by running the warning against the live catalog; the second by writing the test before the
rule, having just been burned by the first. *Three times in one session, a guard's first cut was
more confident than the evidence it was built on — and each time the cost would have fallen on a
config that was already correct.*

**Live, all three fixes together** on the shipped 1.3B grid cfg at its $0.60 cap: the MI300X is gone
from both pools (U44), the renamed 4090 ranks first among in-stock preferences (U43), and under
secure pricing it is refused PRE-BOOK at $0.740 rather than booked and destroyed (U36).

### SESSION 2026-09-14 (continued) — U35 closes the matrix, and its golden immediately files U45

**U35 is FIXED and `EXCLUDED_CONFIGS` is now EMPTY** — the first time since the Modal command matrix
opened that every shipped compute config is inside the launch-payload ratchet. The stub transport
answered every mutation with the on-demand shape, so `mode: serverless` (which routes to
`saveTemplate`) died on `RunPod create-serverless returned no endpoint id`. One branch fixed it.

**The repo's own machinery drove the order, and that is worth noticing.**
`test_excluded_configs_exist_and_still_fail_to_capture` re-runs every excluded capture and demands
it STILL fails — so fixing the stub turned that test RED before anything else, and the exclusion
could not have been left behind even by an inattentive session. *An exclusion list that tests its
own justification is the only kind that expires.*

**The real work was NOT the stub — it was refusing to loosen two guards.** Bringing the serverless
golden in broke `test_runpod_goldens_carry_a_real_create_pod_input` and
`..._the_provision_script_on_the_wire`, which assert `cloudType` / `dockerArgs` / `ports` / `env` —
none of which exist on `saveTemplate`. The tempting fix is to relax those predicates until the new
payload fits; that would have quietly gutted the guard for the 31 configs that really are pods. Both
are now scoped to pod goldens **with a `>= 5` floor so the scoping cannot silently empty them**, the
capture's `seam` field names the mutation it actually used (so the split is on recorded fact, not a
filename convention), and the serverless shape got its OWN guard instead of a pass — one that
asserts values hand-computed from the cfg (`job_timeout: 20m` -> `executionTimeoutMs == 1_200_000`,
`idle_timeout: 1h` -> `idleTimeout == 3600`), confirmed by falsifying both a unit slip and a
collapsed payload. *Scoping a guard is the moment it is easiest to destroy; the floor and the
replacement guard are what make it a narrowing rather than a hole.*

**Reading the new golden filed U45 on the spot.** The serverless path hardcodes
`"gpuIds": "ADA_24"`, so `placement` — enumerated, filtered, rate-capped and ranked on the way in —
is **discarded**; the `offer` parameter is never read and `cost_rate_usd_per_hr` is hardcoded `0.0`,
which satisfies `_enforce_rate_cap` for any cap. And the mutation carries **no `env`**, so neither
`KINOFORGE_PROVISION_SCRIPT` nor `RUNPOD_TERMINATE_KEY` reaches that path. **The 0.0 is deliberately
NOT called a defect** — serverless bills per execution, so an hourly rate may genuinely not apply,
and nobody has measured it. *This is the U36 lesson being applied rather than re-learned: the fix
differs completely depending on the measurement, so the measurement comes first.* That U35's whole
value was making this visible in a single afternoon is the argument for the ratchet in one line.

**And then reading it once more filed U46, which is the bigger one: the serverless create is INVALID
GRAPHQL and has never worked.** The mutation declares `EndpointInput!` and calls `saveTemplate`,
which takes `SaveTemplateInput`. RunPod answers `GRAPHQL_VALIDATION_FAILED` as a raw **HTTP 400** —
the exact shape `CLAUDE.md` already warns reads as "RunPod is down" rather than "your request is
malformed". Every `mode: serverless` create has always failed.

**The whole diagnosis cost $0.00 and created nothing, because GraphQL validates before it executes.**
Sending the provider's own mutation with `input: {}` returns the schema's verdict as a
side-effect-free error. Three facts arrived from that one probe: the correct field is
`saveEndpoint(input: EndpointInput!)` — kinoforge declares the right TYPE and names the wrong FIELD,
which is why it looks so nearly right; `EndpointInput` accepts `templateId` and requires only `name`;
and `saveTemplate(SaveTemplateInput!)` requires `containerDiskInGb`, `dockerArgs`, `env` and `name`.
That last one **explains U45's missing `env`** rather than merely restating it: on RunPod, image and
env and dockerArgs live on a TEMPLATE and an endpoint references it by id, so the fix is two
mutations, not one with more fields.

**The probe stopped one character short of creating something.** A `name` longer than two characters
gets past schema validation into RunPod's own resolver, which is a real endpoint. Knowing where the
free part of a probe ends is the whole skill — *validation is free, resolution is not.*

**Deliberately NOT half-fixed.** Renaming the field to `saveEndpoint` would turn a loud, honest 400
into a created-but-imageless endpoint — a silent wrong result in place of a noisy right one. Filed
with the schema facts attached so the fix is mechanical rather than exploratory.

*The pattern across this whole session, four times over: the cheap read contained the answer and the
expensive part was only ever confirmation.* U36's pool prices, U43's catalog ids, U44's vendor list
and U46's schema were all free. The four pods cost a cent and told us nothing the catalog had not
...
### SESSION 2026-09-16 — the `Offer.cuda` audit, and SkyPilot's was worse than RunPod's

**U44 ended by saying `Offer.cuda` is a provider-asserted constant worth auditing on the other
enumerating providers. It was. Score: 2 of 4 asserted a CUDA version they could not know, and both
were load-bearing.**

**SkyPilot (U48) reported a value from a lookup that cannot succeed.**
`cuda=_record_field(info, "cuda", default="12.0")` reads like a catalog lookup;
`sky.catalog.common.InstanceTypeInfo` has no `cuda` field at all. Checked against the pinned sky in
`live-skypilot` rather than assumed — its `_fields` are cloud, instance_type, accelerator_name,
accelerator_count, cpu_count, device_memory, memory, price, spot_price, region. So the default was
the only value producible, on every offer, forever. *A default that can never be displaced is not a
default, it is a constant wearing a lookup's clothes.*

**The harm is bigger than U44's.** `Placement.min_cuda` defaults to `"12.8"`, so a cfg that never
mentions CUDA had its **whole catalog filtered away** — an H100 rejected against a floor that
describes nothing about it. The tell was in the configs, not the code: **all five shipped SkyPilot
cfgs set `min_cuda` to `"0"`, `"11.0"` or `"12.0"`**, and it is load-bearing for the two that name no
`accelerators`. A config surface uniformly avoiding a default is evidence about the default.

**Modal and Local were audited and deliberately NOT changed**, which is half the value of doing an
audit properly. Modal's `"12.8"` is an assertion whose own comment admits it was picked to clear the
default floor — but the fleet is NVIDIA-only, so U44's bug cannot arise, and the claim is real and
sourced. Giving it SkyPilot's treatment would REMOVE a filter that bounds a genuine risk. Local's is
a fixture on an unbilled in-process fake. *An audit that changes everything it touches has not
distinguished anything.*

**The parity guard made the fix honest without being asked.** Dropping `min_cuda` to UNSUPPORTED
broke `test_no_proof_outlives_its_declaration[skypilot]` and `test_wire_proof_holds[skypilot-min_cuda]`
until the obsolete wire proof was removed — a proof that had only ever demonstrated a fabricated
constant could be filtered against itself. Declaration and evidence are not allowed to drift apart
here, and that is worth more than either alone.

**Two near-misses in the fix itself, both the same shape as the last three.** The finding had to be a
WARN, not an ERROR: an ERROR row would have REFUSED the two shipped cfgs that set `min_cuda`
perfectly reasonably — turning a silent wrong answer into a hard refusal of working configs is not an
improvement. And the sentinel is `"9999"` rather than the obvious `"99.0"`, because that is the exact
...
instructive reason: at the hour the catalog was first dumped the **RTX A5000** had null stock in
every pool, so it printed no price at all and was read as absent rather than as temporarily
unavailable. It came back within the session, and its secure rate is **$0.270** — the 2026-09-12
observation to the cent, and its community rate is $0.160. *An absent row is not a zero; a catalog
read is a snapshot and was treated as the catalog.*

So realized == advertised(pool) holds on that reading too, and the actual mistake on 2026-09-12 was
comparing **different machines**: a secure A5000 at $0.27 against community pods that were other
GPUs at $0.49-$0.59. Held to the SAME GPU, secure is strictly the more expensive pool every time —
A5000 $0.16 -> $0.27, 4090 $0.34 -> $0.74, 3090 $0.22 -> $0.50. The hypothesis that session filed
("the gap is a community-pool artifact") is still refuted; the number it rested on was real.

**Post-fix demonstration against the live catalog**, shipped 1.3B grid cfg, $0.60 cap:

- **before** — 12 offers survive, led by A5000 $0.160, L4 $0.440, **MI300X $0.500**, 4090 $0.340.
- **after** — 7 offers survive, led by A5000 **$0.270**, L4 **$0.490**, A40 $0.490, 3090 **$0.500**.

The secure 4090 ($0.740) and the MI300X ($2.390) are now **refused pre-book** instead of being
booked and destroyed, which is the entire nine-attempt loop U36 was filed for. Note the fix also
removes the AMD card from this cfg's list as a side effect of pricing it correctly — **U44 still
stands**, because an `any` cfg or a higher cap would offer it again.

**Method note worth carrying.** The offline half — three `lowestPrice` queries with different pool
filters — cost $0.00 and contained the entire answer. The four pods only confirmed it. The reason to
spend anyway was that the offline half is a claim about a *mechanism* and U36 is a claim about an
...
### NEXT ACTION (single, current as of 2026-09-17)

**The urgent-items ledger has nothing actionable left that is cheap.** 45 of 49 fixed or answered;
U5 and U10 remain partly fixed from long ago; U45 and U46 are OPEN but CONTAINED behind the
validation refusal, so neither can bite anyone silently. The `Offer.cuda` audit is complete across
all four providers and the `unknown` GPU-type question is answered (U49 — no defect, and it corrected
a claim this ledger had asserted rather than measured).

So the next move is a choice rather than a queue:

1. **U45/U46 — build RunPod serverless for real**, if it is ever wanted. Both rows carry the full
   schema already probed at $0.00 (`saveTemplate` then `saveEndpoint`, `isServerless: true`,
   delete-by-NAME, the endpoints list as a free 0 -> 1 success signal) plus the template-lifecycle
   decision named and left open. **The protocol gap is the real work** — a serverless worker polls
   RunPod's queue through a handler, and every kinoforge engine is an HTTP server behind the pod
   proxy. That is a brainstorm-and-plan piece, not an afternoon.
2. **Anything the operator actually wants to generate.** The compute seam is in good order: U36 made
   the rate cap mean what it says, U43/U44/U48 stopped three providers offering hardware nobody
   asked for, and every shipped compute cfg is inside the launch-payload ratchet.

**The method that paid five times running**, and the one thing that did not: U36's pool prices,
U43's catalog ids, U44's vendor list, U46's schema, U48's `InstanceTypeInfo._fields` and U49's
catalog row were ALL free reads, and each turned a guess into a fact before a line was written. The
single thing that genuinely required spending was proving realized == advertised, which no read could
establish. **Probe first; spend only for what a read cannot settle.**

**And the two failure shapes this stretch kept producing**, worth reading before adding any guard:
*over-blocking* — U43 calling a real out-of-stock GPU a typo, U44 nearly dropping the CUDA-capable
Tesla V100s, U46's check nearly refusing Modal over a word on a different field, U48's sentinel
nearly colliding with a test fixture's impossible value; and *asserting across a gap* — U49's
"priced and bookable" filled in between two probes that never overlapped, and U36's "$2.39 on the L4"
which was the MI300X all along. Both are cured by the same question: what would I have to observe for
this to be false, and did I observe it?
...
`kinoforge-eph-<8hex>`, cell 2 attaches from a separate process with zero `✓ App deployed` lines,
the controller's destroy fires, and the ephemeral index is empty afterwards. **Do not accept
`grid --dry-run` as the proof.** After that the ledger's only open items are **U35** (harness-only)
and **U36** (RunPod's advertised-vs-realized rate gap; the guard is correct, the loop is the cost).
...
**Spend: ~$0.11, and $0.07 of it bought nothing** — nine live attempts, one green. The green one
(pod `h0921u4lt7bmln`, RTX 4090 $0.74/hr, ~$0.015, 70 s) proved U33 causes **no regression**:
provision succeeded, artifact published, frame-QA PASS, teardown verified from a fresh process with
both `kinoforge list` lines clean. Seven of the other eight died on `RateCapExceeded` at $2.39/hr
against the $2.20 default cap, and two on transient RunPod create errors — that is **U36**, newly
filed: RunPod's advertised `lowestPrice` ($0.22-$0.44 across this cfg's whole allowlist) under-
predicts the realized pod rate by 5-10x, so a routine run becomes a create-then-destroy loop at
~$0.008 a turn. **S4's realized-rate check behaved exactly as designed every single time** — the
cost is the loop, not the guard.

**U34 is NOT closed, and the distinction matters.** The pod ran `torch 2.6.0+cu124`, which is what
U33 predicted — but pip logged it as *"Requirement already satisfied"* and the provision took 64 s,
far too short to have pulled a 766 MB wheel. So the base image may already ship `+cu124`, in which
case U33 changed nothing on RunPod. The discriminator is committed and ready and was NOT run;
running it is the next session's first action. *Reporting the reading I preferred would have banked
a false green on a question the run could not answer.*

**Two items were FILED rather than fixed.** U35 is harness-only. U36 is above.

**A probe mistake worth copying the correction from:** the first resolve passed
`--platform manylinux1_x86_64` alone, which excludes the pytorch index's wheels, and duly reported
a byte-identical wheel — "the fix is inert". It was an artifact of the filter, and it was reported
before it was checked. *A negative result from a filtered query is a statement about the filter.*

### SESSION 2026-09-10/11 — six defects closed, $0.05 of live spend, ledger down to three

**Closed this session: U2, U1, U25, U19, U6, U31, U24 and U32** (U24 offline-only — its live
proof is owed; U32 was found live while hunting U13) — U31 was FILED in this same session, from the
U13 measurement, and then fixed once it became clear its own "past the one-guard bar" assessment
was wrong. The ledger went from **7 open to 0 open**: seven fixed, U24 owing a live proof, and
**U13 hunted live and NOT REPRODUCIBLE** — a negative result, not a closure, since nothing was
fixed. Spend: **~$0.45 total** ($0.036 U19, $0.014 U6, ~$0.4 for the U13 hunt, most of that one
FlashVSR image re-bake). The U13 hunt paid for itself by finding **U32**: the spandrel weights
fetch aborted the Modal image build on an unbound `${HF_TOKEN}`, so every Modal spandrel cfg was
unbuildable.
...
### START HERE — SUPERSEDED (was: U34, then U24's live proof). **U34 is CLOSED (answered live 2026-09-11, third session — see the top of this snapshot). Do NOT re-run its probe.** U24's live proof still stands; read on for it

**The U32 family is DONE — both items.** Do not re-open either.

- **U32** — fixed AND live-proven (`7bd528e3`), 2026-09-11.
- **U33** — fixed offline `af36000b`, RED scaffold `743b8f76`. Read its STATUS INDEX row before doing
...
1. ~~**U34 — ONE command, ~$0.015, and it is the first thing to run.**~~ **DONE — U34 is CLOSED**
   (2026-09-11, third session): answered by a paired A/B, the cu124 INDEX supplies the wheel, and
   both arms' outputs are byte-identical. The probe cfg below was run, plus a cu124 twin
   (`tests/live/probes/u34-spandrel-cu124-index.yaml`); evidence is committed. Kept for provenance
   only — do not re-run it. Note the probe cfg was amended (`7ba8c3c1`) to lead with the 4090
   rather than the L4, which is what made it placeable at all; see U36.

   ```
   pixi run kinoforge upscale \
     -c tests/live/probes/u34-spandrel-no-pytorch-index.yaml \
     --video output/20260630-221857_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4 \
     --no-reuse --ephemeral
   ```

   Then read the pod's torch version out of `https://<pod-id>-8001.proxy.runpod.net/bootstrap.log`.
   **Still `2.6.0+cu124` → the base IMAGE supplies it, U33 is a no-op on RunPod, and the exposure
   is Modal's different base plus any image bump. Bare `2.6.0` → the INDEX supplies it and U34
   stands as filed.** Background: the 2026-09-11 run proved U33 causes no regression (pod
   `h0921u4lt7bmln`, green, frame-QA PASS) but could NOT separate those two explanations, because
   pip logged torch as already satisfied and the whole provision took 64 s. Nine attempts to place
   this probe failed on U36, not on anything wrong with the probe.
...
- **U24's live proof** — the fix shipped 2026-09-11 offline; what is owed is one real swap group:
  cell 1 cold-boots under an opaque `kinoforge-eph-<8hex>` name, cell 2 attaches from a separate
  process with zero `✓ App deployed` lines, the controller's destroy fires, and the ephemeral
  index is empty afterwards. Use the CHEAP spec
  (`examples/configs/grids/wan-2_1-1_3b-loras-swap.grid.yaml`, 1.3B), not the A14B one.
  **Do not accept `grid --dry-run` as the proof** — it returns before `run_grid` and never
  reaches the changed code.
- ~~**U13**~~ — hunted live 2026-09-11 and **NOT REPRODUCIBLE** across two engines, both reaching
  a real server-side `UpscaleFailed`. Nothing was fixed, so do not treat it as closed; if it is
  ever seen again, run the committed probe (`tests/live/_u13_hang_probe.py`) FIRST. Read its row
  for the one dimension the hunt could not reach (a failure after a full inference run).

**Three items were filed as "past the one-guard bar" — U28, U31 and U24 — and all three were
not.** Each assessment was reasoned from the shape of a mechanism without grepping the layer it
claimed was missing: U28's "new cross-layer seam" already existed as `on_instance_created`, U31's
"shutdown semantics for every command" was one line in one kinoforge-owned class, and U24's
"cross-process handoff" already existed as the ephemeral index. **Treat that label as a
hypothesis, never as a reason to skip the entry.**

~~Do U1, U6, U19, U24 or U25~~ — U1 (`33a31586`), U25 (`602c3f7a`), U19 (`96d6920e`), U6
(`b79e32b0`) and U31 (`952869a4`) are all fixed; U19 and U6 live-proven.
...
a real failing Modal upscale, so the next U13 observation is a LIVE one: fold it into the next
Modal upscale run and dump the HUNG process (`py-spy dump --pid`, or `faulthandler` armed in the
driver) before killing it. See U13's STATUS INDEX row for what is eliminated and for the framing
that made it falsifiable — the traceback prints BEFORE the hang, so the holder cannot be in any
`finally`, only in interpreter shutdown.

**U31 was filed 2026-09-10 from that measurement and is OPEN:** a DAEMON `ConcurrentPool` worker
parked mid-work-item blocks interpreter exit anyway, because `concurrent.futures`' own atexit
handler joins every registered worker unboundedly regardless of the daemon flag — demonstrated at
$0.00. It is U13's symptom on `generate` / `batch`, and it is NOT U13's holder (`upscale` submits
nothing to the pool). The false docstring claim in `_DaemonThreadPoolExecutor` that this disproves
is corrected; the behaviour is deliberately unfixed, because every honest fix changes
process-shutdown semantics for every command.
