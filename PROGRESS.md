...
Thirty-seven items, U1-U37. **Thirty-one are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
U11, U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30,
U31, U32, U33, U34), **two are
partly fixed** (U5, U10), **one is fixed but OWES a live proof** (U24), **three are OPEN**
...
though the underlying item stands. U37 — the FlashVSR x4 cfg pins a cu128 image and a torch-2.8
wheel while provably installing `2.6.0+cu124`; its cheapest next step is a $0.00 offline render),
and
**U13 is NOT
...
| U37 | OPEN — filed 2026-09-11, split out of U34 on its resolution | **The FlashVSR x4 cfg's cu128 intent is expressed by no cfg, and U34's A/B now proves what it actually gets.** The cfg pins a cu128 base image and a `bsa-cu128-torch2.8-v1` wheel while installing `torch==2.6.0` and setting no `pytorch_extra_index_url` — so after U33 it renders the engine's cu124 default and installs `2.6.0+cu124`. Before U34 this was an inference from reading the cfg; it is now a measurement, because the paired arms established that the index is exactly what decides the installed build (bare `2.6.0` under PyPI, `2.6.0+cu124` under the cu124 index — pods `67k6ihp9w1usli` / `u7g1h7fu0j7scm`). So a cfg that names cu128 in a comment, pins a cu128 image, and fetches a torch-2.8 wheel is provably running a cu124 torch 2.6.0 underneath it. **Not yet known whether this is harmful**: U34 proved cu124-vs-bare is byte-identical on the spandrel x2 path, which says nothing about a cu128 wheel loaded against a cu124 torch — that pairing is the one that would fail loudly at import or silently at numerics, and FlashVSR is the engine with a false-colour corruption history (entries #13/#14, U-fix `e82b0d1`). **Cheapest decisive step:** render the x4 cfg offline and read its pip line (`DiffusersEngine().render_provision(cfg.model_dump())`, grep `extra-index-url`) — $0.00, no pod — then decide whether the cfg should declare `pytorch_extra_index_url: .../whl/cu128` or drop its cu128 claims. **Do not assume the comment or the image tag tells you what is installed** — that assumption is precisely what U34 cost two sessions to disprove |
...
**Do U37's $0.00 step first — it needs no pod.** Render the FlashVSR x4 cfg offline
(`DiffusersEngine().render_provision(load_config(...).model_dump())`, grep `extra-index-url`) and
confirm what it actually installs; then decide whether the cfg declares cu128 or drops the claim.
After that, the only live debt left is **U24's** cheap 1.3B swap group (see below, unchanged) —
and U35, which is harness-only.
