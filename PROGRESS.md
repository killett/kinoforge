...
**Spend: $0.00 — including the measurement that rescoped U34.** Two items were FILED rather than
fixed. **U34** started as "the FlashVSR x4 cfg claims cu128 and renders cu124" and a $0
`pip --dry-run --report` showed it was much bigger: U33 changes the installed torch wheel on EVERY
diffusers cfg (`2.6.0` → `2.6.0+cu124`, different hash), and since the engine always rendered
`--extra-index-url None`, **no pod has ever installed a `+cuXXX` wheel** — so the path U33 turns on
is unexercised and its live proof is a blocker. **U35** is harness-only. *The third scope claim of
the session to be wrong, and the first one of mine.*
...
1. **U34 — do this BEFORE any other live diffusers work; it is a blocker, not polish.** Measured
   for $0.00 after the fix landed, and it is bigger than the row's original framing: **U33 changes
   the installed torch wheel on EVERY diffusers cfg.** PyPI-only resolves `torch 2.6.0`
   (`manylinux1_x86_64`, sha `7979...`); with the cu124 index it resolves **`torch 2.6.0+cu124`**
   (`linux_x86_64`, sha `d4c3...`) — the pytorch indexes publish under local version tags and
   `2.6.0+cu124` sorts above bare `2.6.0`, so pip prefers it. Because the engine rendered
   `--extra-index-url None` on every build there has ever been, **no kinoforge pod has ever
   installed a `+cuXXX` wheel**; every green live run to date was on PyPI's build. The fix restores
   the documented intent and cu124 matches the image's CUDA 12.4 — but it is an unexercised path,
   and `+cuXXX` wheels vendor CUDA differently, so the dependency closure differs too. Prove it on
   one upscale-only run (~$0.08): `pip list | grep -E 'torch|nvidia'` and
   `python -c 'import torch;print(torch.__version__, torch.version.cuda, torch.cuda.is_available())'`
   from the pod, then frame-QA. **If it regresses**, the behaviour-preserving fallback is in U34's
   row: let an unset key mean *no* extra index, which still fixes all three U33 sites.
   **Do NOT repeat the probe mistake recorded in that row:** a `pip --dry-run` with only
   `--platform manylinux1_x86_64` hides the pytorch index's wheels and will tell you the fix is
   inert. It is not.
