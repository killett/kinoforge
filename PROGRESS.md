...
**STATUS INDEX (current as of 2026-09-14. Provenance, newest first: **U36 is ROOT-CAUSED and it was
...
The 2026-09-12 "secure is cheaper" reading does NOT reproduce and is superseded — secure is uniformly
1.5-2.3x the community price. Before that: the SHIPPED 1.3B swap grid
...
Forty-four items, U1-U44. **Thirty-eight are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
U11, U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30,
U31, U32, U33, U34, U37, U38, U39, U40, U41, U42), **two are
partly fixed** (U5, U10), **NO live proof is owed** (U24 discharged 2026-09-11), **four are OPEN**
(U35 — the serverless cfg renders now but the snapshot harness cannot capture its create call, so
it is still unfrozen. U36 — ROOT-CAUSED 2026-09-14 and no longer a mystery: `find_offers` reads the
COMMUNITY price while the create books SECURE, realized == advertised(pool) on 4/4 measured arms;
open only because the one-field fix is not written yet, and its option-(b) retry is retired.
U43 — an accelerator name the provider catalog does not carry ranks no higher than an unlisted one,
...
**The 2026-09-12 "secure is cheaper" reading does NOT reproduce and should not be carried forward.**
That session recorded a secure host billing $0.27/hr against community pods at $0.49-$0.59 and asked
whether the gap was a community artifact. It is the exact opposite: secure is uniformly the more
expensive pool, 1.5-2.3x. No GPU in the catalog advertises $0.27 in either pool. The $0.27 is
unexplained and was a single uncontrolled reading; this session's four controlled arms disagree with
it, so the controlled measurement wins. *The question it prompted was still the right one to ask —
it just answered the other way.*
...
**Fix U36: thread the cfg's `cloud_type` into `find_offers`' `lowestPrice` query.** The measurement
is DONE and it named the fix exactly (see the 2026-09-14 session above): realized == advertised(pool)
on 4/4 arms, so the only thing wrong is that `find_offers` asks the unfiltered query — which answers
for the COMMUNITY pool — while `_create_pod` books `cloudType: SECURE`. The seam is already clean:
`_create_with_offer_retry` holds the full `spec`, so `RunPodProvider.validate_options(
spec.backend_options.get("runpod", {})).cloud_type` is in scope at the one call site
(`providers/runpod/__init__.py:701`). Do NOT build U36's option (b) retry loop — there is nothing to
converge on once the lookup is right. Note that `secureCloud: false` returns `null` for
secure-only GPUs (L4, A40, MI300X), so a community cfg correctly stops being offered them, and
`cloud_type: any` must keep sending the unfiltered query it sends today.

Then **U43** (an accelerator name the catalog does not carry is silently inert — seven shipped cfgs
affected, and it is the whole of U36's "L4-first ordering" cause) and **U44** (`cuda="12.8"`
hardcoded on every RunPod offer, so an AMD MI300X passes `min_cuda`). U43's product fix is the
warning, not the seven renames; rename the cfgs too, but a cfg naming a GPU its provider does not
have must say so out loud. After that, **U35** is harness-only and is the last item from the
original matrix.
