...
**Subject-morph decision LOCKED (2026-06-03):**
- **Verdict:** ship `start_latent_strength=0.6` (commit `b7b4ff2`, graph node 63). Partial morph: human hand + bare shoulder visibly cradles the cat from t≈2.25s to t≈3.0s, then fades back to cat anchor for the remainder of the clip. The 0.6 knob preserves enough init-frame anchor that the diagonal-seam regression check stays load-bearing.
- **Evidence package:** `output/20260603-171159_a-cat-turns-into-a-w.mp4` (1 072 044 bytes, ftypisom, 11m 51s wall-clock, ~$0.053 spend). Pod `1j97p5p3xnk29n` provisioned + destroyed cleanly via the new L2 surface (`cold-path pod surfaced by generate()` log line at line 281).
- **Lower-strength options (deferred — out of current scope):** 0.4 likely produces full sustained woman but weakens init-fixture coverage (the diagonal-seam regression test wouldn't fail as starkly); 0.5 a middle ground. Not pursued — 0.6 meets the smoke purpose (ftyp + size AC + visible knob routing).

**Orphan-pod bug CLOSED (2026-06-03):**
- **Root cause:** `_pod_to_instance` in `src/kinoforge/providers/runpod/__init__.py:743` hardcoded `tags={"mode": "pod"}`. RunPod's GraphQL pod schema doesn't surface user tags on list/get responses, so `find_instance_by_tag` always missed cold-start pods even though `_create_pod` correctly attached `spec.tags` to the immediate-return Instance. Live smoke's `[phase=destroy]` block silently no-op'd every cold-start run; selfterm idle_timeout was the only thing reaping pods.
- **L1 fix (commit `93beb14`):** `RunPodProvider._created_instances` process-local registry populated on `create_instance`, consulted first by `find_instance_by_tag`, pruned by `destroy_instance` once teardown confirmed. RED test `test_find_instance_by_tag_recovers_just_created_pod` locks down the contract.
- **L2 fix (commit `7a10fd4`):** `orchestrator.generate()` now returns `tuple[Artifact, Instance | None]` so callers can teardown by id without tag fallback. Second element: `None` for hosted / caller-supplied-instance paths, otherwise the live Instance the orchestrator owns. 11 call sites updated (CLI + 6 test_orchestrator sites + 4 fake_generate stubs in test_cli + 1 e2e_spec_routing + the live smoke itself). Live smoke now binds `pod_id = owned_instance.id` directly when `generate()` returns it; tag-discovery cold-path block deleted.
- **Test count delta:** `1032 → 1034 pass` net (+2: L1 registry test + L2 tuple-shape test).

**Budget remaining: ~$11.26 of $15.** Total live spend across the item #3 wave: **~$0.74** (T6 + diagnostic + capture + quality re-render + cat-fixture re-render + morph re-render at strength=0.6). Within the $1.50 AC6 cap. Orphan-pod bug fixes (L1 + L2) were 100% offline (TDD red→green via FakeCreds + MultiResponseHttpPostSpy + LocalProvider); zero live spend.

**Item #3 wave fully CLOSED.** Layer P T8/T9/T10 follow-ups (refactor offline ComfyUI tests onto captured fixtures + README + main-branch merge) are now unblocked.
