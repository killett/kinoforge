...
## Active workstream
**C33 follow-ups (filed 2026-06-16, see C33 entry in §Known limitations for full evidence).** Priority order: **(h) cheapest unblocker** — single $0.02 probe with `KINOFORGE_SELFTERM_SCRIPT=""` override to test if selfterm causes the T+15s bash-exit during Wan cold-boot. **(i) blocks C25 production-heartbeat enable** — Wan workloads currently can't cold-boot regardless of whether `podEditJob` restarts containers, so C25 must wait. **(f) trivial** — fix the wrong "restart-on-failure" warning string in `src/kinoforge/providers/runpod/__init__.py` (Q9 proved RunPod is restart-always). **(a) small** — refine `classify_run` negative-uptime heuristic in `src/kinoforge/diagnostics/c30_probe.py` to require corroborating signal (S3 trap-fire or multi-sample pattern). **(d) small** — add a unit test asserting the codebase never reads top-level `Pod.uptimeSeconds`. **(g) larger** — design `diagnostic_mode: "trace"` cfg opt-in that cleanly bundles the Q5–Q8 bisection instrumentation (`set -x` + `PS4` timestamps + `ps`/`p.sh`/`selfterm.log` dumps + `stdbuf` flushing) so future restart-loop debugging doesn't require re-implementing the throwaway patches. 9 local commits ahead of `origin/main`; not pushed.
