...
Thirty-one items, U1-U31. **Twenty-eight are fixed** (U1, U2, U3, U4, U6, U7, U8, U9, U11,
U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30, U31),
**two are
partly fixed** (U5, U10), **one is open** (U13) and **one is fixed but OWES a live proof**
(U24). **U19 and U6 are both LIVE-PROVEN as of 2026-09-11, for $0.036 and $0.014** — the first
...
| U13 | OPEN — holder still unidentified, but FOUR candidate mechanisms are now eliminated by measurement (2026-09-10, $0.00) | The entry's own $0 first step was run and **did not reproduce the hang**, which is itself the finding. Probes live in `tools/diagnose_u13_exit_holder.py` (`8bed6296`). **Eliminated:** (1) the whole controller-side upscale path — the real CLI, orchestrator, `deploy_session`, heartbeat and `ConcurrentPool` with `UpscaleStage.run` patched to raise `UpscaleFailed`, against the local provider, exits in **0.8 s**; (2) the upscaler engines and `_pod_http`, which by inspection start no thread, subprocess or event loop at all — `submit_and_poll` is a synchronous poll loop over `urllib` with timeouts; (3) a real read-only Modal client, whose two threads (`thread_inner`, `asyncio_0`) are both DAEMON, exiting clean even under an uncaught exception; (4) a real in-process `app.deploy()` — the only in-process SDK work the Modal upscale path does — which exits in **2.1 s** under the same uncaught `UpscaleFailed` (CPU-only app, never invoked, no container, no compute booked; stopped and verified afterwards). **The framing that made these falsifiable, and which the entry should have carried from the start:** the traceback printing BEFORE the hang means the exception already reached the top of `main`, so the holder cannot be in any `finally` — including `deploy_session`'s documented unbounded `pool.close()`. Only interpreter shutdown is left, and only two things there block forever: `threading._shutdown` joining non-daemon threads, and `atexit` handlers, which may join threads of their own. **A daemon flag exempts a thread from the first and from nothing else** — see **U31**, a real instance of the second, demonstrated at $0 while chasing this. **Next step is a live observation, not another offline probe:** the holder appears only on a real failing Modal upscale, so fold it into the next one and dump the HUNG process (`py-spy dump --pid`, or `faulthandler` armed in the driver) before killing it. Do NOT spend a pod on U13 alone |
...
a task to pick up:**
**Two live debts, no offline work queued.**
...
- **U13** — blocked on a LIVE observation; see the paragraph below. Fold it into the next Modal
  upscale run rather than booking a pod for it.
