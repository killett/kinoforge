...
**STATUS INDEX (current as of 2026-09-16. Provenance, newest first: **U48 FIXED** — the `Offer.cuda`
...
Forty-eight items, U1-U48. **Forty-four are fixed or answered** (U1, U2, U3, U4, U6, U7, U8, U9,
U11, U12, U14, U15, U16, U17, U18, U19, U20, U21, U22, U23, U24, U25, U26, U27, U28, U29, U30,
U31, U32, U33, U34, U36, U37, U38, U39, U35, U40, U41, U42, U43, U44, U47, U48), **two are
...
### NEXT ACTION (single, current as of 2026-09-16)

**Nothing is blocked and nothing is mid-flight.** The Modal command matrix is discharged, and the
`Offer.cuda` audit it handed forward is now complete across all four providers.

The one item left from this session's own list:

1. **RunPod's catalog carries a bookable GPU type whose id is the literal string `unknown`**, priced
   and returned by `gpuTypes` like any other. U44 leaves it on the CUDA baseline rather than widening
   its vendor blocklist, so it survives filtering and can be booked. Nothing anywhere decides what
   that should mean. Cheap to investigate ($0.00 — it is a catalog read), and the honest options are
   the same three as always: refuse it, treat it as non-CUDA, or leave it and say why.
2. **U45/U46**, if serverless is ever wanted for real — both rows carry the full schema probed at
   $0.00, and both are contained behind the validation refusal, so neither can bite anyone silently.
   The protocol gap is the real work, not the mutations.

**The method that keeps paying**, now five times over: U36's pool prices, U43's catalog ids, U44's
vendor list, U46's schema and U48's `InstanceTypeInfo._fields` were ALL free reads, and each one
turned a guess into a fact before a line was written. The only thing this session genuinely had to
spend on was proving realized == advertised, which no read could establish. Probe first.

**And the recurring failure it guards against**, four times in three days: a guard's first cut
over-blocking something correct — U43 calling a real out-of-stock GPU a typo, U44 nearly dropping the
CUDA-capable Tesla V100s, U46's check nearly refusing Modal for a word on a different field, U48's
sentinel nearly colliding with a test fixture's impossible value. Every one was caught by asking
"what does this wrongly exclude?" *before* shipping, and every one would have broken a working config.
