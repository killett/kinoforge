...
though the underlying item stands. U37 — filed for the FlashVSR cu128 mismatch and CORRECTED the same
hour: those cfgs pin no torch at all and U33 cannot touch them; the real residual is MODAL, whose
`python:3.13-slim` base genuinely downloads the wheel U33 changed, on 19 of the 20 torch-pinning
cfgs still unproven),
and
**U13 is NOT
...
**One cheap MODAL diffusers run, capturing `torch.__version__` the way U34's arms did** — the RIFE
60fps interpolate cfg is the cheapest that pins torch. That is U37, and it is what makes U33 proven
on more than one of the 20 cfgs it changes. The live debts after that are **U24's** cheap 1.3B swap
group (below, unchanged) and **U35**, which is harness-only.
