...
| U31 | OPEN — filed 2026-09-10, DEMONSTRATED at $0.00, deliberately not fixed here | **A DAEMON `ConcurrentPool` worker parked mid-work-item still blocks interpreter exit, and the class that exists to prevent that says otherwise in its own docstring.** `_DaemonThreadPoolExecutor` (`core/pool.py:28`) flips `daemon=True` before `start()` and its docstring concludes "on ungraceful exit the workers now die with the process instead of blocking pytest's interpreter shutdown". That holds only while a worker is IDLE. Its `_adjust_thread_count` also registers each worker in `concurrent.futures.thread._threads_queues`, and `_python_exit` — installed through `threading._register_atexit`, so it runs BEFORE `threading._shutdown` — puts a sentinel on every registered queue and then **joins every registered thread unboundedly, daemon or not**. A worker blocked inside its current work item never reaches the sentinel, so the join never returns. **Demonstrated**, not reasoned: `pixi run python tools/diagnose_u13_exit_holder.py pool-worker` hangs and self-dumps `_python_exit` → `join` on the main thread with the daemon worker parked in its item (`8bed6296`). **Why it matters beyond the docstring:** `generate_clip.py:157` submits the backend job to this pool and its own comment at `:86` describes "a worker parked in a backend poll loop" — so on `generate` / `batch`, an ungraceful exit while a poll loop is in flight hangs the CLI at exit with no traceback after the one that caused it. That is **U13's symptom on a different command**, which is why this was filed rather than folded into U13: U13's own path (`upscale`) submits nothing to the pool, so this is NOT U13's holder. **Not fixed here on purpose** — the honest fixes (bound the join, or make the poll loops interruptible at exit) change process-shutdown semantics for every command, which is exactly the bar U13's entry says is past a one-guard change. What IS cheap and owed: the docstring claim is false as written and should not be left standing |
