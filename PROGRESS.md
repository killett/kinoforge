...
## Active workstream
**C33 follow-ups (filed 2026-06-16, see C33 entry in §Known limitations for full evidence).** Priority order: ~~**(h)**~~ — **CLOSED 2026-06-16** by `6dd3442` + `6f9104e` ($0.06 selfterm-disabled probe ran for 14.1 min wall, 27 EXIT-trap uploads, 32.5s avg cycle vs Q4 baseline 29 uploads / ~31s cycle — statistically identical; H_h **REFUTED**. Selfterm is NOT the cause of the T+15s bash-exit. Evidence: `tests/live/_c33_probe_h_evidence.json`). **(j) new cheapest unblocker** — single $0.005 image-only probe: spin a pod with `bash -c 'echo START; sleep 30; echo STILL_HERE; sleep 90; echo END'` as `dockerArgs` on `runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04` (no provision script, no selfterm, no aws/git/pip). If THIS bash exits at T+15s, the image entrypoint is killing it; if it survives 120s, the kill is triggered by something the provision script does in its first 15s (apt? pip? wget?). **(i) blocks C25 production-heartbeat enable** — Wan workloads currently can't cold-boot regardless of whether `podEditJob` restarts containers, so C25 must wait pending (j) + follow-on isolation. **(f) trivial** — fix the wrong "restart-on-failure" warning string in `src/kinoforge/providers/runpod/__init__.py` (Q9 proved RunPod is restart-always). **(a) small** — refine `classify_run` negative-uptime heuristic in `src/kinoforge/diagnostics/c30_probe.py` to require corroborating signal (S3 trap-fire or multi-sample pattern). **(d) small** — add a unit test asserting the codebase never reads top-level `Pod.uptimeSeconds`. **(g) larger** — design `diagnostic_mode: "trace"` cfg opt-in that cleanly bundles the Q5–Q8 bisection instrumentation (`set -x` + `PS4` timestamps + `ps`/`p.sh`/`selfterm.log` dumps + `stdbuf` flushing) so future restart-loop debugging doesn't require re-implementing the throwaway patches. 12 local commits ahead of `origin/main`; not pushed.
