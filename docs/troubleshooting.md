...
| `gcp_status: export-not-ready` from `tools/quota_burn.py snapshot` | BigQuery billing export enabled less than 24h ago; first table not yet landed | Wait for the first table (6–24h lag). `_do_snapshot` returns partial data with this status in the meantime. `kinoforge cost` never reads BigQuery and is unaffected. See [../PROGRESS.md](../PROGRESS.md). |
