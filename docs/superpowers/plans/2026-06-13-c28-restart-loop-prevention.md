...
**Goal:** S3 bucket `<DIAG_BUCKET>` exists in `us-west-2` with 7-day lifecycle; `kinoforge-ci` self-grants a scoped `PutObject`-only policy.
...
**Acceptance Criteria:**
- [ ] Bucket `<DIAG_BUCKET>` exists in `us-west-2`.
- [ ] Bucket has lifecycle rule deleting objects under `boot-logs/` older than 7 days.
- [ ] IAM policy `kinoforge-c28-diag-put` exists with statement: `Action: s3:PutObject`, `Resource: arn:aws:s3:::<DIAG_BUCKET>/boot-logs/*`.
...
Expected: `OK: bucket=<DIAG_BUCKET> policy=kinoforge-c28-diag-put user=kinoforge-ci`.
...
Find the orchestrator path that builds `InstanceSpec` (search: `rg -n 'InstanceSpec(' src/kinoforge/core/orchestrator.py`). At the construction site, when `cfg.diagnostic_mode == True`, pass `diagnostic_env={...}` populated from `os.environ` keys: `KINOFORGE_DIAG_BUCKET` (default `"<DIAG_BUCKET>"`), `KINOFORGE_DIAG_PREFIX` (default `f"boot-logs/{run_id}"`), `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION` (default `"us-west-2"`). Add a separate orchestrator unit test mirroring the `test_diagnostic_env_*` shape but at the orchestrator boundary.
...
def test_c28_phase_a_diagnostic_capture_live() -> None:
    ...
    for attempt in range(1, _MAX_BOOT_ATTEMPTS + 1):
        ...
        body = s3.get_object(Bucket="<DIAG_BUCKET>", Key=obj_key)["Body"].read().decode()
