...
Three statements:
  ...
  - NovaReelOutputBucket: S3 read/write on <S3_OUTPUT_BUCKET>
...
**Goal:** Use the AWS CLI from inside the container to (1) create the S3 output bucket `<S3_OUTPUT_BUCKET>` in `us-east-1` if it doesn't exist, (2) attach the inline policy from Task 3 to the existing `kinoforge-ci` IAM user. Both operations are reversible (delete bucket, delete inline policy) — explicitly note this in the commit body.

**Files:**
- Modify: `docs/CLOUD-CREDS.md` — add the new policy + bucket rows under the AWS section

**Acceptance Criteria:**
- [ ] S3 bucket `<S3_OUTPUT_BUCKET>` exists in `us-east-1`
...
**Verify:**
```
aws s3api head-bucket --bucket <S3_OUTPUT_BUCKET> --region us-east-1 && \
...
```markdown
| `<S3_OUTPUT_BUCKET>` | Nova Reel async-invoke output prefix | `us-east-1` | 2026-06-07 | created by Layer 3 Task 4 |
```
