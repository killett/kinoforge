...
| `grace_after_session_too_tight` | STATIC | WARN | `cfg.compute is not None` | Asserts `compute.lifecycle.grace_after_session_s >= 600` (10-min minimum for operator-typing-pace between two `kinoforge generate` invocations). Caught 2026-06-18 Wan 14B smoke: 300-s default + 5-min operator gap between cmd 1 finish and cmd 2 start → cmd 2's classify chain saw sentinel_age=299s past 300s grace → ORPHAN_REAP → cold create instead of warm-attach. Default already bumped to 1800 (interfaces.py:88, commit pending); this check guards against operators explicitly setting a value below the floor. | NO — operator explicit choice; warn only. |
