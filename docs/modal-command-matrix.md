...
the flag combinations that matter, run 2026-09-05/06 against the Modal provider: **33 PASS, 15
FAIL, 9 EXPECTED-REFUSAL, 0 PENDING**, for **$3.25 of the $20 budget** (live spend from later
re-proof sessions is tracked separately in `PROGRESS.md`, not folded into this figure). Five
defects were fixed in-session; the rest were filed in `PROGRESS.md`, where the list has since grown
to **U1–U29** as the fixes exposed their own boundaries. Verdict counts here are the current ones:
T2-03 moved FAIL → PASS when U14 was fixed and live-proven (2026-09-07), and T1-29 moved
FAIL → PASS when U11 was fixed and live-re-proven (2026-09-09, `fix/ephemeral-and-recovery-gaps`).
...
(**1920×1920** and **1080×1080**) for **$0.64**. T2-01's row now records PASS with the original
failure preserved in Notes. **T2-03's did not, until 2026-09-07:** the same re-run reproduced the warm-attach miss
(**U14**), cold-booting a second $2.50/hr A100 while the first sat idle at 0% GPU, so its verdict was
**FAIL**. **U14 is now fixed (`49394b1d`) and live-proven — a second upscale attaches to the warm pod
with no `✓ App deployed` — and T2-03 is reclassified PASS.** RunPod and SkyPilot are protected by the *same*
pin — it is one shared line — but neither was re-proven live, so their status is **inferred, not
demonstrated**.

**Follow-up fixes applied after the campaign closed**, on top of the five in-session ones:
...
| Item | Fix | Live proof | Cost |
|---|---|---|---|
| **U7** — `provision` booked a pod no kinoforge command could see | `8191bd1b` — the pre-create `kf_launch_phase=launching` row `deploy` already writes, plus a refusal for an id-less create and one for a capability key already recorded | Three SIGKILLed provisions, including one 1.0 s INSIDE `create_instance`; every kill left a row naming the app | $0.00 (no container started) |
| **U8** — an `--ephemeral` run was invisible to every state file until it finished | `9d34d008` + `8403a71c` — the index row is reserved BEFORE the orchestrator is entered, and `created_at_local` now means launch time | Index row readable 2.5 s into a live run naming app `kinoforge-eph-b8aa04a1`; pod still reapable from a fresh process after SIGKILL | ~$0.06 |
| **U9** — nothing automatically reaped an idle ephemeral pod | `e582bd0f` — an age+idle `ORPHAN_REAP` verdict, plus `sweeper_thresholds_from_cfg` as the daemon's single threshold source (that half is **U20**) | The daemon reaped `eph-64dac102` at `age=119s idle on probe gpu_util=0.0% cpu=0.0%` | ~$0.03 |
| **U15** — `--attach-pod` refused a healthy pod whose endpoint only the ledger held | `ccd4c5e7` — the recorded endpoint map is seeded onto the instance before `ensure_endpoints` | A fresh process attached to warm pod `run-20260907-001839` in 38 s with no `✓ App deployed` | ~$0.07 |
| **U14** — the warm-attach matcher cold-booted a duplicate A100 | `49394b1d`, review-corrected in `b00a53d1` — `_cfg_want_stages` delegates to `cfg.capability_key().stages` | A second upscale logged `warm-reuse: attached to upscale-20260907-011323` and published a 1080² clip on the same pod | $0.12 proof, $0.54 to reproduce and diagnose |
...
2026-09-08 (a live re-proof means racing the same ~1 s mid-deploy window and is a separate
follow-up), ~~**U18** (`reap`
short-circuits on an empty ledger, so `--include-orphans` is unreachable for exactly the
`--ephemeral` case)~~ — **U18 is FIXED in `71582382` + `abe0c21e` and LIVE-PROVEN 2026-09-09**
(one-shot `reap` against a genuinely empty ledger classified the live orphan `eph-63cda383`
instead of printing "nothing to do"),
**U19** (no in-pod capability term for interpolation, open), ~~**U21** (`provision` has no
destroy-on-error path)~~ — **U21 is FIXED (offline-proven only) in `9e268c18`**,
`fix/ephemeral-and-recovery-gaps`, 2026-09-09 — a stated behaviour change on a money path
(`provision` now destroys the pod on a post-create failure where it previously left it running),
not yet live re-proven — and **U22** (the ephemeral orphan reap acts on one probe sample, open).
**U20**
was filed retroactively and is fixed: the truncated sweeper thresholds dict made `STALL_REAP` and
`RESTART_LOOP_REAP` unreachable from `sweeper start` for every pod, not only ephemeral ones.
...
- **FlashVSR upscale on A100-80GB — after the `av<18` pin.** 4x to 1920×1920 (T2-01) and the
  ...
  false colour. Broken for the whole first pass; fixed in `82ad084b` and re-proven live. (T2-03's
  cell was a FAIL for the warm-attach miss its re-run reproduced — not for the upscale, which
  worked; that miss is **U14**, fixed and live-proven 2026-09-07, and the cell is now a **PASS**.)
...
| ~~**`--attach-pod` cannot attach to a healthy pod whose endpoint the ledger is holding**~~ — the merge read `tags`, never `endpoints`; **FIXED offline in `ccd4c5e7`** (the ledger's `endpoints` are now seeded onto the instance before `ensure_endpoints`, live-wins-on-collision), **live-proven 2026-09-07 on the `generate --attach-pod` path** ($0.07, A10): a fresh process attached to warm pod `run-20260907-001839` in 38 s with no `✓ App deployed` line. T2-02's own `upscale` cell has still not been re-run | T2-02 | **U15** fixed, live-proven |
| ~~**The warm-attach matcher cold-boots a duplicate A100 beside an idle pod carrying the same capability key**~~ — the T14 `/health` stage gate demanded `t2v` from an `upscale_only` pod that by design never loads a Wan pipeline, so an upscale-only pod was structurally unattachable; **FIXED in `49394b1d`** (`_cfg_want_stages` now delegates to `cfg.capability_key().stages`) and **live-proven 2026-09-07** ($0.12, A100-80GB): a second upscale logged `warm-reuse: attached to upscale-20260907-011323` with no `✓ App deployed` | T2-03 | **U14** fixed, live-proven |
| **`kinoforge interpolate` cannot participate in warm reuse at all** — the in-pod capability vocabulary (`_capability_for_model`) has no term for interpolation, so a `rife-*` entry never reaches `/health`'s `capabilities[]`; the T14 stage gate is therefore carved off for interpolate cfgs and a half-failed RIFE pod looks identical to a healthy one. Not a duplicate boot today — the carve-out is what prevents one — but the upscale path's protection is simply absent here. Found reviewing U14's fix round 1 | T2-06, T2-07 | **U19** |
| **The deploy-first lifecycle.** `deploy` crashes before booking anything; `provision` ~~books a pod no kinoforge command can see or destroy~~ — the `provision` half is **FIXED** in `8191bd1b` (durable pre-launch row, id-less create refused, double-book refused) and **live-proven in part 2026-09-07**: three SIGKILLed provisions all left a row naming the app, but at the time `destroy --id` could not reap one killed mid-deploy — that gap is **U17**, now **FIXED (offline-proven only)** in `8069f376`+`45f4254c` (`fix/ephemeral-and-recovery-gaps`, 2026-09-08); a live re-proof means racing the same ~1 s mid-deploy window and is a separate follow-up | T1-19, T1-20 | **U6**; **U7** fixed, live-proven in part; **U17** fixed, offline-proven only |
...
| ~~**Nothing automatically reaps an idle ephemeral pod**~~ — the sweeper cannot see the index; `reap` sees it but can only ever say LIVE. **FIXED in `e582bd0f`, live-proven 2026-09-07**: the daemon reaped `eph-64dac102` at `age=119s idle on probe gpu_util=0.0% cpu=0.0%`. Two residuals: a row abandoned MID-BOOT has `endpoints: {}` so the util probe returns nulls and the pod stays LIVE, and ~~`kinoforge reap` still never reaches `sweep()` on an empty ledger~~ (**U18** — fixed in `71582382` + `abe0c21e`, **live-proven 2026-09-09**: `reap` on an empty ledger classified the live orphan `eph-63cda383`; the `--apply --include-orphans` promotion still cannot fire for a row whose `endpoints` is `{}`, which is the same starvation path as the mid-boot residual) | T1-24, T1-26 | **U9** fixed, live-proven; **U18** fixed, live-proven |
| ~~**`grid --ephemeral` is accepted and silently dropped**, publishing the run id and timestamp to the provider~~ — **WORSE as of `0dfe90a9`+`7ee50a04`+`fdc4f388`, live-disproven 2026-09-09**: the flag is now forwarded, but appended AFTER the `generate` subcommand, where argparse rejects it (`--ephemeral` lives on the root parser only). Every cell dies in ~0.75 s with `kinoforge: error: unrecognized arguments: --ephemeral`, so `grid --ephemeral` no longer runs at all. **U11 re-opened** — then **FIXED in `b9b4fcd6` and LIVE-RE-PROVEN 2026-09-09** ($0.08): `_build_generate_cmd` emits the flag in ROOT position, and the same commit closed a second, silent door (`p_grid` re-declared `--ephemeral` with an implicit `default=False`, so `kinoforge --ephemeral grid …` ran NON-ephemerally at exit 0). The A2 re-run published `kinoforge-eph-2edbbd45` / `kinoforge-eph-c89c0418` — opaque 8-hex, no run id, no timestamp | T1-29 (re-run 2026-09-09, then re-proven 2026-09-09) | **U11** fixed, live-proven |
| **`--vault` cannot supply a prompt**, and the empty-prompt path fails *after* the pod is billing | T1-12 | **U5** |
| **The warm-attach matcher is provider-blind** — and separately ~~misses its own live pods~~ (that half is **U14**, **fixed and live-proven 2026-09-07 in `49394b1d`**; the provider-blindness of **U1** is untouched) | T0-07, T0-08, T2-03 | **U1**; **U14** fixed, live-proven |
| **`batch --dry-run-swap` never reads the manifest** | T0-08 | **U2** |
| ~~**An `--ephemeral` run is invisible to every state file until it has finished**~~ — **FIXED on Modal in `9d34d008` + `8403a71c`, live-proven 2026-09-07**: the index row was readable 2.5 s into a live run, naming app `kinoforge-eph-b8aa04a1` with a launch-time stamp, and the pod stayed reapable after the controller was SIGKILLed. **PARTIAL on RunPod (U16).** `kinoforge list` still shows no ephemeral pod at all | T1-23 | **U8** fixed on Modal, live-proven; **U16** |
...
Three of these cost real money when they bit: `provision` leaked **$0.13** on an untracked A10
(U7), the warm-attach miss put **two $2.50/hr A100s** on the clock at once (U14), and an
`--ephemeral` pod whose controller died billed until a human typed `destroy` (U9). **All three are
now fixed and live-proven** — see the 2026-09-07 update at the top. Recovery from a mid-deploy
kill (U17) and `provision` leaving a pod up on a post-create failure (U21) — both once "still
open" here — are **also now fixed**, in `fix/ephemeral-and-recovery-gaps` (`8069f376`+`45f4254c`
for U17, `9e268c18` for U21, 2026-09-08/09), but **offline-proven only**: a live re-proof of
either means racing a narrow create/deploy timing window and is a separate follow-up, not yet
run. What genuinely remains untouched is an ephemeral pod abandoned MID-BOOT, which carries
`endpoints: {}` so the reaper's probe cannot see it (U3's blast radius).
...
**Four of the fifteen FAILs are not code defects, and no item is filed for them.** T1-14a/b/c
...
one grid cell that rendered a band across its lower third. The campaign plan's Global Constraints block
(`docs/superpowers/plans/2026-09-05-modal-command-matrix.md`) makes visual QA an
unconditional precondition — "every output mp4 gets 5 frames via `ffmpeg_frames_by_count`,
...
| T0-12 | `kinoforge reap` / `kinoforge reap --format json` | — | PASS | $0.00 | `logs/T0-12a.log`, `logs/T0-12b.log` | FAILed on first pass: `--format json` printed the human line `reap: ledger empty (nothing to do)` rather than JSON, breaking any `jq` consumer on the empty ledger. Fixed in `3c7822b8`; the re-run emits `{"type": "header", "entries": 0}` under `--format json`. As quoted, this row is a historical snapshot: at the time of the re-run the default human format kept the sentence `reap: ledger empty (nothing to do)` verbatim. **U18** (`71582382` + `abe0c21e`, `fix/ephemeral-and-recovery-gaps`) later changed that sentence to gate on the union of the ledger AND the `EphemeralIndex`, so at HEAD the same empty-state invocation instead prints `reap: ledger and ephemeral index both empty (nothing to do)` (or, under `--id`, `reap: '<id>' not found in ledger or ephemeral index (nothing to do)`) — see T1-07's row and U18's `PROGRESS.md` entry. Both exit 0 |
...
| T1-07 | `kinoforge reap` and `reap --format json --id ID` | WAN13B | PASS | $0.00 | `logs/T1-07a.log`, `logs/T1-07b.log` | Both exit 0 and classify the pod LIVE without destroying it: `HEARTBEAT_SUBSTRATE_MISSING`, `1 entries classified — pass --apply to act on default policy`. `--format json --id` emits `{"type": "header", "entries": 1}` then the verdict object with `provider: modal` (the T0-12 fix holds on a non-empty ledger). Pod confirmed still alive afterwards. **Defect as run, now fixed:** the human table's verdict column was too narrow for the longest verdict, so it rendered `HEARTBEAT_SUBSTRATE_MISSINGrun-20260906-010255` with no separator. It was excused here as "cosmetic"; **this document has no cosmetic exemption** — the Verdict rule says a cell whose notes describe a real defect is a FAIL, full stop. **Fixed in `9ae52274`**: the field is 28 wide in both header and row, red/green with a test that derives the required width from the `Verdict` enum so a longer future member fails it too. The row keeps PASS on the T2-01 precedent (cause fixed *and* re-proven), and the re-proof is honest here even without a re-run: the table is a pure local rendering with no provider in the path, so a deterministic offline test **demonstrates** the fix rather than inferring it — unlike T2-05b, whose row stays FAIL precisely because its fix could only be inferred. See follow-up F6 |
...
### Tier 1a utilisation readings (the health signal, per the plan's Global Constraints)
...
| T1-20 | `kinoforge provision -c CFG` | WAN13B | FAIL | $0.13 | `logs/T1-20.log` | Exit code not observed: the run was killed at ~7 min once `modal app list` revealed a live untracked container (see below), so the code is unrecorded rather than non-zero. Two defects. **(a)** It printed `provisioned: instance=''` — an **empty instance id**. `ModalProvider.create_instance` returned an instance whose id never reached the print, and the app was deployed as `kinoforge-` (bare prefix, no suffix) at `https://<operator>--kinoforge--build-modal-app--locals--server.modal.run`. **(b)** `_cmd_provision` writes **nothing to the ledger** — no provisional row, no real row — so `kinoforge list` showed `No instances recorded in ledger.` while `modal app list` showed `ap-U8nQQQVqDECy3iTqpoKQ7j` `deployed` with **1 running task**, i.e. a live A10 billing at $1.10/hr that no kinoforge command could name or destroy. Recovered manually with `modal app stop -y ap-U8nQQQVqDECy3iTqpoKQ7j`; confirmed `stopped`/0 tasks. Alive 01:45:21 → 01:52 ≈ 7 min ≈ **$0.13 of unrecoverable spend**, the single most expensive defect in the campaign so far. Not a "re-provision of the existing instance" and not an "already provisioned" refusal — it is an unconditional second create with no ledger record. See follow-up F9 / **U7**. **FIX (`8191bd1b`, 2026-09-06):** `_cmd_provision` now reuses `deploy`'s mechanism — `_record_provisional_row` writes the `kf_launch_phase=launching` row BEFORE `create_instance`, ruling C1 governs the failure split (only a provably-nothing-booked error forgets the row), and the success path records the real row then collapses the provisional one. An empty instance id is a non-zero exit naming the run id, with the launching row left as the only handle; a capability key that already has a recorded instance is refused by name instead of booked twice. The hand-rolled `InstanceSpec` is gone in favour of `build_instance_spec`, so the row carries `kinoforge_key` and the resource is named `kinoforge-provision-*`. Ten tests in `tests/cli/test_cmd_provision.py`. **LIVE RE-PROVEN 2026-09-07 (Task 5 of the money-leaks plan, $0.00 — no GPU container ever started).** Three `provision` runs SIGKILLed as a process group with the ledger polled at 50 ms. Modal deploys the already-built image in **~1.4 s**, so the create window is seconds wide. (a) Kill 4.4 s after the create returned: the real row survived, a fresh `kinoforge list` named `kinoforge-provision-20260907-000607-de8004` with its endpoint, `destroy --id` reaped it, app went `stopped`. (b) Kill 1.17 s in, pre-create: the `kf_launch_phase=launching` row was on disk before Modal had the request. (c) Kill 1.0 s INSIDE `create_instance`: only the launching row existed, Modal had already committed `kinoforge-kinoforge-provision-20260907-000832-70d75b` (`initializing...`, `tasks=0`), and `kinoforge list` named it from that row alone. **So the durability half is proven at all three kill points against a pre-fix baseline that recorded nothing at any point.** **What is NOT fixed:** in case (c) `kinoforge destroy --id` failed — `modal app stop <name>` errors `No App with name … found` for an app that never finished deploying, and the `CalledProcessError` escapes as a traceback. Only `modal app stop <app_id>` worked. Filed as **U17**, since **FIXED (offline-proven only)** in `8069f376`+`45f4254c` (`fix/ephemeral-and-recovery-gaps`, 2026-09-08) — `destroy --id` now resolves the Modal `app_id` via a listing lookup before falling back to the by-name stop, but this specific mid-deploy kill window has not been re-proven live. The cell's own stated recovery (a raw `modal app stop`) therefore survives, as originally run, for the one window U7's pre-create row exists for. **Also filed 2026-09-07: `provision` has no destroy-on-error path** — a create that succeeds followed by a readiness poll or weight download that fails leaves the pod running with nothing to tear it down, and the readiness loop has no timeout (**U21**, since **FIXED (offline-proven only)** in `9e268c18`, same branch, 2026-09-09 — destroy-on-error now wraps the post-create tail and the readiness poll is bounded by `boot_timeout_s`; this is a stated behaviour change on a money path, and it too has not been live re-proven). **The verdict stays FAIL: the original defects are fixed (U7 live-proven in part, U17/U21 offline-proven only), but this cell has never been re-run end to end.** The Verdict column carries a bare token per the reading note above; the qualifier that used to live there is this sentence |
...
**The tier's stated goal is not met** (as measured; the `provision` half has since been fixed in
`8191bd1b` and live-proven in part on 2026-09-07 — the durable row held at all three kill points;
reaping a mid-deploy app (**U17**) and destroy-on-error (**U21**) are also now fixed, offline-proven
only, in `fix/ephemeral-and-recovery-gaps`). `deploy` → `provision` → `generate --instance-id` →
`reap --apply` did not work on Modal: the first step crashes without booking anything (T1-19,
**U6**) and the second booked a pod that no kinoforge command could see or destroy (T1-20, **U7**).
...
| T1-29 | `kinoforge grid … --ephemeral` with `--out grid-eph.mp4` | WAN13B | PASS | $0.05 | `logs/T1-29.log`, `logs/T1-29-proof-list.log`, `sheetG.png` | **Verdict reclassified FAIL → PASS on 2026-09-09** (T2-01/T2-03/T1-04 precedent: the cause was fixed and the cell was re-proven live in `b9b4fcd6` + the Task 6 A2 re-run, so the verdict records the fixed behaviour, not the as-run failure or the intervening regression). As originally run: exit **0** and the composed mp4 is correct (960x480/33f, both cells captioned, frame-QA **PASS** on both — cell A a strong backlit maroon-dress silhouette over a yellow flower field, cell B a clean teal-dress render with glowing pollen; no trace of T1-28's cell-B band). The ledger and the ephemeral index are both empty afterwards. **But `--ephemeral` is silently ignored.** The flag's help text says "pass-through to each underlying generate"; `_cmd_grid` (`src/kinoforge/cli/_commands.py:3612`) does `del ctx`, never reads `args.ephemeral`, and calls `run_grid(spec=…, output_dir=…, max_parallel_groups=…, out_path=…)` — the string `ephemeral` does not appear anywhere in `src/kinoforge/core/grid/`. The proof is on Modal's side: the two apps are named **`kinoforge-grid_20260906-022232_a61d55b2__cell0` / `__cell1`**, not the opaque `kinoforge-eph-<8hex>` shape that EM1's STRICT_POLICY requires, so the run id, the local timestamp and the cell index were all published to the provider. The clean ledger is an artefact of the `no_reuse=True` teardown described at T1-28, **not** of ephemeral mode — a plain non-ephemeral grid leaves exactly the same clean ledger. A user asking for ephemeral got a normal run with an identity-leaking app name and no warning. See follow-up F13 / **U11**. **RE-RUN 2026-09-09 (`fix/ephemeral-and-recovery-gaps` Task 6, cell A2, $0.00) — the U11 fix does NOT hold live, and the command is now WORSE than when this cell was first run.** `pixi run -e live-modal kinoforge grid --spec /home/claudeuser/kinoforge-proof/grid.yaml --out … --max-parallel-groups 1 --ephemeral` (1x2, both cells the standard-prompt Wan 2.1 1.3B cfg, spec outside the repo, no `lora_swap`) failed cell 0 **0.75 s** in — `2026-09-09 00:29:38,858 WARNING kinoforge.core.grid.executor grid cell 0 failed; aborting remaining cells in group`, `[grid summary] status=partial` (exit 2) — and cell 1 never dispatched. **No Modal app was created at all**, verified from a fresh process (`modal app list` held only the stopped `kinoforge-eph-63cda383` from cell A1), so the cell cost nothing. Root cause, from the child's own captured stderr (`output/_grid_grid_20260909-002938_069bb8a6/cell_0.stderr.txt`): `_build_generate_cmd` APPENDS `--ephemeral` after the `generate` subcommand, but `--ephemeral` is declared only on the ROOT parser (`cli/_main.py:382`) and on `p_grid` (`:919`) — never on the `generate` subparser — so the child exits with `kinoforge: error: unrecognized arguments: --ephemeral`. Confirmed against `_build_parser()` directly: `['generate', …, '--ephemeral']` leaves `['--ephemeral']` unparsed with `ephemeral=False`, while `['--ephemeral', 'generate', …]` parses `ephemeral=True`. The opaque `kinoforge-eph-<8hex>` naming this cell was re-run to observe was captured instead via `batch --ephemeral` in the same session (`kinoforge-eph-63cda383`), so the NAMING contract is live-proven — the grid pass-through is not. Not fixed in Task 6 by design (the brief forbids retrofitting a failing proof); the remedy is a one-line argv reorder. **RE-PROVEN 2026-09-09 (`b9b4fcd6` + Task 6 cell A2 re-run, $0.08) — the fix HOLDS live and this cell's original defect is closed.** `pixi run -e live-modal kinoforge --ephemeral grid --spec /home/claudeuser/kinoforge-proof/grid.yaml --out … --max-parallel-groups 1` (same 1x2 fixture, `budget_cap_usd` lowered to 0.30) ran both cells to completion — `[grid summary] composed mp4 → …`, status `full`, exit 0 — and the cell child observed live in `ps` was `python -m kinoforge --ephemeral generate … --no-reuse`, with the flag in ROOT position and the process alive rather than dead in 0.75 s. **The naming contract this cell exists to observe is now proven through `grid` itself:** `modal app list` from fresh processes showed `kinoforge-eph-2edbbd45` (01:08:01 → 01:10:29) and `kinoforge-eph-c89c0418` (01:10:32 → 01:12:22) — opaque 8-hex, no run id (the grid ids were `grid_20260909-010759_12fe6ccc__cell0` / `__cell1`), no local timestamp — where this cell originally published `kinoforge-grid_20260906-022232_a61d55b2__cell0`. The ROOT flag position was used deliberately (it is the door `p_grid` had been silently dropping); the SUB position was closed at $0.00 with the root-only `--debug-show-secrets` mutual exclusion — both positions → exit 2 mutex error, the no-`--ephemeral` control → exit 0 `[grid dry-run] 2 cells` — so both doors are proven without a second pair of pods. Frame-QA PASS on both 480² cells and the composed 960x480 grid, with ⚠️ soft flags (stylised rather than photoreal, blown highlights) and no false colour or seam band. Spend 258 s of A10 @ $1.10/hr = $0.08. Teardown from fresh processes at 01:14:10: both `kinoforge list` lines, 0 index rows, empty ledger, all apps `state=stopped tasks=0`, `modal container list` → none. **⚠️ One gap: no `gpuUtilPercent` reading was obtainable** — `grid` writes each cell's captured stderr only at cell end, so the `.modal.run` URL A1's util probe scraped from a live log does not exist for a grid cell, and **U3** blocks resolving it otherwise; the 75 s poll degraded to `modal app list` `state`/`tasks` (never `est_spend`) |

**Tier 1d tally (as run, 2026-09-06):** 0 PASS, **2 FAIL** (T1-28, T1-29). **T1-29 was reclassified
PASS on 2026-09-09** once U11 was fixed and live-re-proven, so the tier's current tally is **1
PASS, 1 FAIL** (T1-28 only) — the counts below describe the as-run tier, kept for the cost/timing
narrative. Actual spend **$0.10** — four A10 apps, each
alive about a minute (02:11-02:13 and 02:22-02:25). The two failures (as run) were unrelated in
kind, and the
distinction matters: **T1-29 was a code defect** (`--ephemeral` accepted and dropped, U11 — since
fixed), while
...
| T2-02 | `kinoforge upscale -c VSRX4 --video FIX --scale 1080p --attach-pod ID` | VSRX4 | FAIL | $0.00 | `logs/T2-02.log`, `logs/T2-02-rerun.log` | **As run: EXPECTED-REFUSAL. Re-run with the blocking flag removed: FAIL, and it exposed a new defect.** As written the cell exits 2 at argument validation, before touching the pod: `error: --scale 1080p deferred to a later session; use --scale Nx for v1` — a deliberate guard, since the height-target path is a **cfg** key (`upscale.scale: 1080p`, the VSR1080 cfg), not a CLI flag. Correct, but it fires before attach, so the cell never reached `--attach-pod` and warm-attach on the `upscale` path was left unproven. **The 2026-09-06 09:xx re-run closed that gap and found it broken.** Dropping only the refused flag and pointing the VSR1080 cfg at T2-01's live warm pod — `kinoforge upscale -c VSR1080 --video FIX --attach-pod upscale-20260906-093919` — exits **1** with `pod upscale-20260906-093919 has no endpoints after ledger tag merge (ledger tag keys=['kinoforge_engine', 'kinoforge_key', 'mode']); cannot --attach-pod.` The claim is false: the ledger entry read at that moment held `endpoints: {"8000": "https://<operator>--kinoforge-upscale-20260906-093919-build-mod-bcb59b.modal.run"}`, and that exact URL answered `GET /util` seconds earlier with `gpu_util_percent=0.0, uptime_seconds=118`. So **`--attach-pod` cannot attach to a healthy Modal pod whose endpoint the ledger is holding** — the merge reads `tags` and never the `endpoints` field. Same family as **U3** (endpoint in the ledger, no read path consults it) but a distinct site on the write-path side. $0.00 — refused before any pod work. Filed as **U15**. **Fixed 2026-09-06 in `ccd4c5e7`, live-proven 2026-09-07 on the sibling `generate --attach-pod` path; THIS cell (the `upscale` path on an A100) has still not been re-run, which is why the verdict stays FAIL.** The filed hypothesis held on inspection: `_resolve_attach_pod` merged `entry["tags"]` onto the live instance and then called `provider.ensure_endpoints`, never reading the entry's sibling `endpoints` field, which on Modal leaves the provider with nothing (`get_instance` builds the `Instance` from `modal app list`, which carries no URL, and `ModalProvider.endpoints` falls back to a per-process `_deployments` dict a fresh process never populated). The fix seeds the recorded endpoint map onto the instance before the `ensure_endpoints` call, with the tag merge's own precedence (live wins on collision), and rewrites the refusal to cite the endpoint ports it actually checked instead of the tag keys. Red/green in `tests/cli/test_resolve_attach_pod.py` (4 tests, 4/4 RED then GREEN; `tests/cli` 421 passed). **Verdict stays FAIL until a live Modal re-run of THIS cell (the `upscale` path, VSR1080 cfg) attaches to a warm pod.** **The defect itself is live-proven fixed on the sibling `generate` path, 2026-09-07 (Task 5, A10, $0.07):** a warm pod `run-20260907-001839` was booted, then from a FRESH process (`ModalProvider._deployments` empty — the exact state that made the refusal fire) `kinoforge generate -c WAN13B --mode t2v --prompt PROMPT --attach-pod run-20260907-001839` **attached**: rc=0, 38 s, no `✓ App deployed` and no `✓ Created objects` anywhere in the log, no refusal. Frame QA PASS (480×480/33f/16fps/2.06 s). `_resolve_attach_pod` is shared by both commands, so the fix is exercised; the upscale cell's own A100 re-run is what is still owed. The U3 half of the family is **not** fixed by this: `status` and `pod lora ls` still read no ledger endpoints |
| T2-03 | `kinoforge upscale -c VSR1080 --video FIX` | VSR1080 | PASS | $0.08 + $0.22 re-run + $0.66 U14 diagnose/fix/re-proof | `logs/T2-03.log`, `logs/T2-03-rerun.log`, `sheetT2-03-rerun.png` | **Two findings as run, both negative; one is fixed and re-proven, the other reproduced verbatim.** **(a) It did not warm-attach — still true.** T2-01's pod was alive and idle at 0% GPU, and this cfg resolved to the **same capability key `7afe34198cc9`**, yet the matcher cold-booted a *second* A100 (`upscale-20260906-025335`), putting two $2.50/hr cards on the clock. **The post-fix re-run did exactly the same thing** — with `upscale-20260906-093919` warm and idle at 0% GPU, it cold-booted `upscale-20260906-100335` (`✓ App deployed in 1.700s`, image cached). So **U14 is reproducible, not a one-off**, and it is now the only defect standing between this cell and a single-pod run. **(b) The encode failure is fixed.** As run it died at the same writer (`UpscaleFailed: … Cannot change width after codec is open.`, `u-c11cfbd94d…`), no artifact. After the `av<18` pin (**`82ad084b`**, see T2-01) the re-run exits 0 and publishes `output/20260906-100840_upscaled_flashvsr_flashvsr-wan21-bfloat16_upscale.mp4`, ffprobe **1080×1080, 16/1 fps** — the height target resolves correctly (4x to 1920² then downscale to the 1080 height), matching §27. Frame-QA **PASS** against a source sheet: correct colour, no false-colour cast, coherent motion, sharper than the source at every sampled frame. Both pods destroyed at T2-04. **Verdict FAIL, on U14.** The 1080p height-target upscale itself is fixed and demonstrated, and that is what the See-also under §27 records — but this cell's own re-run reproduced an open, money-costing defect, and the rule above admits no subject-scoped exemption: scoping a verdict to "the cell's own subject" is the banned `PASS but` relocated into Notes, and it is the same shape as the `PASS ⚠️` that let T1-28's whole-clip band through three reviews. **Contrast T2-01**, which failed as run, had its cause fixed, and was re-proven live with nothing left standing (T0-02 / T0-12 / T1-04 precedent) — T2-03's re-run left **U14** standing, which is the whole difference. Reclassified PASS → FAIL on the 2026-09-06 review. **Reclassified FAIL → PASS on 2026-09-07: U14 is fixed and live-proven, so nothing this cell raised is still standing.** The diagnosis was one log line once Task 0 made the warm scan state its reason — `warm-reuse: scanned 1, 0 attachable (reasons: 1 stage-mismatch) — cold create`, captured live 00:56:11 against warm idle pod `upscale-20260907-004328`, whose `/health` at the same moment advertised `capabilities: ["upload","upscale"]`. The filed suspected site (`find_warm_attach_candidate` + the `EphemeralIndex` lookups) was **wrong** — neither is on the `upscale` path. Real cause: `_cfg_want_stages` re-derived the required stage tuple as `("t2v","upscale")` for any cfg with an `upscale:` block, while `Config.capability_key()` derives `("upscale",)` when `upscale_only` is set, so the T14 `/health` subset check could never pass for an upscale-only pod — it was structurally unattachable by its own cfg, and `upscale.scale` (`4x` vs `1080p`) was never the discriminator. **Fixed in `49394b1d`** (the helper now delegates to `cfg.capability_key().stages`; the gate still refuses a genuine t2v cfg an upscale-only pod). **Live re-proof 2026-09-07, $0.12:** the x4 cfg cold-booted `upscale-20260907-011323` (1920×1920/77f) and the VSR1080 cfg then logged `warm-reuse: attached to upscale-20260907-011323`, exited 0 and published 1080×1080 — **zero `✓ App deployed` / `✓ Created objects` lines** (grep count 0), GPU 100% mid-attach on the one pod. Frame QA PASS on both. Teardown proven from a new process (`kinoforge list` both lines + no non-stopped `kinoforge-*` app). **U14 closed.** Fix round 1 caught a self-inflicted side effect in review, disclosed here rather than buried: the first cut delegated to `capability_key().stages` wholesale, which also emits `"interpolate"`, so the shipped RIFE cfg went from bypassing the gate to being gated on a term no pod can advertise — this defect relocated to `kinoforge interpolate`. `"interpolate"` is now filtered back out and the underlying vocabulary gap is filed as **U19**; a sweep of every config in `examples/configs` now confirms the only remaining behaviour changes are the eight upscale-only cfgs correctly dropping their phantom `t2v` requirement |
...
it flipped to PASS on 2026-09-07 once U14 was fixed (`49394b1d`) and the attach proven live.
T2-02 flipped from EXPECTED-REFUSAL to FAIL because re-running it without the refused flag exposed
a real `--attach-pod` defect (**U15**).
...
defers it deliberately for RunPod/Modal — but the consequence is now measured: the campaign plan's own
Global Constraints block instructs the operator to get the pod URL from `kinoforge status`, and
that does not work. This task had to read `.kinoforge/_lifecycle/ledger.json` directly to poll `/util`.
...
**F6 — `reap`'s human table under-pads the verdict column. FIXED in `9ae52274`.** With the longest
verdict string the column ran into the id: `HEARTBEAT_SUBSTRATE_MISSINGrun-20260906-010255` (T1-07,
reproduced verbatim at T1-22). It was filed here as "cosmetic", **which is not a category this
document has** — the Verdict rule admits no cosmetic exemption, and "the JSON format is unaffected"
does not make the default human output parseable by eye at exactly the moment an operator is
deciding whether to reap. `HEARTBEAT_SUBSTRATE_MISSING` is 27 characters against an 18-wide field;
the field is now 28 in both the header and the row, red/green with a test that derives the required
width from the `Verdict` enum so a longer future member fails it too. **F6 closed.**
...
**F9 — FIXED in `8191bd1b` (offline; live re-run owed) — `kinoforge provision` books an instance that nothing records and nothing can destroy.**
...
id an error rather than an app name. Filed as **U7** — and implemented exactly that way in
`8191bd1b`: the same `_record_provisional_row` / ruling-C1 / collapse contract `deploy` uses, plus
an id-less-create refusal and a double-book refusal keyed on `kinoforge_key`.
...
**Resolved 2026-09-07.** The reason-logging landed first (`e0759f9f`, `1c2c4795`) and turned the
diagnosis into one line — `warm-reuse: scanned 1, 0 attachable (reasons: 1 stage-mismatch) —
cold create`. The reject did live past the key comparison, but not where this note guessed: the
T14 `/health` stage gate demanded `t2v` of an `upscale_only` pod that never loads a Wan pipeline.
`upscale.scale` was never involved. Fixed in `49394b1d`.
