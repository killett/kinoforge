...
the flag combinations that matter, run 2026-09-05/06 against the Modal provider: **31 PASS, 17
...
**The tier's stated goal is not met** (as measured; the `provision` half has since been fixed
offline in `8191bd1b` and awaits a live re-run). `deploy` → `provision` → `generate --instance-id` →
