...
failure preserved in Notes. **T2-03's does not:** the same re-run reproduced the warm-attach miss
(**U14**), cold-booting a second $2.50/hr A100 while the first sat idle at 0% GPU, so its own notes
describe a live defect and its verdict is **FAIL**. RunPod and SkyPilot are protected by the *same*
...
- **FlashVSR upscale on A100-80GB — after the `av<18` pin.** 4x to 1920×1920 (T2-01) and the
  ...
  *cell* is a FAIL, for the warm-attach miss its re-run reproduced — not for the upscale, which
  worked.)
...
applied and re-proven live; **T2-03 did not flip**, because the same re-run that fixed its encode
