...
**U17** (`destroy --id` cannot reap a Modal app killed mid-deploy), **U18** (`reap` short-circuits
on an empty ledger, so `--include-orphans` is unreachable for exactly the `--ephemeral` case),
...
| ~~**Nothing automatically reaps an idle ephemeral pod**~~ — the sweeper cannot see the index; `reap` sees it but can only ever say LIVE. **FIXED in `e582bd0f`, live-proven 2026-09-07**: the daemon reaped `eph-64dac102` at `age=119s idle on probe gpu_util=0.0% cpu=0.0%`. Two residuals: a row abandoned MID-BOOT has `endpoints: {}` so the util probe returns nulls and the pod stays LIVE, and `kinoforge reap` still never reaches `sweep()` on an empty ledger (**U18**) | T1-24, T1-26 | **U9** fixed, live-proven; **U18** |
