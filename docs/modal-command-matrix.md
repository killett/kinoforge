...
| ~~**`--attach-pod` cannot attach to a healthy pod whose endpoint the ledger is holding**~~ — the merge read `tags`, never `endpoints`; **FIXED offline in `ccd4c5e7`** (the ledger's `endpoints` are now seeded onto the instance before `ensure_endpoints`, live-wins-on-collision), **live proof still owed** — T2-02 has not been re-run | T2-02 | **U15** fixed offline |
| **The deploy-first lifecycle.** `deploy` crashes before booking anything; `provision` ~~books a pod no kinoforge command can see or destroy~~ — the `provision` half is **FIXED** offline in `8191bd1b` (durable pre-launch row, id-less create refused, double-book refused); awaits a live Modal re-run | T1-19, T1-20 | **U6**; **U7** fixed |
