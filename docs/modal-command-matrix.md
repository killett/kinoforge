...
FAIL, 9 EXPECTED-REFUSAL, 0 PENDING**, for **$3.25 of the $20 budget**. Five defects were fixed
in-session; the rest were filed in `PROGRESS.md`, where the list has since grown to **U1–U22** as
the fixes exposed their own boundaries. Verdict counts here are the post-2026-09-07 ones: T2-03
moved FAIL → PASS when U14 was fixed and live-proven.
...
**Five new items came out of those proofs and are open**, which is the honest shape of the result
rather than a clean sweep: **U16** (the ephemeral launch row is only a partial handle on RunPod),
**U17** (`destroy --id` cannot reap a Modal app killed mid-deploy), ~~**U18** (`reap`
...
**U19** (no in-pod capability term for interpolation), **U21** (`provision` has no
destroy-on-error path) and **U22** (the ephemeral orphan reap acts on one probe sample). **U20**
...
now fixed and live-proven** — see the 2026-09-07 update at the top. What is left of them is
narrower and named: recovery from a mid-deploy kill still needs the raw `modal app stop` (U17),
`provision` still leaves a pod up if its readiness poll or weight download fails (U21), and an
ephemeral pod abandoned MID-BOOT carries `endpoints: {}` so the reaper's probe cannot see it (U3's
blast radius).
...
(U1–U22) or a fix commit in its own row.
...
but reaping a mid-deploy app is **U17** and destroy-on-error is **U21**). `deploy` → `provision` → `generate --instance-id` →
