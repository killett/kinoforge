...
11. `2026-06-23 21:35:52` — [Diffusers WanPipeline Wan 2.2 T2V-A14B + per-transformer branch routing on RunPod (A100 80GB SXM) — t2v](#11-2026-06-23-213552--diffusers-wanpipeline-wan-22-t2v-a14b--per-transformer-branch-routing-on-runpod-a100-80gb-sxm--t2v)
   - See also: `2026-09-25 12:23:15` — **U60 live re-proof: a `target:`-spelled Wan 2.2 MoE cfg routes.** Pod `4gizxff8enqfeg` (secure pool), cfg `examples/configs/runpod-diffusers-wan-2_2-14b-t2v-lora-target-spelled.yaml` — byte-for-byte the strength-grid sibling except its two `branch:` keys are `target:`, so any difference is attributable to the spelling alone. Before U60's symmetric map this cfg was ACCEPTED at config load and then refused by the pod with HTTP 400 `branch_auto_disallowed_on_moe`, *after* the 25-30 min boot and ~70 GB fetch. Now: `lora-apply: pod 4gizxff8enqfeg accepted the 2-entry stack`, and `/lora/inventory` reports each adapter on the transformer it named — `civitai:2197303@2474081` -> `branch='high_noise'` / `adapter_name='lora_0_h'`, `civitai:2197303@2474073` -> `branch='low_noise'` / `adapter_name='lora_1_l'`. The `_h`/`_l` suffixes are the server's own encoding of the destination, so they corroborate the `branch` field independently. Equivalence to the `branch:` spelling is established OFFLINE and more rigorously than frames could manage: both cfgs resolve to identical `(ref, strength, branch, target)` tuples and both produced `capability_key=03a20caf5c2d` on live pods — neither cfg pins a seed, so a frame comparison could never have been pixel-exact. Output `output/20260925-122315_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4` (480x480 / 81f / 16fps / 979,972 B). GPU probed at 100% through denoise, never inferred from `est_spend`. Frame-QA PASS: meadow, waterfall, golden-hour backlight, over-shoulder glance, butterflies, light ribbons and god rays all present; clean temporal coherence on the push-in; no false-colour corruption; and the render is visibly STYLIZED against a photorealism prompt, i.e. the Arcane pair bound rather than merely loaded. Two pods destroyed, `kinoforge list` verified with BOTH required lines. **Two unrelated defects were flushed out getting here and are fixed in the same branch**: 17 long-boot RunPod cfgs were unpinned from the secure pool (U42 had recorded one; the community pool reclaimed the first attempt mid-fetch, ~$0.39), and U52's module-level `kinoforge.core.pod_paths` import had been killing EVERY RunPod diffusers pod at boot since that morning (~$0.12). Cumulative live spend for this proof ~$1.13. Evidence: `tests/live/evidence/2026-09-25-u60-target-routing/`. Same tuple `(runpod, DiffusersEngine, Wan-AI/Wan2.2-T2V-A14B-Diffusers, t2v)`.
...
22. `2026-07-08 22:12:07` — [Diffusers WanPipeline Wan 2.1 T2V-1.3B on Modal serverless GPU (A10) — t2v](#22-2026-07-08-221207--diffusers-wanpipeline-wan-21-t2v-13b-on-modal-serverless-gpu-a10--t2v)
    ...
    - See also: `2026-09-09 18:25:48` — **U3 live re-proof** (endpoint URL reachable from a fresh process), commit `e033b170`. Cold boot `run-20260909-182423` (A10, $1.10/hr): app deployed in **1.6 s** and first mp4 at **1 m 22 s** wall — the fastest cold start recorded for this tuple, courtesy of the §24 image-bake plus the §26 HF Volume cache. 480×480/33f/16fps. Util probe read **gpu=98 %** at uptime 21 s and **gpu=100 %** mid-generation, then **gpu=0.0 %** once the clip published — real compute, not wall-clock accrual. Frame-QA **PASS with soft flags**: coherent alpine meadow, backlit waterfall over mossy cliffs, volumetric god rays, figure tracking consistently across all five frames, no false colour and no temporal flicker; flags are stylised-painterly rather than photorealistic, no push-in-to-close-up or over-shoulder turn, no magical creatures, and minor anatomy artifacts on the figure at the frame edge — all normal for 1.3B at 480 px. Pod lifetime 18:24:23 → 18:27:26 (3 m 03 s), spend **~$0.055**, teardown verified from fresh processes (`kinoforge list` both lines + `modal app list` state `stopped`, 0 tasks, no ephemeral index). Same tuple `(modal, DiffusersEngine, Wan-AI/Wan2.1-T2V-1.3B-Diffusers, t2v)` — logged as a See-also, not a section, because the capability axis is unchanged; the clip is incidental evidence that the pod really served, which is the premise the endpoint reads rest on. **What the run was actually for:** `kinoforge status --id` from a process that did not create the pod now prints `endpoints={"8000": "https://<operator>--kinoforge-run-20260909-182423-build-modal-a-84522e.modal.run"} (recorded at launch, not verified live)` where it previously printed `unknown (no live endpoint)`, and `kinoforge pod lora ls` reached that same host over HTTP (exit 0, `no LoRAs loaded`) where it previously exited 2 with `no endpoint URL`. Directly relevant to the note on the See-also above: the ledger-endpoints workaround it describes is no longer needed — the CLI resolves the host itself.
23. `2026-07-08 23:55:31` — [Diffusers WanPipeline Wan 2.2 T2V-A14B on Modal serverless GPU (A100-80GB) — t2v](#23-2026-07-08-235531--diffusers-wanpipeline-wan-22-t2v-a14b-on-modal-serverless-gpu-a100-80gb--t2v)
    - See also: `2026-09-06 03:59:59` — Modal command matrix **Tier 3** (`docs/modal-command-matrix.md`, cell T3-01), one `--no-reuse` cold run of the same Wan 2.2 T2V-A14B stack on A100-80GB at $2.50/hr. Pod life **27m37s → $1.15**: app deployed in 84.8 s, then ~23 min of **cold** ~63 GB HF fetch (the `kinoforge-hf-cache` Volume held only the 1.3B model beforehand and holds A14B after, so a warm re-run should be ~$0.30), then ~3m06s of generation at `gpu_util_percent=100.0, cpu=6.0`. Output 480x480/81f/16fps/5.06 s, `output/20260906-035959_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4`, frame-QA **PASS** (coherent meadow + waterfall, correct golden-hour backlight, camera push-in and the over-the-shoulder turn both land, temporally stable; soft flags on a flag-like dress drape and an indistinct right hand). Teardown proven from new processes: both `kinoforge list` lines, empty `modal container list`, 0 non-stopped `kinoforge-*` apps. No new section — same `(modal, diffusers, Wan2.2-T2V-A14B-Diffusers, t2v)` tuple as this entry.
    - See also: `2026-09-17 20:04:57` — the plain single-command form of this recipe, run to verify a shell block an operator can paste as-is (`python -m kinoforge generate` under `eval "$(pixi shell-hook -e live-modal)"`, heredoc prompt, `--no-reuse`). Pod `run-20260917-195624`, life 19:56:24 → 20:05:02 (**8 m 38 s**), `✓ App deployed in 119.256s`, first mp4 at 8 m 33 s wall. **The ~63 GB HF fetch did not recur** — the `kinoforge-hf-cache` Volume already held A14B from the See-also above, so model load took **78 s** against that run's ~23 min, and the util probe read `gpu_util_percent=100.0, cpu_percent=5.9` at uptime 78 s and again at 153 s (real compute, not wall-clock accrual). **Non-standard prompt by operator request** (the chocolate-volcano prompt from the session brief, not `field-realistic.txt`), so this clip is deliberately NOT comparable to the standard-prompt corpus. Output `output/20260917-200457_diffusers_Wan2.2-T2V-A14B-Diffuser_A-giant-chocolate-vo.mp4` — 480×480/81f/16fps, 1,053,733 B, sha256 `8c8c1fda4623660374085665e5478df29d5efd7825765f228dced9252a3454a9`. Frame-QA **PASS with soft flags**: golden-hour valley, molten eruption on the horizon, three figures fleeing toward camera, chocolate slabs raining down; cinematic grade, temporally coherent, no false colour — flags are a near-black mid-clip frame (heavy ash occlusion), no skittles anywhere despite the prompt naming them, and mushy faces at 480 px. Teardown verified from a fresh process (both `kinoforge list` lines). Spend **est. $0.36** (pod wall-clock × published A100-80GB rate; billing NOT measured). Same tuple `(modal, DiffusersEngine, Wan-AI/Wan2.2-T2V-A14B-Diffusers, t2v)`.
24. `2026-07-10 23:52:23` — [FlashVSR v1.1 4x upscale on Modal A100-80GB via image-bake fast boot (Milestone 3) — upscale](#24-2026-07-10-235223--flashvsr-v11-4x-upscale-on-modal-a100-80gb-via-image-bake-fast-boot-milestone-3--upscale)
    - See also: `2026-09-06 10:00:20` — first FlashVSR run on Modal after the **`av<18`** pin (`82ad084b`), which closed urgent item **U12**: the same command had failed 3/3 hours earlier at the container's mp4 writer (`Cannot change width after codec is open.`) because the freshly-baked image had taken `av` 18.1.0. Command: `pixi run -e live-modal kinoforge upscale --config examples/configs/modal-diffusers-flashvsr-x4-upscale.yaml --video output/20260630-221857_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4` (warm-reuse, no `--no-reuse`, so the pod was held for the next two cells). Pod `upscale-20260906-093919` (A100-80GB, $2.50/hr); the pip-line change forced a **full image re-bake, `✓ App deployed in 1167.378s`** on Modal's builder (not A100 time), and the build log carries the proof: `Successfully installed … av-17.1.0`. Util probe `gpu_util_percent=100.0, cpu_percent=5.9` mid-job. Output `output/20260906-100020_upscaled_flashvsr_flashvsr-wan21-bfloat16_upscale.mp4` — **1920×1920, 16/1 fps, 77 frames, 4.8125 s, 6.42 MB** from the 480×480/81f fixture (the window trims 4 frames, as in this entry). Frame-QA **PASS** (`/home/claudeuser/kinoforge-matrix/sheetT2-01-rerun.png` and a 640 px native-crop comparison `cropcmp.png`): colour faithful with **no false-colour cast**, temporally coherent, and genuine detail synthesis against a nearest-neighbour blow-up of the same source region — resolved rock texture, individual hair strands, facial features, butterfly wing edges. A100 time ~10 min, **$0.42**; destroyed, teardown proven from a new process (both `kinoforge list` lines + 0 non-stopped `kinoforge-*` apps). Same tuple `(modal, flashvsr, flashvsr-wan21-bfloat16, upscale)`. Matrix cell T2-01, `docs/modal-command-matrix.md`.
25. `2026-07-11 17:59:51` — [RIFE v4.26 frame interpolation (16fps→60fps) on Modal T4 via image-bake fast boot (Milestone 4) — interpolate](#25-2026-07-11-175951--rife-v426-frame-interpolation-16fps60fps-on-modal-t4-via-image-bake-fast-boot-milestone-4--interpolate)
   - See also: `2026-09-11 00:07:51` — **U19 live proof: `kinoforge interpolate` warm-attach, the first one ever.** Two back-to-back runs of the same cfg (`examples/configs/modal-diffusers-rife-60fps-interpolate.yaml`, cfg-default 60 fps — NOT `--fps`-overridden, because the fps is part of the capability key and a differing one would have made the two runs unmatchable by construction) against the §25 fixture `output/20260630-221857_…_Photorealistic-cinem.mp4` (480²/81f/16fps). Run 1 cold-created pod `interpolate-20260911-000240` (Modal, `✓ App deployed in 161.523s` — a full image re-bake, since the embedded server module changed) → `output/20260911-000554_interpolated_rife_interp_interpolate.mp4`. Run 2, from a FRESH process 1 m 46 s later, logged `warm-reuse: attached to interpolate-20260911-000240` and produced `output/20260911-000751_…` in **11 s**, with `✓ App deployed` and `✓ Created objects` both at count **0**. **The attach is not the evidence** — before this change the `/health` gate was carved out for interpolate entirely, so an attach would have happened either way. The evidence is what the live pod answered: `GET /health` → `{"models":[{"name":"rife-rife426","on_device":"cuda","ready":true}],"capabilities":["interpolate","upload"]}` — the term U19 recorded as one no pod could ever advertise. `GET /util` at the same moment: `gpu_util_percent=0.0, cpu_percent=1.1, memory_percent=1.2, uptime_seconds=195` (idle, post-job — consistent with both jobs having finished). Both outputs 480×480 / 60 fps / 304 frames / 5.067 s, matching §25 exactly and the source's 5.0625 s duration to 4 ms. **Frame-QA PASS on both** (5-frame contact sheets against a source sheet): colour faithful to the source with no cast, subject pose and butterfly positions tracking the source frame-for-frame at every sampled timestamp, no ghosting or halo around the moving subject — RIFE's classic failure — and no warping in the waterfall spray. The two runs' sheets are indistinguishable from each other, which is the point: the warm-attached run is not a degraded one. ⚠️ Honest limit: 5 sampled frames prove fidelity and absence of artifacts at those points, not inter-frame smoothness. Pod destroyed explicitly; teardown verified from fresh processes (`kinoforge list` both lines, zero non-stopped Modal apps, preflight PASS). Spend **$0.036** (`est≤$0.0359` at destroy). Same tuple `(modal, DiffusersEngine, rife-rife426, interpolate)` — no new axis, so no new section; what is new is that the interpolate path now participates in warm reuse at all.
    - See also: `2026-09-06 03:05:22` — Modal command matrix **Tier 2b** (`docs/modal-command-matrix.md`, cells T2-06 / T2-07), two one-shot `--no-reuse` runs off the standard 480²/81f/16fps fixture. **T2-06** (`--fps 60`, pod `interpolate-20260906-030152`, T4): **3 m 30 s end to end including the first image bake**, published `output/20260906-030522_interpolated_rife_interp_interpolate.mp4` — 480×480, **60/1 fps, 304 frames, 5.066667 s** (3.75× the frames at 3.75× the rate, duration preserved). **T2-07** (`--fps 32`, pod `interpolate-20260906-030726`, image cached): **32 s**, published `output/20260906-030753_interpolated_rife_interp_interpolate.mp4` — 480×480, **32/1 fps, 162 frames = exactly 2× the source's 81**, duration bit-identical to the source; the CLI `--fps` override is honoured over the cfg's `fps: 60.0`. Frame-QA **PASS** on both (contact sheets `/home/claudeuser/kinoforge-matrix/frames/T2-06.png`, `T2-07.png`, judged against a sheet of the source): content, colour and sharpness match the source with no false-colour cast, ghosting or subject morphing. `--no-reuse` self-teardown fired on both, verified from a new process (`kinoforge list` both lines + **0 non-stopped `kinoforge-*` apps**). Spend ~$0.07 for the pair. Same tuple `(modal, rife, rife426, interpolate)`. NB the companion FlashVSR cells in the same campaign (Tier 2a) all failed at the container's mp4 writer — urgent item **U12** — so no See-also was added under §24 or §27 at the time; the `av<18` pin (`82ad084b`) fixed that later the same day and both See-alsos are now present.
    - See also: `2026-09-17 20:16:24` — stage 2 of a two-command operator recipe (upscale an existing clip, then interpolate it), and **the first RIFE run in this log whose input is 1080p rather than 480²**. That is the reason it is recorded: RIFE v4's IFNet pads H/W up to a multiple of 64 (1080→1088, the `Expected size 512 but got size 480` fix in this entry's boot log), and no prior run exercised that padding at this size. Pod `interpolate-20260917-201446`, life 20:14:46 → 20:16:29 (**1 m 43 s**), `✓ App deployed in 1.462s` (baked-image cache hit). Input is the 1080×1080/77f upscale from §27's 2026-09-17 See-also; output `output/20260917-201624_interpolated_rife_interp_interpolate.mp4` — **1080×1080/289f/60fps**, 4,029,178 B, sha256 `21665fcf4bb60b3b237dc87e50ad037644bff980461389fd55efcc35f0489b0c` (77 × 60/16 = 288.75 ✓). Frame-QA **PASS**, checked the way an interpolation actually has to be checked: **four *consecutive* frames (n=140-143), not evenly-spaced ones** — smooth sub-frame camera drift and butterfly motion, no ghosting, no doubled edges, no warping. Evenly-spaced sampling lands near the original keyframes and would have proven nothing about the synthesized intermediates; that is the trap to avoid when QA-ing any future interpolate. Teardown verified from a fresh process. Spend **est. $0.03**. Same tuple `(modal, rife, rife-rife49, interpolate)`.
...
27. `2026-07-12 20:13:28` — [FlashVSR height-target upscale (scale=1080p → 4x+downscale) on Modal A100-80GB — upscale](#27-2026-07-12-201328--flashvsr-height-target-upscale-scale1080p--4xdownscale-on-modal-a100-80gb--upscale)
    - See also: `2026-09-06 10:08:40` — the height-target half of the same post-`av<18` proof (matrix cell T2-03). `pixi run -e live-modal kinoforge upscale --config examples/configs/modal-diffusers-flashvsr-1080p-upscale.yaml --video output/20260630-221857_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4`, exit 0, publishing `output/20260906-100840_upscaled_flashvsr_flashvsr-wan21-bfloat16_upscale.mp4` — **1080×1080, 16/1 fps**, i.e. the 4x-then-downscale height resolution of this entry still lands after the pin. Frame-QA **PASS** (`/home/claudeuser/kinoforge-matrix/sheetT2-03-rerun.png`): correct colour, no false-colour cast, coherent motion, sharper than the source at every sampled frame. Pod `upscale-20260906-100335` (A100-80GB), `✓ App deployed in 1.700s` off the now-cached image, ~5 min, **$0.22**, destroyed with teardown proven from a new process. **Negative finding worth carrying:** this run was launched with an identical-capability-key pod (`upscale-20260906-093919`) warm and idle at 0% GPU, and the matcher **cold-booted anyway** — urgent item **U14**, now reproduced in two separate passes; the explicit `--attach-pod` escape hatch refused with a false "no endpoints" claim (new item **U15**), so two A100s billed concurrently. Same tuple `(modal, flashvsr, flashvsr-wan21-bfloat16, upscale)`.
    - See also: `2026-09-17 20:14:40` — stage 1 of the two-command operator recipe whose stage 2 sits under §25. Pod `upscale-20260917-200542`, life 20:05:42 → 20:14:45 (**9 m 03 s**), `✓ App deployed in 356.301s` — a genuine cold image bake, and the useful contrast is that the SAME cfg redeployed in **1.638 s** twenty minutes later once the image was cached (§30). Source is the standard 480²/81f fixture `output/20260630-221857_diffusers_Wan2.2-T2V-A14B-Diffuser_Photorealistic-cinem.mp4`. Output `output/20260917-201440_upscaled_flashvsr_flashvsr-wan21-bfloat16_upscale.mp4` — **1080×1080/77f/16fps**, 2,318,530 B, sha256 `9cccc174af9db715a44166c1516cdb6d0a04f3a4056943581b5efe448488b783` (the 81→77 window trim is this entry's documented behaviour). Frame-QA **PASS**: colour faithful with no false-colour cast, temporally coherent, genuine detail synthesis — individual meadow flowers, hair strands, waterfall texture, butterfly wing edges. Teardown verified from a fresh process. Spend **est. $0.38**. Same tuple `(modal, flashvsr, flashvsr-wan21-bfloat16, upscale)`.
...
31. `2026-09-18 00:45:08` — [MiniMax-H3 joint video+audio (t2va) on Modal H200 — the first soundtrack kinoforge has ever generated — t2va](#31-2026-09-18-004508--minimax-h3-joint-videoaudio-t2va-on-modal-h200--the-first-soundtrack-kinoforge-has-ever-generated--t2va)
    - See also: `2026-09-18 01:46:21` — **the LONGEST clip MiniMax-H3 can produce**, and the first run of `examples/configs/modal-diffusers-minimax-h3-t2va-long.yaml`: **345 frames / 14.375 s at 960x544**, aac stereo 32 kHz, `output/20260918-014621_diffusers_MiniMax-H3_Photorealistic-cinem.mp4` (4,295,182 B, sha256 `88cdfa698a37e83f...`). **15 s is unreachable** — the count snaps up to the next `17*n+5` and the 5-15 s window is checked on the ALIGNED value, so 360 aligns to 362 = 15.083 s and is refused; 345 is the ceiling. Pod `run-20260918-012850` (H200 $4.54/hr, `--no-reuse`), `✓ App deployed in 1.663s` off the cached image, launched 01:28:48 → `generate completed` 01:46:21, **~17.6 min, spend est. $1.33**. Audio duration is **exactly** 14.375000 s, matching the video to the millisecond. ⚠️ **But this soundtrack is effectively MONO** — L/R correlation **0.9956** against the §31 clip's 0.3190, L peak 0.5556 vs R 0.5539. That is not a mux defect (§31 proves decorrelated stereo passes through unchanged); it is this generation's content. ⚠️ **Frame QA found a HARD CUT at frame 272 (t = 11.33 s)**, which evenly-spaced sampling could not have distinguished from a fast dolly — frame-to-frame mean delta spikes to **70.05** against a median of 1.54 and p95 of 2.70, a 26x outlier, with the next largest transition only 3.8. So the clip is a two-shot sequence: ~11.3 s of a meadow mid-shot, then a cut to ~3.0 s of an extreme eyes-only close-up. At 5.167 s (§31) the same prompt produced ONE continuous push-in. **If you want continuous camera motion, use a short clip; at maximum length this model cuts.** Consecutive frames either side of the cut are smooth and uncorrupted. ⚠️ **`_flash_3_hub` is UNVERIFIED on this run.** The cfg requests it and the server reports the backend in force on `/health` — which is exactly why that field was added — and I failed to read `/health` while the pod was alive; Modal retains only the last ~100 log lines, so the startup line had rolled off by the time I looked. The timing argues it did NOT help: 345 frames at 960x544 is 102 latent frames x 510 rows = **52,020 video rows against §31's 37,296** (1.39x), and the denoise ran ~15.5 min against §31's ~8.5 min (**1.8x**). Attention is ~quadratic in rows (1.39² = 1.94x), so 1.8x is what **no** attention speed-up looks like. One `curl /health` on the next run settles it for $0 — do that before spending again on the question. Also note the first attempt at this cfg died for ~$0.16 on `CapabilityMismatch: profile drift on field 'max_frames': expected 360 got 345` — correcting a `capability` value strands the cached profile, and `verify()` runs AFTER the pod is booked (see PROGRESS). Same tuple `(modal, DiffusersEngine, MiniMaxAI/MiniMax-H3, t2va)`.
    - See also: `2026-09-18 15:22:49` — **first run at the 640x352 canvas** (`examples/configs/modal-diffusers-minimax-h3-t2va-long-640.yaml`, HEAD `9684af4e`), longest clip, operator prompt: **345 frames / 14.375 s at 640x352**, aac stereo 32 kHz, `output/20260918-152249_diffusers_MiniMax-H3_A-giant-chocolate-vo.mp4` (4,429,166 B, sha256 `3651e5e632d5b830...`). Pod `run-20260918-151232`, H200 $4.54/hr, `--no-reuse`: `App deployed` 1.5 s (cached image), `/health` at 15:15:58, **denoise 15:16→15:21 at a sustained `gpu=100.0` (~4.5 min against ~15.5 at 960x544)**, published 15:22:49, destroyed 15:22:54, teardown verified from fresh processes (`kinoforge list` both lines, 0 Modal apps); **~$0.78**. `tools/av_qa.py --expect-rate 32000 --cut-scan` **PASS** (peak 0.858, rms 0.153, L/R corr 0.812, no hard cuts — one continuous move, unlike the 960x544 max-length clip's cut at frame 272). Frame QA **PASS**: erupting volcano, continuous push/tilt down to a carpet of skittles with colourful gummy figures running toward camera; no false colour or artefacts at this scale. ⚠️ Soft flag: the "chocolate" reads as ordinary lava. **`GET /health` read `attention_backend: default`** — `_flash_3_hub` is NOT engaging on this image (the startup WARNING had rolled off Modal's 100-line log by the time it was read; capture it at boot on a future run). Why this canvas exists: FlashVSR's attention canvas is its 4x output, and a 960x544 source (3840x2176) OOMs every Modal card; 640x352 x4 sits just under the token budget §24 proved. H3 refuses 640x360 (both axes must be multiples of 32). Exact command (from `/workspace`):
      ```bash
      (
...
## 30. `2026-09-17 20:26:18` — Three-stage Modal chain in one shell block: Wan 2.2 T2V-A14B → FlashVSR 1080p → RIFE 60 fps across three cold pods — t2v+upscale+interpolate

| Field | Value |
|---|---|
| **Stack triple** | `Modal / DiffusersEngine + flashvsr + rife / Wan-AI/Wan2.2-T2V-A14B-Diffusers → flashvsr-wan21-bfloat16 → rife-rife49` |
| **Mode** | t2v (480x480/81f/16fps) → upscale (1080x1080/77f/16fps) → interpolate (1080x1080/289f/60fps) |
| **New capability axis** | **A three-stage Modal pipeline driven from a single operator shell block, each stage on its own cold `--no-reuse` pod, with the inter-stage hand-off resolved by the controller's own filesystem.** The individual stages are all logged already (§23 / §27 / §25); what had never been recorded is the *chain* — that `generate` → `upscale` → `interpolate` compose on Modal with no warm pod, no manual path copying, and no intermediate babysitting. §14 is the nearest prior art and differs in both provider (RunPod) and shape (co-resident stages on one warm pod). |
| **First-success SHA** | `d4054eb8` (working tree clean; **no production code changed for this axis** — every stage ran shipped configs through the shipped CLI) |
| **Date (local TZ)** | 2026-09-17 20:26:18 -0700 (PDT) |
| **GPU** | Stage 1 + 2: Modal **A100-80GB** (`placement.accelerators` leads `A100-80GB`, then `H100`). Stage 3: one of `T4` / `L4` / `A10` per the RIFE cfg's list. **The provider-side card is NOT recorded anywhere this run could read** — neither the ledger tags nor `/util` carry a GPU name, so the specific card is the cfg's declared preference, not a measurement. |
| **Runs** | 1 shell block × 3 cold pods. `run-20260917-201643` 20:16:43→20:23:38 (6 m 55 s); `upscale-20260917-202339` 20:23:39→20:25:26 (1 m 47 s); `interpolate-20260917-202527` 20:25:27→20:26:23 (56 s). Whole block 9 m 40 s wall. |
| **Est. spend** | **~$0.38 for this entry** (~$0.29 + ~$0.07 + ~$0.02), **~$1.15 across the whole 2026-09-17 session** including the §23/§25/§27 See-alsos. **These are estimates** — pod wall-clock × published Modal rate. Billing was not queried; do not cite them as measured. |
| **Layer / phase** | Operator-recipe verification: the session brief asked for paste-able Modal equivalents of two RunPod/Modal example blocks, tested before being proposed. |

### Exact commands

The whole pipeline is one block. Paste it as-is:
...
(
    ...
    # Stage 1 — generate (Wan 2.2 T2V-A14B, A100-80GB, own pod)
    python -m kinoforge generate \
        --config examples/configs/modal-diffusers-wan-2_2-14b-t2v.yaml \
        --mode t2v \
        --no-reuse \
        --prompt "$(cat <<'EOF'
A giant chocolate volcano erupts, showering the land with skittles. The Sour Patch Kids run for their lives. Photorealistic, cinematic lighting.
EOF
)"

    CLIP="$(find /workspace/output -name '*_diffusers_*.mp4' -newer "$MARK" \
        | sort | tail -1)"
    test -n "$CLIP" || { echo "stage 1 published no new file"; exit 1; }
    echo "stage 1 -> $CLIP"

    # Stage 2 — FlashVSR 4x -> lanczos downscale to 1080p (own pod)
    python -m kinoforge upscale \
        --config examples/configs/modal-diffusers-flashvsr-1080p-upscale.yaml \
        --video "$CLIP" \
        --no-reuse
    ...
    UPSCALED="$(find /workspace/output -name '*upscaled*.mp4' -newer "$MARK" \
        | sort | tail -1)"
    test -n "$UPSCALED" || { echo "stage 2 published no new file"; exit 1; }
    echo "stage 2 -> $UPSCALED"

    # Stage 3 — RIFE interpolate to 60 fps (own pod)
    python -m kinoforge interpolate \
        --config examples/configs/modal-diffusers-rife-60fps-interpolate.yaml \
        --video "$UPSCALED" \
        --fps 60 \
    ...
    FINAL="$(find /workspace/output -name '*interpolated*.mp4' -newer "$MARK" \
        | sort | tail -1)"
    echo "stage 3 -> $FINAL"
)
```
...
| Stage | Path | Shape | Bytes | sha256 |
|---|---|---|---|---|
| 1 generate | `output/20260917-202333_diffusers_Wan2.2-T2V-A14B-Diffuser_A-giant-chocolate-vo.mp4` | 480×480 / 81f / 16 fps | 1,112,648 | `1980844e92ef229f12e49c500b26debef6370d60d97f3a5d102bbdb3f7ef2463` |
| 2 upscale | `output/20260917-202521_upscaled_flashvsr_flashvsr-wan21-bfloat16_upscale.mp4` | 1080×1080 / 77f / 16 fps | 2,172,033 | `6b7b41f1d77e2fa23a300420b74e5cc33f9c1728c0a8e59b13eea96a6ebc8e68` |
| 3 interpolate | `output/20260917-202618_interpolated_rife_interp_interpolate.mp4` | 1080×1080 / 289f / 60 fps | 4,195,489 | `197ac615d00ce538a7d2f8329cb20ebc1b450a6722f5c053e886a604cc7feeed` |

### Frame QA — all three stages PASS

- **Stage 1 (480²)** — chocolate volcano erupting over a field of round candy, three children fleeing across it, lava-orange sky. Prompt adherence is notably *better* than the same prompt's solo run earlier the same session (§23 See-also): the "skittles" read as the candy field here, where the earlier seed produced none.
- **Stage 2 (1080²)** — **PASS with genuine detail synthesis**, verified the way this log requires: a 640 px native crop of the same region, source blown up 4× nearest-neighbour against the FlashVSR output. The upscale resolves facial features, individual candy spheres with specular highlights, hair strands, a shirt logo, and a sharp ridge line that the source renders as mush. Colour faithful, **no false-colour cast** (the entry-#13/#14 corruption mode).
- **Stage 3 (1080² @ 60 fps)** — **PASS**, checked on four *consecutive* frames rather than evenly-spaced ones: smooth sub-frame camera drift and candy motion, no ghosting, no doubled edges, no warping.
- Soft flags across the chain: the clip is dark overall, faces are plasticky at this scale, and the runners' anatomy wobbles in places. Normal for A14B at 480 px before upscale; none of it is a pipeline defect.

### What this run actually establishes

1. **Image caching turns the chain cheap on the second pass.** Stage 1 deployed in **1.320 s** here against **119.256 s** for the identical cfg 20 minutes earlier, stage 2 in **1.638 s** against **356.301 s**. The first run of any Modal cfg pays an image bake; the chain does not.
2. **The HF Volume cache is what makes A14B affordable.** Model load was 78 s, not the ~23 min cold fetch in §23. A cold `kinoforge-hf-cache` would roughly triple this entry's spend.
3. **`--no-reuse` on every stage is correct here and is not a warm-reuse regression.** Each stage is a different capability key, so a warm pod from stage 1 could not serve stage 2 anyway; destroying it immediately is strictly cheaper. Teardown verified from a fresh process after the block exited: `kinoforge list` → `[instance overview] No running instances.` **and** `No instances recorded in ledger.`
4. **The inter-stage glue must fail loudly, and `ls -t` does not.** The obvious `--video "$(ls -t output/*upscaled*.mp4 | head -1)"` silently feeds the *previous* run's file to the next stage whenever a stage fails — and `output/` already held three older `*upscaled*` files when this was written, so the failure would have looked like success. The `find -newer "$MARK"` + `test -n` form above fails the block instead. **Use it in any multi-stage recipe.**
